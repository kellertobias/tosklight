...
fn open_hardware_controls() -> Result<(), String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let app = debug
            .filter(|p| p.exists())
            .or_else(|| installed.filter(|p| p.exists()))
    ...
    {
        Command::new(executable.with_file_name(if cfg!(windows) {
...
#[tauri::command]
fn close_console_screen(app: tauri::AppHandle, screen_id: String) -> Result<(), String> {
...
#[tauri::command]
fn hide_console_screen(app: tauri::AppHandle, screen_id: String) -> Result<(), String> {
    if let Some(window) = app.get_webview_window(&format!("screen-{screen_id}")) {
...
#[tauri::command]
fn cancel_quit() {
    QUIT_ARMED.store(false, Ordering::Release);
}
#[tauri::command]
...
fn request_quit(app: &tauri::AppHandle) {
    if QUIT_ARMED.swap(true, Ordering::AcqRel) {
        let _ = app.emit("app-shutting-down", ());
        app.exit(0);
    } else {
...
fn open_console_screen(
    ...
    if let Some(value) = bounds {
        ...
        if let (Some(x), Some(y), Some(w), Some(h)) = (x, y, w, h) {
            builder = builder
                .position(x, y)
                .inner_size(w.max(640.0), h.max(480.0));
        }
    } else if let Some(monitor) = monitor {
        let p = monitor.position();
        builder = builder.position(f64::from(p.x) + 20.0, f64::from(p.y) + 20.0);
    }
    builder
...
fn server_address() -> SocketAddr {
    std::env::var("LIGHT_DESKTOP_TEST_BIND")
        .ok()
