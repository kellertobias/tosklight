...
describe("unified form enforcement",()=>{it("keeps ordinary fields out of the legacy Input compatibility wrapper",()=>{const offenders=Object.entries(sources).filter(([file])=>!file.endsWith("/components/control/CommandLineBar.tsx")&&!file.endsWith("/components/common/controls.tsx")).filter(([,source])=>[...source.matchAll(/<Input\b[^>]*>/g)].some(([tag])=>!/(?:type="(?:file|range|hidden)"|hidden)/.test(tag))).map(([file])=>file);expect(offenders).toEqual([]);});});
