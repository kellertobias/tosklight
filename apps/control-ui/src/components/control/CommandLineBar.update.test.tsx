...
const dispatch = vi.fn((action: { type: string; value?: boolean }) => {
	...
	if (action.type === "SET_STORE_ARMED")
		state.storeArmed = Boolean(action.value);
...
const server = {
	bootstrap: {
		...
		active_programmers: [] as Array<{
			session_id: string;
			values?: unknown[];
			group_values?: Record<string, Record<string, unknown>>;
			preload_pending?: unknown[];
			preload_group_pending?: Record<string, Record<string, unknown>>;
		...
		frame_rate_hz: 60,
		active_timecode: null as string | null,
	},
	...
	commandHistory: [
		{
			...
			command: "FIXTURE 2 AT FULL",
			status: "rejected",
			feedback: "Fixture 2 is not patched",
			source: "software",
			at: "2026-07-17T20:00:02Z",
		},
		{
			id: "entry-1",
			desk_id: "desk-a",
			...
			status: "accepted",
			feedback: "Applied to 1 target(s)",
			source: "osc",
...
describe("scoped command-line integration", () => {
	it("renders optimistic edits and waits for the revisioned write before Enter", async () => {
		...
		expect(replaceCommandLine).toHaveBeenCalledWith(DESK_ID, "FIXTURE 1", 1);
...
describe("Shift+Record Update gestures", () => {
	it("uses scoped recordable values and playback queue without stale bootstrap", () => {
		server.bootstrap.active_programmers = [
			{
				...
				values: [{}, {}, {}, {}],
				preload_pending: [{}, {}, {}, {}],
				preload_group_pending: { front: { intensity: {}, pan: {} } },
				preload_playback_pending: [{ action: "go", playback_number: 4 }],
			},
		];
		activity.current = {
			authority: "preload",
		...
		expect(screen.getByRole("button", { name: "REC" })).toHaveClass(
			"record-empty",
		);
	...
	it("uses gray for no timecode and blue only for a present timecode", () => {
		const { rerender } = render(<CommandLineBar />);
		expect(
			screen.getByText("No Timecode").closest(".timecode-status"),
		).toHaveClass("timecode-idle");
	...
	it("keeps the command-to-REC/Preload space free of Highlight status panels in both layouts", () => {
		...
		const assertNoHighlightPanel = () => {
			...
			expect(
				container.querySelector('[aria-label="Highlight status"]'),
			).not.toBeInTheDocument();
			expect(
				container.querySelector(
					".highlight-feedback,.command-highlight-feedback",
				),
			).not.toBeInTheDocument();
		};
	...
	it("keeps single, second-press, and long-press software gestures mutually exclusive", () => {
		...
		expect(
			dispatch.mock.calls.filter(
				([action]) =>
					action.type === "SET_UPDATE_ARMED" && action.value === true,
			),
		).toHaveLength(1);
