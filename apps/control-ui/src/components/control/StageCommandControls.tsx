...
export function StageCommandControls() {
  ...
  if (state.stageMode === "navigate") return <div className="parameter-controls stage-command-controls"><div className="family-tabs"><Button className="active">Navigate Stage</Button></div><div className="parameter-surfaces">
    ...
    <DualVerticalTouchFader encoder="X/Y Pan" primary={{ label: "X Pan", value: state.stagePanX + 100, maximum: 200, display: String(Math.round(state.stagePanX)), onChange: (value) => dispatch({ type: "SET_STAGE_NAVIGATION", panX: value - 100 }) }} secondary={{ label: "Y Pan", value: state.stagePanY + 100, maximum: 200, display: String(Math.round(state.stagePanY)), onChange: (value) => dispatch({ type: "SET_STAGE_NAVIGATION", panY: value - 100 }) }}/>
    {state.stageView === "3d" && <DualVerticalTouchFader encoder="Orbit" primary={{ label: "Orbit", value: state.stageOrbitX + 180, maximum: 360, display: `${Math.round(state.stageOrbitX)}°`, onChange: (value) => dispatch({ type: "SET_STAGE_NAVIGATION", orbitX: value - 180 }) }} secondary={{ label: "Orbit tilt", value: state.stageOrbitY + 90, maximum: 180, display: `${Math.round(state.stageOrbitY)}°`, onChange: (value) => dispatch({ type: "SET_STAGE_NAVIGATION", orbitY: value - 90 }) }}/>}
  </div></div>;
