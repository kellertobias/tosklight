...
afterEach(cleanup);
beforeEach(() => { mocks.savePlaybackSlot.mockReset().mockResolvedValue(true); mocks.clearPlaybackSlot.mockReset().mockResolvedValue(true); mocks.error = null; mocks.playbacks.cue_lists = [{ id: "cue-1", name: "Main sequence" }, { id: "cue-2", name: "Encore" }]; });
...
function colorTrigger(label: string) { return screen.getByText(label, { selector: "label", exact: true }).closest(".ui-form-field")!.querySelector(".ui-color-input-trigger") as HTMLButtonElement; }
...
describe("PlaybackConfigurationModal", () => {
  it("uses three tabs, title-bar Apply, Close, and no footer Cancel", () => {
    show();
    for (const tab of ["Function", "Behavior", "Layout"]) expect(screen.getByRole("button", { name: tab })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Apply" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Apply" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "Close playback configuration" })).toBeInTheDocument();
  ...
  it("shows scrollable two-column lists and groups the persisted special targets", () => {
    show();
    expect(screen.getByRole("radiogroup", { name: "Playback function" })).toBeInTheDocument();
    expect(document.querySelector(".ui-selection-tree")).toBeInTheDocument();
    expect(document.querySelectorAll(".ui-selection-list-scroll")).toHaveLength(2);
    expect(screen.getByRole("radiogroup", { name: "Cue List options" })).toBeInTheDocument();
    expect(screen.queryByText("Cue List", { selector: "label" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("radio", { name: "Speed Master" }));
    expect(screen.getByRole("radiogroup", { name: "Speed Group options" })).toBeInTheDocument();
    expect(screen.getByRole("radio", { name: "Speed Group E" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("radio", { name: "Special" }));
    expect(screen.getByRole("radiogroup", { name: "Special options" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("radio", { name: "Grand Master" }));
    fireEvent.click(screen.getByRole("button", { name: "Apply" }));
    return waitFor(() => expect(mocks.savePlaybackSlot).toHaveBeenCalledWith(2, 4, expect.objectContaining({ target: { type: "grand_master" } })));
  });
  ...
  it("keeps name and the responsive color dropdown on Function only", async () => {
    ...
    expect(screen.getAllByRole("option", { name: /^Use color #/ })).toHaveLength(16);
    expect(document.querySelector(".ui-color-dropdown-panel")?.closest(".playback-configuration-modal")).toBeNull();
    ...
    expect(screen.queryByText("Playback color", { selector: "label" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Apply" }));
    await waitFor(() => expect(mocks.savePlaybackSlot).toHaveBeenCalledWith(2, 4, expect.objectContaining({ color: "#8b5cf6" })));
  });

  it("uses the stage-style toggle for cue-list flash release and persists Behavior", async () => {
    show(); fireEvent.click(screen.getByRole("button", { name: "Behavior" }));
    expect(screen.getByRole("radiogroup", { name: "When Flash or Swap is released" })).toBeInTheDocument();
    expect(screen.getByText(/leaves this Cue List active at zero intensity/)).toBeInTheDocument();
    expect(screen.getByRole("radio", { name: "Release all" })).toHaveAttribute("aria-checked", "true");
    fireEvent.click(screen.getByRole("radio", { name: "Intensity only" }));
    fireEvent.click(screen.getByRole("switch", { name: "Turn off when other playbacks take full control" }));
    fireEvent.click(screen.getByRole("switch", { name: "Protect from Swap" }));
    fireEvent.click(screen.getByRole("button", { name: "Apply" }));
    await waitFor(() => expect(mocks.savePlaybackSlot).toHaveBeenCalledWith(2, 4, expect.objectContaining({ flash_release: "release_intensity_only", auto_off: false, protect_from_swap: true })));
  });
  ...
  it("resets incompatible mappings and explains choices in an additional modal", async () => {
    ...
    expect(selectTrigger("Middle button")).toHaveTextContent("Select dereferenced");
    expect(selectTrigger("Bottom button")).toHaveTextContent("Flash");
    fireEvent.click(selectTrigger("Top button"));
    const dialog = screen.getByRole("dialog", { name: "Choose Top button function" });
    expect(within(dialog).getByRole("heading", { name: "Selection" })).toBeInTheDocument();
    expect(within(dialog).getByRole("heading", { name: "Temporary State" })).toBeInTheDocument();
    expect(within(dialog).getByRole("button", { name: "Empty Button" })).toBeInTheDocument();
    expect(within(dialog).getByText("Selects this playback or its live Group reference without executing it.")).toBeInTheDocument();
    fireEvent.click(within(dialog).getByRole("button", { name: "Close Top button function choices" }));
    expect(selectTrigger("Fader")).toBeDisabled();
    expect(selectTrigger("Fader")).toHaveTextContent("Group intensity master");
    fireEvent.click(screen.getByRole("button", { name: "Apply" }));
    await waitFor(() => expect(mocks.savePlaybackSlot).toHaveBeenCalledWith(2, 4, expect.objectContaining({ target: { type: "group", group_id: "group-1" }, buttons: ["select", "select_dereferenced", "flash"], fader: "master" })));
  });

  it("groups button and fader functions and moves Empty Button to the title", () => {
    show(); fireEvent.click(screen.getByRole("button", { name: "Layout" }));
    expect(selectTrigger("Top button")).toHaveTextContent("GO −");
    expect(selectTrigger("Middle button")).toHaveTextContent("GO +");
    fireEvent.click(selectTrigger("Top button"));
    let dialog = screen.getByRole("dialog", { name: "Choose Top button function" });
    for (const heading of ["Step Control", "Permanent State", "Temporary State", "Selection"]) expect(within(dialog).getByRole("heading", { name: heading })).toBeInTheDocument();
    expect(within(dialog).getByRole("button", { name: /^FFW/ })).toBeInTheDocument();
    expect(within(dialog).getByRole("button", { name: /^FRW/ })).toBeInTheDocument();
    expect(within(dialog).queryByRole("button", { name: "Disabled" })).not.toBeInTheDocument();
    fireEvent.click(within(dialog).getByRole("button", { name: "Empty Button" }));
    ...
    expect(within(dialog).getByRole("heading", { name: "Level Control" })).toBeInTheDocument();
    expect(within(dialog).getByRole("heading", { name: "Cue Transition" })).toBeInTheDocument();
    expect(within(dialog).queryByRole("button", { name: "Empty Button" })).not.toBeInTheDocument();
  });

  it("previews None as inactive and clears only when Apply is pressed", async () => {
    const close = show(); fireEvent.click(screen.getByRole("radio", { name: "None" }));
    ...
    expect(close).toHaveBeenCalledOnce(); expect(mocks.clearPlaybackSlot).not.toHaveBeenCalled();
    cleanup(); show(); fireEvent.click(screen.getByRole("radio", { name: "None" })); fireEvent.click(screen.getByRole("button", { name: "Apply" }));
  ...
  it("renders exactly one control and no fader for a virtual topology", () => {
    ...
    expect(screen.getByRole("dialog", { name: "Playback Configuration" })).toHaveAttribute("data-topology", "1 button · faderless");
  ...
  it.each(["programmer_fade", "cue_fade"] as const)("applies Double, Half, and Off immediately for the %s time master", (type) => {
    show(withFunctionDefaults(base, type, "cue-1", "group-1")); fireEvent.click(screen.getByRole("button", { name: "Layout" }));
    ...
    for (const label of ["Top button", "Middle button", "Bottom button"]) expect(selectTrigger(label)).toBeEnabled();
    fireEvent.click(selectTrigger("Top button"));
    expect(within(screen.getByRole("dialog", { name: "Choose Top button function" })).getByRole("heading", { name: "Time Control" })).toBeInTheDocument();
    expect(selectTrigger("Fader")).toBeDisabled();
  ...
  it("persists mutually exclusive virtual presentation", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Apply" }));
    await waitFor(() => expect(mocks.savePlaybackSlot).toHaveBeenCalledWith(2, 4, expect.objectContaining({ presentation_image: "show://images/blue-wash.png", presentation_icon: undefined })));
  });

  it("migrates legacy topology deterministically and clears hidden actions", () => {
    expect(normalizePlaybackTopology({ ...base, button_count: undefined, has_fader: undefined }, 2, false)).toMatchObject({ button_count: 2, has_fader: false, buttons: ["go_minus", "go", "none"] });
  });
});
