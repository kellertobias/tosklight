...
export function NumericPad({ demo = false }: { demo?: boolean } = {}) {
  ...
  const command = useCommandLineSurface({ selection: true, observeCommand: false });
  ...
  const toggleRecord = () => {
    ...
    else if (/^RECORD\b/i.test(currentCommand.text)) void command.replace(currentCommand.text.replace(/^RECORD\s*/i, ""), false);
  };
  ...
  const press = (key: SoftwareKey) => {
    ...
    if (state.shiftArmed) {
      ...
      if (key === "TIME") {
        const current = currentCommand.text.trim();
        const nextCommand = currentCommand.pristine || current === "FIXTURE" || current === "GROUP"
          ? "SPD GRP"
          : `${current} SPD GRP`;
        void command.replace(nextCommand, false);
        return;
      }
      if (key === "CLR" || key === "DEL") {
    ...
    if (key === "SET" && currentCommand.pristine && state.builtIn === "patch") return dispatch({ type: "SET_PATCH_ARMED", value: !state.patchSetArmed });
    if (key === "SET" && currentCommand.pristine && document.querySelector(".cue-settings-compact-fallback")) {
      window.dispatchEvent(new CustomEvent("light:desk-action", { detail: "set" }));
      return;
    }
    if (key === "SET" && currentCommand.pristine && document.querySelector(".cuelist-window.pool-window")) {
      if (state.storeArmed) dispatch({ type: "SET_STORE_ARMED", value: false });
      return dispatch({ type: "SET_CUELIST_SET_ARMED", value: !state.cueListSetArmed });
    }
    if (key === "SET" && currentCommand.pristine && document.querySelector(".playback-fader-bank,.virtual-playback-grid")) return dispatch({ type: "SET_PLAYBACK_SET_ARMED", value: !state.playbackSetArmed });
    if (key === "SET" && currentCommand.pristine && (state.builtIn === "presets" || (state.builtIn == null && state.desks.find((desk) => desk.id === state.activeDeskId)?.panes.some((pane) => pane.kind === "presets")))) return dispatch({ type: "SET_PRESET_SET_ARMED", value: !state.presetSetArmed });
  ...
  return <div className={`numeric-pad programmer-number-block ${demo ? "demo-number-block" : ""}`}>
    <div className="numeric-pad-section numeric-pad-command-section">
      {demo ? <>
        ...
        <Button className="demo-escape" style={{ gridColumn: 2, gridRow: 2 }} onClick={() => void command.reset()}>ESCAPE</Button>
