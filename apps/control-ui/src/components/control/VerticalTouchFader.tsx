...
export function VerticalTouchFader({ label, value, maximum = 100, display, disabled = false, accentColor, mode, directInput = false, onChange }: { label: string; value: number; maximum?: number; display?: string; disabled?: boolean; accentColor?: string; mode?: string; directInput?: boolean; onChange?: (value: number) => void }) {
  ...
  return <label onClick={() => hardware && openInput()} className={`vertical-touch-fader ${disabled ? "disabled" : ""} ${directInput ? "direct-input-fader" : ""}`} style={{ "--fader-level": fraction, "--fader-color": accentColor ?? "#176777", "--fader-color-dark": accentColor ? `color-mix(in srgb, ${accentColor} 42%, #081014)` : "#103039" } as CSSProperties}>
    ...
    {directInput && !hardware && <Button type="button" className="set-value-button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); openInput(); }}>Set value</Button>}
