...
export function GroupStrip() {
  ...
  return <section className="group-strip"><header><b>Group shortcuts</b><small>slots 1–{slotCount}</small></header><ButtonGrid ref={gridRef} className="card-pool group-shortcut-grid" style={{ "--group-shortcut-columns": slotCount } as React.CSSProperties}>{visible.map((group, index) => <Button onDoubleClick={() => group && void server.selectGroup(group.id, true)} onClick={() => { if (group && !state.storeArmed) return void server.selectionGesture({ type: "live_group", group_id: group.id }); if (group) { if (!group.body.fixtures.length) { recordGroupCommand(group.id); dispatch({ type: "SET_STORE_ARMED", value: false }); } else { setRecordGroup(group.id); } return; } if (!state.storeArmed) return; recordGroupCommand(String(index + 1)); dispatch({ type: "SET_STORE_ARMED", value: false }); }} className={`group-card pool-cell ${server.selectedGroupId === group?.id ? "selected" : ""} ${group ? "" : "empty"} ${state.storeArmed && !group ? "store-target" : ""}`} key={group?.id ?? `empty-${index + 1}`}><span className="number">{index + 1}</span><b>{group?.body.name ?? "Empty"}</b><small>{group ? group.body.fixtures.length ? `${group.body.fixtures.length} fixtures` : "Group is empty" : state.storeArmed ? "Tap to record" : "Press Rec first"}</small></Button>)}</ButtonGrid>{recordTarget && <RecordModeDialog target={recordTarget.body.name ?? `Group ${recordTarget.id}`} onChoose={recordExistingGroup} onCancel={cancelRecording}/>}</section>;
}
