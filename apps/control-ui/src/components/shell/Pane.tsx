...
export function Pane({
  ...
  const stageActions = pane.kind === "stage" ? [
    state.stageMode === "setup"
      ? [
          { id: "import", label: "Import Scene", onClick: () => window.dispatchEvent(new CustomEvent("light:import-stage-scene", { detail: pane.id })) },
          ...((pane.stageView ?? state.stageView) === "3d" ? [{ id: "add", label: "Add Element", onClick: () => window.dispatchEvent(new CustomEvent("light:add-stage-element", { detail: pane.id })) }] : []),
        ]
