...
export function LeftDock() {
  ...
  return <aside className="left-dock">
    <Button className={`dock-identity ${activeShow?.revision_copy ? "revision-copy-active" : ""}`} aria-label={`Open show menu. ${identityDetail}`} title={identityDetail} onClick={() => dispatch({ type: "SET_MODAL", modal: "setupOpen", value: true })}><span className="app-mark"><img src={appIcon} alt="" /></span><Clock/><b><span className={`show-status-dot ${showIndicator.className}`} aria-hidden="true">●</span> {showIdentity}</b></Button>
