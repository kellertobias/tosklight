  ...
  return <div className="stacked-modal-layer fixture-profile-editor-layer" onPointerDown={(event) => event.target === event.currentTarget && requestClose()}>
    <section className="nested-modal fixture-profile-editor-modal" role="dialog" aria-modal="true" aria-label={initialProfile.revision ? "Edit fixture profile" : "Create fixture profile"}>
      <ModalTitleBar
        ...
        actions={<Fragment>{tab === "modes" && <Button onClick={addMode}>Add mode</Button>}<Button variant="primary" loading={busy} onClick={requestSave}>Save fixture</Button></Fragment>}
      ...
      <div className="fixture-profile-editor-body">
        ...
        {tab === "generic" && <div className="fixture-generic-tab">
          ...
          <section className="fixture-notes-assets"><div><h3>Notes</h3><TextAreaField label="Fixture notes" rows={9} value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })}/></div><div><h3>Fixture photograph</h3><AssetField label="Photograph" preview="image" value={draft.photograph_asset} extensions={["png", "jpg", "jpeg", "gif", "webp"]} onChange={(photograph_asset) => setDraft({ ...draft, photograph_asset })}/></div><div><h3>Visualizer</h3><AssetField label="Visualizer GLB model" preview="glb" value={draft.model_asset} extensions={["glb"]} onChange={(model_asset) => setDraft({ ...draft, model_asset })}/></div></section>
        ...
        {tab === "modes" && <div className="fixture-modes-tab">
          <section className="fixture-mode-list">{draft.modes.map((mode) => <article key={mode.id} data-mode-reorder-id={mode.id}>
            <span className="drag-handle touch-drag-handle" aria-hidden="true" title="Drag to reorder modes" onPointerDown={(event) => { event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (!event.currentTarget.hasPointerCapture(event.pointerId)) return; const target = document.elementFromPoint(event.clientX, event.clientY)?.closest<HTMLElement>("[data-mode-reorder-id]")?.dataset.modeReorderId; if (target) moveModeById(mode.id, target); }} onPointerUp={(event) => event.currentTarget.hasPointerCapture(event.pointerId) && event.currentTarget.releasePointerCapture(event.pointerId)} onPointerCancel={(event) => event.currentTarget.hasPointerCapture(event.pointerId) && event.currentTarget.releasePointerCapture(event.pointerId)}>⠿</span>
            <TextField label="Mode name" required value={mode.name} onChange={(event) => updateMode({ ...mode, name: event.target.value })}/>
            <Button className="fixture-mode-edit" aria-label={`Edit channels for ${mode.name || "unnamed mode"}`} onClick={() => { setModeEditorId(mode.id); setOpenSplit(mode.splits[0]?.number ?? 1); setModeTab("channels"); }}>Edit</Button>
            <Button className="fixture-mode-delete" variant="danger" iconOnly aria-label={`Remove ${mode.name}`} disabled={draft.modes.length === 1} title={draft.modes.length === 1 ? "The final mode cannot be removed" : "Remove mode"} onClick={() => deleteMode(mode.id)}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M4 7h16M9 7V4h6v3m-9 0 1 13h10l1-13M10 11v5m4-5v5"/></svg></Button>
            <LargeTextField className="fixture-mode-notes" label="Mode notes" value={mode.notes} onChange={(event) => updateMode({ ...mode, notes: event.target.value })}/>
          </article>)}</section>
        </div>}
      </div>
    </section>
...
function GlbAssetPreview({ value }: { value: string }) {
  ...
  return <div className="fixture-glb-preview"><div ref={host} aria-label="Visualizer GLB model preview" title="Drag to rotate; scroll to zoom"/><small>Drag to rotate · Scroll to zoom</small><small role={error ? "alert" : "status"}>{error ?? metadata}</small></div>;
}
...
function ChannelsEditor({ mode, attributeRegistry, openSplit, onOpenSplit, onChange }: { mode: FixtureMode; attributeRegistry: AttributeDescriptor[]; openSplit: number; onOpenSplit: (split: number) => void; onChange: (mode: FixtureMode) => void }) {
  ...
  const editedChannel = mode.channels.find((channel) => channel.id === editingChannelId) ?? null;
  const setChannel = (channel: FixtureChannel) => {
    onChange({ ...mode, channels: mode.channels.map((candidate) => candidate.id === channel.id ? channel : candidate) });
  ...
  const renderSplit = (split: number) => {
    ...
    return <div className="fixture-channel-split"><div className="fixture-channel-table-wrap"><table className="fixture-channel-table fixture-channel-summary-table"><thead><tr><th>Slot</th><th>Head</th><th>Attribute</th><th>Resolution</th><th>Default</th><th>Highlight</th><th>Functions</th><th>Order</th></tr></thead><tbody>{channels.map((channel, index) => { const head = mode.heads.find((candidate) => candidate.id === channel.head_id); const open = () => setEditingChannelId(channel.id); return <tr key={channel.id} className="fixture-channel-row" data-channel-reorder-id={channel.id} draggable onDragStart={() => setDragChannel(channel.id)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); dropChannel(channel); }}><td className="channel-primary-slot"><span className="drag-handle touch-drag-handle" aria-hidden="true" title="Drag to reorder channels" onPointerDown={(event) => { if (event.pointerType === "mouse") return; event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (event.pointerType === "mouse" || !event.currentTarget.hasPointerCapture(event.pointerId)) return; const target = document.elementFromPoint(event.clientX, event.clientY)?.closest<HTMLElement>("[data-channel-reorder-id]")?.dataset.channelReorderId; if (target) moveChannelById(channel.id, target); }} onPointerUp={(event) => event.currentTarget.hasPointerCapture(event.pointerId) && event.currentTarget.releasePointerCapture(event.pointerId)}>⠿</span><Button aria-label={`Edit ${channel.attribute} channel`} onClick={open}>{primary.slots.get(channel.id) ?? "!"}</Button></td><td><Button onClick={open}>{head?.name || "Missing head"}</Button></td><td><Button onClick={open}>{channel.behavior === "static" ? "Static" : attributeRegistry.find((item) => item.id === channel.attribute)?.label ?? channel.attribute}</Button></td><td><Button onClick={open}>{channel.resolution.slice(1)} bit</Button></td><td><Button onClick={open}>{channel.default_raw}</Button></td><td><Button onClick={open}>{channel.highlight_raw}</Button></td><td><Button onClick={open}>{channel.functions.length}</Button></td><td><div className="reorder-actions"><Button iconOnly aria-label={`Move ${channel.attribute} up`} disabled={index === 0} onClick={() => moveChannel(channel, -1)}>▲</Button><Button iconOnly aria-label={`Move ${channel.attribute} down`} disabled={index === channels.length - 1} onClick={() => moveChannel(channel, 1)}>▼</Button><Button iconOnly aria-label={`Remove ${channel.attribute}`} onClick={() => onChange({ ...mode, channels: mode.channels.filter((candidate) => candidate.id !== channel.id), control_actions: mode.control_actions.map((action) => ({ ...action, assignments: action.assignments.filter((assignment) => assignment.channel_id !== channel.id) })).filter((action) => action.assignments.length), color_systems: mode.color_systems.map((system) => removeColorChannel(system, channel.id)).filter((system): system is HeadColorSystem => Boolean(system)) })}>×</Button></div></td></tr>; })}</tbody></table></div>{!channels.length && <p className="empty-editor-message">No logical channels are assigned to split {split}.</p>}<Button onClick={() => addChannel(split)}>Add channel</Button></div>;
  };
  return <div className="fixture-channels-editor">
    <section className="fixture-channel-split-manager"><header><h3>Address splits</h3><Button onClick={addSplit}>Add split</Button></header><div className="fixture-split-list">{mode.splits.map((split, index) => { const used = mode.channels.some((channel) => channel.split === split.number); return <article key={split.number}><strong>Split {split.number}</strong><NumberField label="Footprint" min={1} max={512} value={split.footprint} onChange={(event) => onChange({ ...mode, splits: mode.splits.map((candidate) => candidate.number === split.number ? { ...candidate, footprint: Number(event.target.value) } : candidate) })}/><Button iconOnly aria-label={`Move split ${split.number} up`} disabled={index === 0} onClick={() => onChange({ ...mode, splits: reorder(mode.splits, index, index - 1) })}>▲</Button><Button iconOnly aria-label={`Move split ${split.number} down`} disabled={index === mode.splits.length - 1} onClick={() => onChange({ ...mode, splits: reorder(mode.splits, index, index + 1) })}>▼</Button><Button iconOnly aria-label={`Remove split ${split.number}`} disabled={mode.splits.length === 1 || used} title={used ? "Reassign its channels before removing this split" : "Remove split"} onClick={() => onChange({ ...mode, splits: mode.splits.filter((candidate) => candidate.number !== split.number) })}>×</Button></article>; })}</div></section>
    ...
    {editedChannel && <ChannelEditorModal mode={mode} channel={editedChannel} attributeRegistry={attributeRegistry} onChange={setChannel} onResolution={(resolution) => changeResolution(editedChannel, resolution)} onClose={() => setEditingChannelId(null)}/>} 
  </div>;
}
...
function attributeOptions(registry: AttributeDescriptor[], current: string) {
  const options = registry.map((descriptor) => ({ value: descriptor.id, label: `${descriptor.family} · ${descriptor.label}` }));
  if (current && !options.some((option) => option.value === current)) options.push({ value: current, label: current });
  return options;
}

function ChannelEditorModal({ mode, channel, attributeRegistry, onChange, onResolution, onClose }: { mode: FixtureMode; channel: FixtureChannel; attributeRegistry: AttributeDescriptor[]; onChange: (channel: FixtureChannel) => void; onResolution: (resolution: ChannelResolution) => void; onClose: () => void }) {
  ...
  const setRole = (value: string) => onChange(value === "__static" ? { ...channel, behavior: "static" } : { ...applyCanonicalChannelAttribute(channel, value, attributeRegistry), behavior: "controlled" });
  const setSecondary = (index: number, value: number) => { const secondary_slots = [...channel.secondary_slots]; secondary_slots[index] = value; onChange({ ...channel, secondary_slots }); };
  return <div className="stacked-modal-layer fixture-channel-editor-layer" onPointerDown={(event) => event.target === event.currentTarget && onClose()}><section className="nested-modal fixture-channel-editor-modal" role="dialog" aria-modal="true" aria-label={`Edit ${channel.attribute} channel`}><ModalTitleBar title="Edit channel" actions={<Button onClick={() => setFunctionsOpen(true)}>Channel functions ({channel.functions.length})</Button>} closeLabel="Close channel editor" onClose={onClose}/><div className="fixture-channel-editor-body">
    <FormLayout columns={3} minColumnWidth={210}>
      <SelectField label="Channel role" value={role} options={[{ value: "__static", label: "Static output" }, ...attributeOptions(attributeRegistry, channel.attribute)]} onChange={setRole}/>
      <SelectField label="Address split" value={String(channel.split)} options={mode.splits.map((split) => ({ value: String(split.number), label: `Split ${split.number}` }))} onChange={(value) => onChange({ ...channel, split: Number(value) })}/>
      <SelectField label="Logical head" value={channel.head_id} options={mode.heads.map((head) => ({ value: head.id, label: head.name }))} onChange={(head_id) => onChange({ ...channel, head_id })}/>
      <SelectField label="Resolution" value={channel.resolution} options={RESOLUTIONS.map((value) => ({ value, label: value.slice(1) + " bit" }))} onChange={onResolution}/>
      {[0, 1, 2].map((index) => components > index ? <NumberField key={index} label={`${["Fine", "Third byte", "Fourth byte"][index]} slot`} min={1} max={512} value={channel.secondary_slots[index] ?? ""} onChange={(event) => setSecondary(index, Number(event.target.value))}/> : null)}
      <NumberField label="Default raw" min={0} max={maxRaw(channel.resolution)} value={channel.default_raw} onChange={(event) => onChange({ ...channel, default_raw: Number(event.target.value) })}/>
      <NumberField label="Highlight raw" min={0} max={maxRaw(channel.resolution)} value={channel.highlight_raw} onChange={(event) => onChange({ ...channel, highlight_raw: Number(event.target.value) })}/>
      <NumberField label="Physical minimum" allowDecimal value={channel.physical_min ?? ""} onChange={(event) => onChange({ ...channel, physical_min: optionalNumber(event.target.value) })}/>
      <NumberField label="Physical maximum" allowDecimal value={channel.physical_max ?? ""} onChange={(event) => onChange({ ...channel, physical_max: optionalNumber(event.target.value) })}/>
      <TextField label="Physical unit" value={channel.unit ?? ""} onChange={(event) => onChange({ ...channel, unit: event.target.value || null })}/>
    </FormLayout>
    <div className="fixture-channel-flags"><CheckboxField label="Invert" checked={channel.invert} onChange={(event) => onChange({ ...channel, invert: event.target.checked })}/><CheckboxField label="Snap (never fades)" checked={channel.snap} onChange={(event) => onChange({ ...channel, snap: event.target.checked })}/><CheckboxField label="Reacts to virtual intensity" checked={channel.reacts_to_virtual_intensity} onChange={(event) => onChange({ ...channel, reacts_to_virtual_intensity: event.target.checked })}/><CheckboxField label="Reacts to sequence master" checked={channel.reacts_to_sequence_master} onChange={(event) => onChange({ ...channel, reacts_to_sequence_master: event.target.checked })}/><CheckboxField label="Reacts to group master" checked={channel.reacts_to_group_master} onChange={(event) => onChange({ ...channel, reacts_to_group_master: event.target.checked })}/><CheckboxField label="Reacts to grand master" checked={channel.reacts_to_grand_master} onChange={(event) => onChange({ ...channel, reacts_to_grand_master: event.target.checked })}/></div>
  </div>{functionsOpen && <ChannelFunctionsModal channel={channel} attributeRegistry={attributeRegistry} actionIds={mode.control_actions} onChange={onChange} onClose={() => setFunctionsOpen(false)}/>}</section></div>;
}

function ChannelFunctionsModal({ channel, attributeRegistry, actionIds, onChange, onClose }: { channel: FixtureChannel; attributeRegistry: AttributeDescriptor[]; actionIds: Array<{ id: string; name: string }>; onChange: (channel: FixtureChannel) => void; onClose: () => void }) {
  return <div className="stacked-modal-layer fixture-functions-editor-layer" onPointerDown={(event) => event.target === event.currentTarget && onClose()}><section className="nested-modal fixture-functions-editor-modal" role="dialog" aria-modal="true" aria-label="Channel functions"><ModalTitleBar title="Channel functions" actions={<Button onClick={() => onChange({ ...channel, functions: [...channel.functions, blankFunction(channel)] })}>Add function</Button>} closeLabel="Close channel functions" onClose={onClose}/><div className="fixture-functions-editor-body">{!channel.functions.length && <p className="empty-editor-message">No functions are configured for this channel.</p>}{channel.functions.map((fn, index) => <article key={fn.id}><TextField label="Function name" value={fn.name} onChange={(event) => onChange({ ...channel, functions: channel.functions.map((candidate) => candidate.id === fn.id ? { ...candidate, name: event.target.value } : candidate) })}/><SelectField label="Function attribute" value={fn.attribute} options={attributeOptions(attributeRegistry, fn.attribute)} onChange={(attribute) => onChange({ ...channel, functions: channel.functions.map((candidate) => candidate.id === fn.id ? { ...candidate, attribute } : candidate) })}/><NumberField label="DMX from" min={0} max={maxRaw(channel.resolution)} value={fn.dmx_from} onChange={(event) => onChange({ ...channel, functions: channel.functions.map((candidate) => candidate.id === fn.id ? { ...candidate, dmx_from: Number(event.target.value) } : candidate) })}/><NumberField label="DMX to" min={0} max={maxRaw(channel.resolution)} value={fn.dmx_to} onChange={(event) => onChange({ ...channel, functions: channel.functions.map((candidate) => candidate.id === fn.id ? { ...candidate, dmx_to: Number(event.target.value) } : candidate) })}/><NumberField label="Priority" value={fn.priority} onChange={(event) => onChange({ ...channel, functions: channel.functions.map((candidate) => candidate.id === fn.id ? { ...candidate, priority: Number(event.target.value) } : candidate) })}/><SelectField label="Function behavior" value={fn.behavior.type} options={[{ value: "continuous", label: "Continuous mapping" }, { value: "fixed", label: "Named fixed value" }, { value: "indexed", label: "Indexed color or gobo" }, { value: "control", label: "Control action" }]} onChange={(type) => onChange({ ...channel, functions: channel.functions.map((candidate) => candidate.id === fn.id ? replaceFunctionBehavior(candidate, type, channel) : candidate) })}/><FunctionBehaviorEditor behavior={fn.behavior} modeChannel={channel} actionIds={actionIds} onChange={(behavior) => onChange({ ...channel, functions: channel.functions.map((candidate) => candidate.id === fn.id ? { ...candidate, behavior } : candidate) })}/><div className="reorder-actions"><Button iconOnly aria-label={`Move function ${fn.name} up`} disabled={index === 0} onClick={() => onChange({ ...channel, functions: reorder(channel.functions, index, index - 1) })}>▲</Button><Button iconOnly aria-label={`Move function ${fn.name} down`} disabled={index === channel.functions.length - 1} onClick={() => onChange({ ...channel, functions: reorder(channel.functions, index, index + 1) })}>▼</Button><Button onClick={() => onChange({ ...channel, functions: channel.functions.filter((candidate) => candidate.id !== fn.id) })}>Remove function</Button></div></article>)}</div></section></div>;
}
...
function GeometryEditor({ mode, onChange }: { mode: FixtureMode; onChange: (mode: FixtureMode) => void }) {
  ...
  const addEmitter = () => {
    ...
    if (!node || !head) return;
    const emitter: GeometryEmitter = { id: uuid(), name: `Emitter ${mode.geometry.emitters.length + 1}`, node_id: node.id, head_id: head.id, origin: { x: 0, y: 0, z: 0 }, orientation_degrees: { x: 0, y: 0, z: 0 }, beam_angle_degrees: 20, field_angle_degrees: 24, feather: 0, focus: 1, directional: true, layout: { type: "point" } };
...
function GeometryEmitterForm({ emitter, mode, onChange, onRemove }: { emitter: GeometryEmitter; mode: FixtureMode; onChange: (emitter: GeometryEmitter) => void; onRemove: () => void }) {
  ...
  return <div><h3>Emitter properties</h3>
    ...
    <SwitchField label="Projects a directional beam" checked={emitter.directional ?? true} onChange={(event) => onChange({ ...emitter, directional: event.target.checked })}/>
