    ...
    <section className="nested-modal fixture-profile-editor-modal" role="dialog" aria-modal="true" aria-label={initialProfile.revision ? "Edit fixture profile" : "Create fixture profile"}>
      ...
      <div className="fixture-profile-editor-body">
        ...
        {tab === "modes" && <div className="fixture-modes-tab">
          <section className="fixture-mode-list">{draft.modes.map((mode, index) => <article key={mode.id} data-mode-reorder-id={mode.id} draggable onDragStart={() => setDragMode(index)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (dragMode != null) moveMode(dragMode, index); setDragMode(null); }}>
            ...
            <TextField label="Mode notes" value={mode.notes} onChange={(event) => updateMode({ ...mode, notes: event.target.value })}/>
            <div className="fixture-mode-channel-summary"><small>{mode.heads.length} head{mode.heads.length === 1 ? "" : "s"} · {mode.channels.length} logical channel{mode.channels.length === 1 ? "" : "s"} · {mode.splits.length} split{mode.splits.length === 1 ? "" : "s"}</small><Button aria-label={`Edit channels for ${mode.name || "unnamed mode"}`} onClick={() => { setModeEditorId(mode.id); setOpenSplit(mode.splits[0]?.number ?? 1); setModeTab("channels"); }}>Edit channels</Button></div>
