...
export function WindowPicker() {
  ...
  return <div className="floating-dialog window-picker" role="dialog" aria-label="Open Window"><h2>Open Window</h2><div className="dialog-grid">{windowChoices.map(([kind, label]) => <Button key={kind} onClick={() => dispatch({ type: "ADD_WINDOW", kind })}>{label}</Button>)}</div><Button className="dialog-done" onClick={() => dispatch({ type: "OPEN_WINDOW_PICKER", rect: null })}>Cancel</Button></div>;
}
