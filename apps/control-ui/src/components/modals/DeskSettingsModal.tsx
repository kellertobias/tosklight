...
export function DeskSettingsModal() {
  ...
  return <div className="stacked-modal-layer" onPointerDown={(event) => event.target === event.currentTarget && close()}><section className="nested-modal desk-settings-modal" role="dialog" aria-modal="true" aria-label="Desk settings"><Button className="modal-close" onClick={close}>×</Button><h3>Desk View</h3><FormLayout labelPlacement="side"><TextField label="Name" clearable value={name} onChange={(event) => setName(event.target.value)} onBlur={() => name.trim() && dispatch({ type: "UPDATE_DESK", id: desk.id, name: name.trim() })}/><IconPickerField label="Icon" value={desk.icon ?? "⊞"} onChange={(icon) => dispatch({ type: "UPDATE_DESK", id: desk.id, icon })}/></FormLayout>{confirmDelete ? <div className="delete-confirm"><b>Delete “{desk.name}”?</b><Button onClick={() => setConfirmDelete(false)}>Cancel</Button><Button className="danger" onClick={() => dispatch({ type: "DELETE_DESK", id: desk.id })}>Confirm Delete</Button></div> : <Button className="danger large-danger" disabled={state.desks.length <= 1} onClick={() => setConfirmDelete(true)}>Delete Desk View</Button>}</section></div>;
}
