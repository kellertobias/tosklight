...
import { Button, Input, NumberField, SelectField, TextInput } from "../common";
...
export function QuickSetupModal() {
  ...
  return <div className="modal-backdrop" onPointerDown={(event) => { if (event.currentTarget === event.target) close(); }}><section className="modal-card show-modal">
    ...
    <div className="modal-actions show-navigation"><Button onClick={() => { dispatch({ type: "OPEN_BUILTIN", kind: "patch" }); close(); }}><span aria-hidden="true">▦</span> Show Patch</Button><Button onClick={() => { dispatch({ type: "OPEN_BUILTIN", kind: "setup" }); close(); }}><span aria-hidden="true">⚙</span> Enter Setup</Button><Button onClick={() => { dispatch({ type: "OPEN_BUILTIN", kind: "dmx" }); close(); }}><span aria-hidden="true">◉</span> DMX</Button><Button onClick={() => setChangeUserOpen(true)}><span aria-hidden="true">♙</span> Change User</Button><Button onClick={() => dispatch({ type: "SET_MODAL", modal: "debugOpen", value: true })}><span aria-hidden="true">⌁</span> Debug</Button><Button className="help-action" onClick={() => { dispatch({ type: "OPEN_BUILTIN", kind: "help" }); close(); }}><span aria-hidden="true">?</span> Help</Button><Button className="danger shutdown-action" onClick={() => setConfirmShutdown(true)}><span aria-hidden="true">⏻</span> Shut Down Desk</Button></div>
