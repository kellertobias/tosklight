...
export function QuickSetupModal() {
  ...
  const originalShow = revisionCopy ? (lifecycle?.shows ?? []).find((show) => show.id === revisionCopy.show_id) : undefined;
  ...
  const {
    ...
    enabled: state.setupOpen, activeShowId, revisionName, shows: (lifecycle?.shows ?? []),
    listShowRevisions:
			lifecycle?.listShowRevisions ?? (async () => []), saveShowRevision: lifecycle?.saveShowRevision ?? (async () => null),
  ...
  async function saveAs(value = showName) {
    ...
    if (destination === "flash" && bootstrap?.active_show) await lifecycle?.downloadShow({ ...bootstrap.active_show, name });
    setSaveAsOpen(false);
    setShowName("");
  }
  ...
  async function confirmOverwrite() {
    ...
    try {
      if (!await lifecycle?.overwriteShow(overwriteTarget.id)) return;
  ...
  return <div className="modal-backdrop" onPointerDown={(event) => { if (event.currentTarget === event.target) close(); }}><section className="modal-card show-modal" role="dialog" aria-modal="true" aria-label="Show">
    ...
    <div className="show-details"><b>{activeShow?.name ?? "No active show"}</b>{revisionCopy && <div className="revision-copy-notice" role="status"><strong>Separate revision copy</strong><span>Created from <b>{revisionCopy.show_name}</b>, Revision {revisionCopy.revision} · {revisionCopy.revision_name}</span><small>Created {new Date(revisionCopy.copied_at).toLocaleString()}. Current changes are autosaved to this copy, not to {revisionCopy.show_name}.</small></div>}<div className={`show-status-explanation ${showIndicator.className}`} role="status"><span className="show-status-dot" aria-hidden="true">●</span><span><strong>{showIndicator.label}</strong><small>{showIndicator.detail}</small></span></div><span>Server connected <strong>{showIndicator.connected ? "Yes" : "No"}</strong></span><span>Latest named revision <strong>{activeRevisions[0] ? `${activeRevisions[0].revision} · ${activeRevisions[0].name}` : "None"}</strong></span><span>Operator <strong>{session?.user.name ?? "—"}</strong></span><div className="show-primary-actions"><Button onClick={() => setRevisionOpen(true)}><span aria-hidden="true">💾</span> Save Named Revision</Button>{revisionCopy && <Button onClick={() => setCopySaveOpen(true)}><span aria-hidden="true">✓</span> Save</Button>}<Button onClick={() => setSaveAsOpen(true)}><span aria-hidden="true">✎</span> Save As</Button><Button onClick={() => void openLoadMenu()}><span aria-hidden="true">↥</span> Load</Button><Button onClick={() => setNewShowOpen(true)}><span aria-hidden="true">＋</span> New Show</Button></div></div>
    ...
    {mvrMode && stackedModal(<div className="nested-modal mvr-modal" role="dialog" aria-modal="true" aria-label="MVR import and export"><Button className="modal-close" onClick={() => setMvrMode(null)}>×</Button><h3>{mvrMode === "new" ? "New Show from MVR" : mvrMode === "merge" ? "Add MVR to Show" : "Export Show as MVR"}</h3>
      {mvrMode !== "new" && !mvrTarget && <><p>Select any show in the desk library.</p><div className="show-library">{(lifecycle?.shows ?? []).map((show) => <article key={show.id}><span><b>{show.name}</b><small>Autosaved show file</small></span><Button onClick={() => mvrMode === "export" ? void inspectExport(show) : setMvrTarget(show)}>Select</Button></article>)}</div></>}
      ...
      {mvrMode === "export" && mvrTarget && mvrExportPreview && <><div className="mvr-summary"><b>{mvrExportPreview.fixtures} fixtures · {mvrExportPreview.scenery} scenery objects</b><p>Not included: {mvrExportPreview.omitted.join(", ")}</p>{mvrExportPreview.warnings.map((warning) => <p className="modal-warning" key={warning}>{warning}</p>)}</div><Button className="primary" onClick={() => { void lifecycle?.downloadMvr(mvrTarget); setMvrMode(null); }}>Download {mvrTarget.name}.mvr</Button></>}
    </div>, () => setMvrMode(null))}
    {changeUserOpen && stackedModal(<div className="nested-modal" role="dialog" aria-modal="true" aria-label="Change user"><Button className="modal-close" onClick={() => setChangeUserOpen(false)}>×</Button><h3>Change User</h3><div className="show-library">{bootstrap?.users.filter((user) => user.enabled).map((user) => <article key={user.id}><span><b>{user.name}</b><small>{user.id === session?.user.id ? "Current user" : "Use this user's programmer"}</small></span><Button disabled={user.id === session?.user.id} onClick={() => void lifecycle?.changeUser(user)}>{user.id === session?.user.id ? "Logged in" : "Log in"}</Button></article>)}</div><div className="user-create-row"><TextInput clearable value={newUserName} onChange={(event) => setNewUserName(event.target.value)} onKeyboardCommit={(value) => { if (value.trim()) void lifecycle?.createUser(value.trim()); }} placeholder="New user name" aria-label="New user name"/><Button variant="primary" disabled={!newUserName.trim()} onClick={() => void lifecycle?.createUser(newUserName.trim())}>Add user</Button></div></div>, () => setChangeUserOpen(false))}
