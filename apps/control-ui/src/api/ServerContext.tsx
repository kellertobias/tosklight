...
import {
	...
	defaultCommandLine,
	type CommandTargetMode,
} from "../components/control/softwareKeypad";
...
interface ServerContextValue {
	...
	setGroupValue: (attribute: string, value: number | import("./types").AttributeValue) => Promise<void>;
	...
	readVirtualPlaybackExclusionZones: () => Promise<
		import("./types").VirtualPlaybackExclusionSnapshot
	>;
	...
	saveShowRevision: (
		name: string,
	) => Promise<import("./types").ShowRevision | null>;
	...
	previewMvrExport: (
		showId: string,
	) => Promise<import("./types").MvrExportPreview>;
	...
	patchFixture: (input: {
		...
		address: number | null;
		split_patches?: import("./types").SplitPatch[];
		layer_id?: string;
	}) => Promise<string | null>;
...
export function ServerProvider({ children }: PropsWithChildren) {
	...
	const [deskLock, setDeskLock] = useState<
		import("./types").DeskLockState | null
	>(null);
	...
	const [cueObjects, setCueObjects] = useState<
		VersionedObject<import("./types").CueList>[]
	>([]);
		...
		const start = async () => {
			try {
				...
				if (!initial.active_show && !nextDeskLock.locked) {
					...
					const defaultShow =
						library.find((show) => show.name === "Default Stage Show") ??
						(await client.createShow("Default Stage Show"));
				...
				await loadShowObjects(
					effectiveBootstrap.active_show_error
						? null
				...
				const restoredCommand =
					ownProgrammer?.command_line?.trim() || commandTargetModeRef.current;
				const restoredTarget =
					restoredCommand === "GROUP"
						? "GROUP"
						: restoredCommand === "FIXTURE"
							? "FIXTURE"
				...
				unsubscribe = client.onEvent((event) => {
					...
					if (
						...
						const action = event.payload as {
							action: string;
							session_id?: string;
							desk_id?: string;
						};
						if (
							(!action.session_id && !action.desk_id) ||
							action.session_id === nextSession.session_id ||
							action.desk_id === nextSession.desk.id
						)
							window.dispatchEvent(
								new CustomEvent("light:desk-action", { detail: action.action }),
							);
					}
					if (
						event.kind === "desk_action" &&
						...
						const encoder = event.payload as {
							control: string;
							value?: string;
							desk_alias?: string;
						};
						if (
							encoder.desk_alias === nextSession.desk.osc_alias &&
							encoder.control.startsWith("encode/")
						)
							window.dispatchEvent(
								new CustomEvent("light:encoder-action", { detail: encoder }),
							);
					}
					if (event.kind === "file_input_action") {
						...
						if (
							...
							window.dispatchEvent(
								new CustomEvent("light:file-manager-input", { detail: action }),
							);
					}
					...
					if (
						...
						(event.payload as { desk_id?: string }).desk_id ===
							nextSession.desk.id
					)
						void client
							.commandHistory()
							...
							if (event.kind === "update_armed")
								window.dispatchEvent(
									new CustomEvent("light:update-armed", {
										detail: request.armed ?? true,
									}),
								);
							if (event.kind === "update_target_requested" && request.target)
								window.dispatchEvent(
							...
							if (event.kind === "update_target_rejected")
								window.dispatchEvent(
									new CustomEvent("light:command-error", {
										detail:
											request.error ??
											"This playback is not a recordable Update target.",
									}),
								);
						...
						void commandLineWrite.current
							...
							.then((next) => {
								...
								const own = next.active_programmers.find(
									(programmer) =>
										programmer.session_id === nextSession.session_id,
								);
								if (own) {
									...
									if (requestedCommandLineEpoch === commandLineEpoch.current) {
										const restoredCommand =
											own.command_line?.trim() || commandTargetModeRef.current;
										setCommandLineState(restoredCommand);
										setCommandLinePristine(
											restoredCommand === commandTargetModeRef.current,
										);
									}
					...
					if (
						["fixture_library_changed", "fixture_profile_changed"].includes(
							event.kind,
						)
					) {
						void client
		...
		() => ({
			...
			highlightAction: async (action) => {
				...
				try {
					...
					if (
						/409|ownership|owned by|another (?:user|operator)/i.test(message)
					) {
			...
			savePlaybackPage: async (page) => {
				...
				try {
					const pages = await client.objects<import("./types").PlaybackPage>(
						bootstrap.active_show.id,
						"playback_page",
					);
					const existing = pages.find(
						(item) => item.body.number === page.number,
					);
					if (!existing) {
			...
			savePlaybackDefinition: async (playback) => {
				...
				try {
					const objects = await client.objects<
						import("./types").PlaybackDefinition
			...
			savePlaybackSlot: async (page, slot, playback) => {
				...
				try {
					const [pages, definitions] = await Promise.all([
						client.objects<import("./types").PlaybackPage>(
							bootstrap.active_show.id,
							"playback_page",
						),
						client.objects<import("./types").PlaybackDefinition>(
							bootstrap.active_show.id,
							"playback",
						),
					]);
			...
			saveOutputRoute: async (id, route, revision) => {
				...
				try {
					...
					setPatch(await client.patch());
					setOutputRoutes(
						await client.objects<OutputRoute>(
							bootstrap.active_show.id,
							"route",
						),
					);
			...
			storePlayback: async (slot, cueListId, explicitPageNumber) => {
				try {
					...
					const recordValues = recordingBlind
						? programmer.preload_pending
						: programmer.values;
					const recordGroupValues = recordingBlind
						? programmer.preload_group_pending
						: programmer.group_values;
					const objects = await client.objects<import("./types").CueList>(
						bootstrap.active_show.id,
						"cue_list",
					);
					...
					const active = playbacks?.active.find(
						(item) => item.cue_list_id === id,
					);
					const mergeActive =
						localStorage.getItem("light.store-merge-active-cue") === "true" &&
						active &&
						current.cues[active.cue_index];
					const cueNumber = mergeActive
						? current.cues[active.cue_index].number
						: current.cues.length
					...
					const group_changes = Object.entries(recordGroupValues ?? {}).flatMap(
						([group_id, attributes]) =>
					...
					const previousCue = mergeActive
						? current.cues[active.cue_index]
						: null;
					const cueOnly =
						localStorage.getItem("light.store-cue-only") === "true";
					const mergeChanges = <
						T extends {
							fixture_id?: string;
							group_id?: string;
					...
					const cue = {
						...
						cue_only: mergeActive
							? (previousCue?.cue_only ?? cueOnly)
							: cueOnly,
						fade_millis: previousCue?.fade_millis ?? 0,
						delay_millis: previousCue?.delay_millis ?? 0,
						trigger: previousCue?.trigger ?? { type: "manual" },
						changes: mergeActive
							? mergeChanges(previousCue?.changes ?? [], changes)
							: mergeChanges(restoration.changes, changes),
						group_changes: mergeActive
							? mergeChanges(previousCue?.group_changes ?? [], group_changes)
							: mergeChanges(restoration.group_changes, group_changes),
						phasers: previousCue?.phasers ?? [],
					};
					const cues = mergeActive
						? current.cues.map((existingCue, index) =>
								index === active.cue_index ? cue : existingCue,
							)
						: [...current.cues, cue];
					await client.putObject(
						bootstrap.active_show.id,
					...
					const playbackObjects = await client.objects<
						import("./types").PlaybackDefinition
					>(bootstrap.active_show.id, "playback");
					let playbackObject = playbackObjects.find(
						(item) =>
							item.body.target.type === "cue_list" &&
							item.body.target.cue_list_id === id,
					);
					if (!playbackObject) {
						const used = new Set(
							playbackObjects.map((item) => item.body.number),
						);
					...
					await refresh();
					if (!recordingBlind) {
						await client.poolPlaybackAction(
							playbackObject.body.number,
							"go-to",
				...
				try {
					...
					for (const raw of programmer.values) {
						...
						if (includesAttribute(value.attribute))
							(values[value.fixture_id] ??= {})[value.attribute] = value.value;
					}
					for (const [group, attributes] of Object.entries(group_values))
						for (const attribute of Object.keys(attributes))
							if (!includesAttribute(attribute)) delete attributes[attribute];
					const existing = presets.find((item) =>
			...
			saveFixtureProfile: async (profile, expectedRevision) => {
				try {
					const saved = await client.putFixtureProfile(
						profile,
						expectedRevision,
					);
