...
describe("LightApiClient", () => {
  ...
  it("uses typed programmer, fixture-action, and opt-in preset-generation commands", async () => {
    const client = new LightApiClient("http://desk.local");
    const command = vi.spyOn(client, "command").mockResolvedValue({ created: [] });
  ...
  it("creates named revisions, loads them as copies, and overwrites by stable IDs", async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: "copy-a", name: "Tour-rev-1-2026-07-17", revision_copy: { show_id: "show-a", revision: 1 } }), { status: 200, headers: { "content-type": "application/json" } }))
  ...
  it("uses the desk-scoped Update settings, preview, apply, and eligible-target contracts", async () => {
    ...
    const mode = { target_type: "cue" as const, mode: "existing_only" as const };
    const response = (body: unknown) => new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } });
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(response({ session_id: "session-a", token: "token-a", user: { id: "user-a", name: "Operator", enabled: true }, desk: { id: "desk-a" } }))
      .mockResolvedValueOnce(response(settings))
      .mockResolvedValueOnce(response(settings))
      .mockResolvedValueOnce(response({ revision: 4, programmer_revision: "programmer-a", target: { ...target, name: "Main", family: target.family, cue: { id: "cue-a", number: 2 } }, mode, items: [] }))
      .mockResolvedValueOnce(response({ target: { ...target, name: "Main", family: target.family }, revision_before: 4, revision_after: 5, eligible_count: 1, changed_count: 1, added_count: 0, ignored_count: 0, changed_cues: [], programmer_values_retained: true }))
    ...
    expect(JSON.parse(fetchMock.mock.calls[3][1].body)).toEqual({ target, mode });
    expect(JSON.parse(fetchMock.mock.calls[4][1].body)).toEqual({ target, mode, expected_revision: 4, expected_programmer_revision: "programmer-a" });
  });
  ...
  it("reads and changes server-authoritative Highlight state", async () => {
    ...
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ session_id: "session-a", token: "token-a", user: { id: "user-a", name: "Operator", enabled: true }, desk: { id: "desk-a" } }), { status: 200, headers: { "content-type": "application/json" } }))
      .mockImplementation(() => Promise.resolve(new Response(JSON.stringify(state), { status: 200, headers: { "content-type": "application/json" } })));
    vi.stubGlobal("fetch", fetchMock);
    const client = new LightApiClient("http://desk.local");
    await client.login("Operator");
  ...
  it("uses the desk-wide fixture-profile revision contract", async () => {
    ...
    const response = (body: unknown) => new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } });
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(response({ session_id: "session-a", token: "token-a", user: { id: "user-a", name: "Operator", enabled: true } }))
