...
import {
	type ProgrammerPreloadValuesEventStream,
	type ProgrammerPreloadValuesEventTransport,
	ProgrammerPreloadValuesProtocolError,
} from "./transport";
...
export interface ProgrammerPreloadValuesSessionOptions {
	...
	loadSnapshot(): Promise<ProgrammerPreloadValuesSnapshot>;
...
export class ProgrammerPreloadValuesSession {
	...
	private readonly loadSnapshot: ProgrammerPreloadValuesSessionOptions["loadSnapshot"];
	private readonly onError?: (error: Error | null) => void;
	private references = 0;
	private lifecycle = 0;
	private hydrated = false;
	private stopped = false;
	private stream: ProgrammerPreloadValuesEventStream | null = null;
	...
	async repairAuthority(error: Error) {
		if (this.stopped || this.storeScope === null)
			throw new Error("The Preload Programmer values session is unavailable");
		...
		if (state.repairRequired)
			throw state.error ?? new Error("Preload Programmer values repair failed");
	}
	...
	private openStream(generation: number, cursor: number | null) {
		...
		let stream: ProgrammerPreloadValuesEventStream;
		try {
			stream = this.transport.subscribe(this.eventScope, cursor, {
				...
				error: (error) => {
					if (!this.isCurrent(generation)) return;
					if (error instanceof ProgrammerPreloadValuesProtocolError)
	...
	private handleMessage(
		message: ProgrammerPreloadValuesEventMessage,
		generation: number,
	) {
		...
		if (message.type === "gap") {
			void this.repair(
				...
				new ProgrammerPreloadValuesProtocolError(
					"Preload Programmer values event history has a gap",
		...
		if (message.type === "error") {
			void this.repair(
				generation,
				new ProgrammerPreloadValuesProtocolError(message.error),
			);
			return;
		}
	...
	private assertSnapshotUser(snapshot: ProgrammerPreloadValuesSnapshot) {
	...
	private scopeError(subject: string) {
		return new ProgrammerPreloadValuesProtocolError(
			`Preload Programmer values ${subject} does not match the active user view`,
		);
	}
