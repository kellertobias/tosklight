...
import type { ProgrammerPreloadValuesOptimisticReducer } from "./store";
...
function applyMutation(
	...
	fixtures: Map<string, ProgrammerPreloadFixtureValue>,
	groups: Map<string, ProgrammerPreloadGroupValue>,
...
function fixtureWrite(
	mutation: Extract<ProgrammerPreloadValuesMutation, { action: "set_fixture" }>,
	programmerOrder: number,
): ProgrammerPreloadFixtureValue {
	return {
...
function groupWrite(
	mutation: Extract<ProgrammerPreloadValuesMutation, { action: "set_group" }>,
	programmerOrder: number,
): ProgrammerPreloadGroupValue {
	return {
...
function sameFixtureWrite(
	current: ProgrammerPreloadFixtureValue | undefined,
	mutation: Extract<ProgrammerPreloadValuesMutation, { action: "set_fixture" }>,
) {
	return Boolean(
...
function sameGroupWrite(
	current: ProgrammerPreloadGroupValue | undefined,
	mutation: Extract<ProgrammerPreloadValuesMutation, { action: "set_group" }>,
) {
	return Boolean(
		current && sameWrite(current, mutation.value, mutation.timing),
...
function sameWrite(
	current: ProgrammerPreloadFixtureValue | ProgrammerPreloadGroupValue,
...
function greatestOrder(projection: ProgrammerPreloadValuesProjection) {
	let greatest = 0;
