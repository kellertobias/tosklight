...
import type { ProgrammerPriorityStore } from "./store";
import type {
...
export interface ProgrammerPrioritySessionOptions {
...
/** Reference-counted exact-user priority snapshot and event owner. */
export class ProgrammerPrioritySession {
	private references = 0;
	private lifecycle = 0;
	private hydrated = false;
	private stopped = false;
	private stream: ProgrammerPriorityEventStream | null = null;
	...
	constructor(private readonly options: ProgrammerPrioritySessionOptions) {}
	...
	async repairAuthority(error: Error) {
		if (this.stopped || this.storeScope === null || this.references === 0)
			throw new Error("The Programmer priority view is unavailable");
		await this.repair(this.lifecycle, error);
		const state = this.options.store.getSnapshot();
		if (state.repairRequired)
			throw state.error ?? new Error("Programmer priority repair failed");
	}
	...
	private openStream(generation: number, cursor: number | null) {
		if (!this.isCurrent(generation)) return;
		let stream: ProgrammerPriorityEventStream;
		try {
			stream = this.options.transport.subscribe(this.options.scope, cursor, {
				...
				error: (error) => {
					if (!this.isCurrent(generation)) return;
					if (error instanceof ProgrammerPriorityProtocolError)
	...
	private handleMessage(
		message: ProgrammerPriorityEventMessage,
		generation: number,
	) {
		...
		if (message.type === "gap" || message.type === "error") {
			void this.repair(
				...
				new ProgrammerPriorityProtocolError(
					message.type === "gap"
						? "Programmer priority event history has a gap"
						: message.error,
	...
	private loadSnapshot(): Promise<ProgrammerPrioritySnapshot> {
	...
	private scopeError(subject: string) {
		return new ProgrammerPriorityProtocolError(
			`Programmer priority ${subject} does not match the active user authority`,
		);
	}
