...
vi.mock("./FileManagerPickerHost", () => ({
	openFileManagerPicker: mocks.openPicker,
}));
...
beforeEach(() => {
	...
	mocks.server.fileEntries.mockReset().mockResolvedValue({
		root_id: "shows",
		path: "",
		entries: [entry("notes.txt")],
	});
	mocks.server.readTextFile.mockReset().mockResolvedValue(document());
	mocks.server.saveTextFile
		.mockReset()
		.mockImplementation(async (_root, path, text) =>
			document(text, "rev-2", false, path),
		);
...
describe("text file discovery and location changes", () => {
	it("discovers supported text files in nested folders without offering arbitrary binary files", async () => {
		...
		list.mockImplementationOnce(async () => ({
			entries: [entry("run/cues.txt"), entry("run/audio.wav")],
		}));
	...
	it("maps a parent-folder move to the open document's descendant path", () => {
		expect(
			textFileLocationChange("shows", "run/act-1/cues.md", {
				operation: "move",
				items: [
					{
			...
			kind: "moved",
			rootId: "archive",
			path: "past-shows/run/act-1/cues.md",
		});
	});
});

describe("TextEditorWindow file loading and presentation", () => {
	it("loads an associated read-only file and makes the state explicit", async () => {
		mocks.server.readTextFile.mockResolvedValue(
			document("Do not alter", "read-only", true),
		);
		...
		expect(
			screen.getByText("Read-only", { selector: ".text-save-state" }),
		).toBeInTheDocument();
		expect(screen.getByRole("button", { name: /^Save$/ })).toBeDisabled();
		expect(
			screen.getByText(/contents can be copied with Save As/i),
		).toBeInTheDocument();
	});

	it("opens files through the root-confined ToskLight picker", async () => {
		mocks.openPicker.mockResolvedValue([
			{ rootId: "shows", entry: entry("run/cues.md") },
		]);
		render(<TextEditorWindow paneId="editor" />);
		await waitFor(() =>
			expect(screen.getByLabelText("File text")).toHaveValue("House open"),
		);
...
describe("TextEditorWindow concurrent saves", () => {
	it("reflects a clean save from another Text Editor window", async () => {
		...
		act(() => {
			window.dispatchEvent(
				new CustomEvent("light:text-file-saved", {
					detail: {
						document: document("Beginners", "rev-2"),
						sourcePaneId: "other-editor",
					},
				}),
			);
		});
		...
		expect(
			screen.getByText("Saved", { selector: ".text-save-state" }),
		).toBeInTheDocument();
		expect(
			screen.getByText(/Another Text Editor window saved a newer version/i),
		).toBeInTheDocument();
	});
	...
	it("preserves dirty text and provides compare/reload actions for a concurrent save", async () => {
		...
		act(() => {
			window.dispatchEvent(
				new CustomEvent("light:text-file-saved", {
					detail: {
						document: document("External edit", "rev-2"),
						sourcePaneId: "other-editor",
					},
				}),
			);
		...
		expect(
			screen.getByText("Conflict", { selector: ".text-save-state" }),
		).toBeInTheDocument();
		expect(editor).toHaveValue("My local edit");
		expect(screen.getByLabelText("Your unsaved version")).toHaveValue(
			"My local edit",
		);
		expect(screen.getByLabelText("Newer file version")).toHaveValue(
			"External edit",
		);
		expect(screen.getByRole("button", { name: /^Save$/ })).toBeDisabled();

		fireEvent.click(
			screen.getByRole("button", { name: "Reload Newer Version" }),
		);
		expect(window.confirm).toHaveBeenCalledWith(
			"Discard your unsaved version and load the newer file?",
		);
...
describe("TextEditorWindow revision conflicts and Save As", () => {
	it("turns a stale write into a visible revision conflict instead of overwriting", async () => {
		...
		mocks.server.saveTextFile.mockRejectedValue(
			new Error('{"error":"file changed since it was opened"}'),
		);
		render(<TextEditorWindow paneId="editor" />);
		const editor = screen.getByLabelText("File text");
...
describe("TextEditorWindow missing-file recreation", () => {
	it("detects deletion, retains the last loaded text, and can recreate the file", async () => {
		...
		expect(
			screen.getByText("Missing", { selector: ".text-save-state" }),
		).toBeInTheDocument();
		expect(screen.getByLabelText("File text")).toHaveValue("Retain me");
...
describe("TextEditorWindow moved-file handling", () => {
	it("follows an externally moved open file without losing an unsaved draft", async () => {
		render(<TextEditorWindow paneId="editor" />);
		const editor = screen.getByLabelText("File text");
		await waitFor(() => expect(editor).toHaveValue("House open"));
		fireEvent.change(editor, {
			target: { value: "Draft retained across move" },
		});
		...
		expect(
			screen.getByText(/moved from notes.txt to run\/renamed-notes.txt/i),
		).toBeInTheDocument();
		expect(mocks.dispatch).toHaveBeenCalledWith({
			type: "SET_TEXT_EDITOR_FILE",
			id: "editor",
...
describe("TextEditorWindow external deletion and recovery", () => {
	...
	it("recovers from missing when another editor recreates identical content", async () => {
		...
		act(() => {
			window.dispatchEvent(
				new CustomEvent("light:text-file-saved", {
					detail: {
						document: document("Same content", "same-content-revision"),
						sourcePaneId: "other-editor",
					},
				}),
			);
...
describe("TextEditorWindow dirty lifecycle and view persistence", () => {
	...
	it("persists cursor and scroll state in the pane layout without persisting the draft", async () => {
		...
		expect(mocks.dispatch.mock.calls.flat()).not.toContainEqual(
