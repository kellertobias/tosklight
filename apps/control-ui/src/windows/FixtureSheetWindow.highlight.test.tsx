...
const state = {
	...
	fixtureSheetIncludedHeads: "all" as
		| "all"
		| "no-sub-heads"
		| "no-master-heads",
};
...
describe("Fixture Sheet Highlight stepping visualization", () => {
	...
	it("keeps every fixture row available in a compact scrollable pane", async () => {
		server.patch.fixtures = Array.from({ length: 13 }, (_, index) => {
			...
			return {
				...
				definition: {
					...fixture.definition,
					heads: [fixture.definition.heads[0]],
				},
			};
		});
		...
		await waitFor(() =>
			expect(
				container.querySelector('[data-fixture-id="fixture-13"]'),
			).toBeInTheDocument(),
		);
		expect(
			container.querySelectorAll(".ui-data-table-row:not(.header)"),
		).toHaveLength(13);
	});
	...
	it("routes logical-head rows through the scoped fixture gesture", async () => {
		...
		const left = await waitFor(() => {
			const row = container.querySelector<HTMLElement>(
				'[data-fixture-id="left"]',
			);
			expect(row).toBeInTheDocument();
			if (!row) throw new Error("Missing left fixture row");
			return row;
		});
	...
	it("keeps the remembered heads subdued, marks the actual step prominently, and survives HIGH toggles", async () => {
		...
		await waitFor(() =>
			expect(
				container.querySelector('[data-fixture-id="left"]'),
			).toBeInTheDocument(),
		);

		const row = (fixtureId: string) =>
			container.querySelector<HTMLElement>(`[data-fixture-id="${fixtureId}"]`);
		expect(row("left")).toHaveAttribute("data-step-selection", "active");
		expect(row("left")).toHaveClass(
			"fixture-step-current",
			"fixture-step-base",
			"selected",
		);
	...
	it("uses settings instead of row buttons to show masters, subheads, or both", async () => {
		...
		await waitFor(() =>
			expect(
				container.querySelector('[data-fixture-id="master"]'),
			).toBeInTheDocument(),
		);

		expect(
			screen.queryByRole("button", { name: /fixture 100\.0 heads/i }),
		).not.toBeInTheDocument();
		expect(container.querySelector('[data-fixture-id="left"]')).toHaveClass(
			"fixture-head-row",
		);
		expect(
			container.querySelector('[data-fixture-id="left"] .fixture-sheet-id'),
		).toBeInTheDocument();
		expect(
			container.querySelector('[data-fixture-id="left"] .fixture-name'),
		).toBeInTheDocument();
		...
		expect(
			container.querySelector('[data-fixture-id="master"]'),
		).not.toHaveTextContent("100.0");
		expect(
			container.querySelector('[data-fixture-id="left"]'),
		).not.toBeInTheDocument();
		expect(
			container.querySelector('[data-fixture-id="right"]'),
		).not.toBeInTheDocument();
		...
		expect(container.querySelector('[data-fixture-id="left"]')).not.toHaveClass(
			"fixture-head-indented-row",
		);
		expect(
			container.querySelector('[data-fixture-id="right"]'),
		).not.toHaveClass("fixture-head-indented-row");
		...
		expect(
			container.querySelector('[data-fixture-id="master"]'),
		).not.toHaveAttribute("data-step-contained");
	});
	...
	it("uses a compact View tab, exposes column controls, and hides optional name details", async () => {
		...
		expect(
			screen.getByText("Name", { selector: ".ui-data-table-row.header span" }),
		).toBeInTheDocument();
		expect(
			screen.queryByText("Beam", {
				selector: ".ui-data-table-row.header span",
			}),
		).not.toBeInTheDocument();
		...
		expect(
			screen.queryByRole("tab", { name: "Ordering" }),
		).not.toBeInTheDocument();
		expect(
			screen.queryByRole("tab", { name: "Filters" }),
		).not.toBeInTheDocument();
	...
	it("keeps fixture type aligned with the name and renders icon and patch as dedicated columns", async () => {
		...
		const master = await screen.findByRole("row", {
			name: /100\.0.*Pixel Bar · Master.*Test · 2 cells.*U1\.1/,
		});
		...
		expect(
			master.querySelector(
				'.fixture-sheet-icon img[src="data:image/png;base64,fixture-icon"]',
			),
		).toBeInTheDocument();
		expect(master.querySelector(".fixture-sheet-patch")).toHaveTextContent(
			"U1.1",
		);
