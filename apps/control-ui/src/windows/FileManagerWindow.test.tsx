...
const entries: FileEntry[] = [
  { name: "image.png", path: "image.png", kind: "file", size: 2048, modified_millis: 2_000, created_millis: null, hidden: false, writable: true },
  { name: "Folder", path: "Folder", kind: "folder", size: 0, modified_millis: 1_000, created_millis: 500, hidden: false, writable: true },
  { name: "alpha.txt", path: "alpha.txt", kind: "file", size: 12, modified_millis: null, created_millis: null, hidden: false, writable: false },
];
...
describe("FileManager helpers", () => {
  it("sorts folders first and names naturally", () => {
    expect(sortFileEntries(entries).map((entry) => entry.name)).toEqual(["Folder", "alpha.txt", "image.png"]);
  });
  ...
  it("validates picker targets, extensions, and cardinality", () => {
    ...
    const picker = { target: "files" as const, allowedExtensions: [".PNG"], onSelect: vi.fn(), onCancel: vi.fn() };
    ...
    expect(pickerSelectionIsValid([file, file], { ...picker, multiple: true })).toBe(true);
    expect(pickerSelectionIsValid([folder], { ...picker, target: "either" })).toBe(true);
  });

  it("generates safe Keep Both names", () => {
    expect(nextKeepBothName("plot.png", ["plot.png", "plot copy.png"])).toBe("plot copy 2.png");
    expect(nextKeepBothName("Folder", ["folder", "Folder copy"])).toBe("Folder copy 2");
...
describe("FileManager", () => {
  beforeEach(() => {
    ...
    mocks.server.fileRoots.mockReset().mockResolvedValue([
      ...
      { id: "usb", label: "Tour USB", icon: "drive", removable: true, writable: true },
    ]);
    mocks.server.fileEntries.mockReset().mockImplementation(async (root: string, path: string, hidden: boolean) => ({
      root_id: root,
      path,
      entries: path === "Folder"
        ? [{ name: "nested.txt", path: "Folder/nested.txt", kind: "file", size: 4, modified_millis: null, created_millis: null, hidden: false, writable: true }]
        : hidden
          ? [...entries, { name: ".secret", path: ".secret", kind: "file", size: 1, modified_millis: null, created_millis: null, hidden: true, writable: true }]
          : entries,
    }));
    mocks.server.fileContent.mockReset().mockResolvedValue(new Blob(["preview"]));
    mocks.server.fileStreamUrl.mockReset().mockResolvedValue("http://light.test/api/v1/files/shows/content?path=audio.wav&ticket=capability");
    mocks.server.fileThumbnail.mockReset().mockResolvedValue(new Blob(["thumbnail"]));
    mocks.server.fileOperation.mockReset().mockResolvedValue({ paths: [], complete: true, items: [] });
    ...
    mocks.server.readFileNote.mockReset().mockResolvedValue({ supported: false, note: null });
    mocks.server.saveFileNote.mockReset().mockImplementation(async (root_id: string, path: string, note: string) => ({ root_id, path, supported: true, note }));
    mocks.server.readTextFile.mockReset().mockResolvedValue({ root_id: "shows", path: "alpha.txt", text: "Alpha", revision: "1", read_only: false });
    mocks.server.saveTextFile.mockReset().mockImplementation(async (root: string, path: string, text: string) => ({ root_id: root, path, text, revision: "2", read_only: false }));
    mocks.server.resetCommandLine.mockReset().mockImplementation(() => { mocks.server.commandLine = "FIXTURE"; });
  });
  ...
  it("renders the normal three-column layout, folders-first list, properties, and toggles", async () => {
    ...
    const rows = await screen.findAllByRole("button", { name: /, (?:folder|file)$/ });
  ...
  it("loads tree folders lazily and preserves breadcrumb/history navigation", async () => {
    ...
    expect(await screen.findByRole("button", { name: "nested.txt, file" })).toBeVisible();
    expect(screen.getByRole("navigation", { name: "Breadcrumb" })).toHaveTextContent("/ Folder");
  ...
  it("gives audio players a range-capable stream URL instead of downloading a whole Blob", async () => {
    mocks.server.fileEntries.mockResolvedValue({
      root_id: "shows",
      path: "",
      entries: [{ name: "walk-in.wav", path: "walk-in.wav", kind: "file", size: 4096, modified_millis: null, created_millis: null, hidden: false, writable: true }],
    });
    render(<FileManager instanceId="audio-preview" />);
    fireEvent.click(await screen.findByRole("button", { name: "walk-in.wav, file" }));

    const player = await screen.findByLabelText("Audio preview of walk-in.wav");
    expect(player).toHaveAttribute("src", expect.stringContaining("ticket=capability"));
    expect(mocks.server.fileStreamUrl).toHaveBeenCalledWith("shows", "walk-in.wav");
  ...
  it("supports picker target, extension, callback, ENTER, and ESC contracts", async () => {
    ...
    render(<FileManager instanceId="picker" picker={{ target: "files", allowedExtensions: ["png"], onSelect, onCancel }} />);
    ...
    expect(onSelect).toHaveBeenCalledWith([expect.objectContaining({ rootId: "shows", entry: expect.objectContaining({ path: "image.png" }) })]);
  ...
  it("supports Ctrl toggles and Shift range selection", async () => {
    ...
    const folder = await screen.findByRole("button", { name: "Folder, folder" });
  ...
  it("honors an initial picker location and allows folder targets", async () => {
    const onSelect = vi.fn();
    render(<FileManager instanceId="folder-picker" picker={{ target: "folders", multiple: true, initialRootId: "usb", initialDirectory: "", onSelect, onCancel: vi.fn() }} />);
    ...
    expect(onSelect).toHaveBeenCalledWith([expect.objectContaining({ rootId: "usb", entry: expect.objectContaining({ kind: "folder" }) })]);
  });

  it("allows an either-target picker to return a deliberate multi-selection", async () => {
    const onSelect = vi.fn();
    render(<FileManager instanceId="either-picker" picker={{ target: "either", multiple: true, onSelect, onCancel: vi.fn() }} />);
    const folder = await screen.findByRole("button", { name: "Folder, folder" });
    fireEvent.click(folder);
    fireEvent.click(screen.getByRole("button", { name: "alpha.txt, file" }), { ctrlKey: true });
    expect(onSelect).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: "Select" }));
    expect(onSelect).toHaveBeenCalledWith([
      expect.objectContaining({ entry: expect.objectContaining({ kind: "folder" }) }),
      expect.objectContaining({ entry: expect.objectContaining({ path: "alpha.txt" }) }),
    ]);
  });

  it("opens a picker at its requested non-root directory", async () => {
    const onSelect = vi.fn();
    render(<FileManager instanceId="directory-picker" picker={{ target: "files", initialRootId: "usb", initialDirectory: "Folder", allowedExtensions: ["txt"], onSelect, onCancel: vi.fn() }} />);
    expect(await screen.findByRole("button", { name: "nested.txt, file" })).toBeVisible();
    expect(screen.getByRole("navigation", { name: "Breadcrumb" })).toHaveTextContent(/Tour USB.*Folder/);
    fireEvent.click(screen.getByRole("button", { name: "nested.txt, file" }));
    fireEvent.click(screen.getByRole("button", { name: "Select" }));
    expect(onSelect).toHaveBeenCalledWith([expect.objectContaining({ rootId: "usb", entry: expect.objectContaining({ path: "Folder/nested.txt" }) })]);
  });
  ...
  it("replaces toolbar mutations with operation controls and confirms permanent delete", async () => {
    ...
    await waitFor(() => expect(mocks.server.fileOperation).toHaveBeenCalledWith("shows", { operation: "delete", sources: ["alpha.txt"] }));
  });
  ...
  it("offers Replace, Keep Both, Skip, and applies one decision to all conflicts", async () => {
    ...
    expect(within(dialog).getByRole("button", { name: "Keep Both" })).toBeVisible();
    ...
    await waitFor(() => expect(mocks.server.fileOperation).toHaveBeenLastCalledWith("shows", expect.objectContaining({ operation: "copy", conflict: "keep_both", apply_to_all: true })));
  });
  ...
  it("copies across configured roots through the server cross-root contract", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Copy Here" }));
    await waitFor(() => expect(mocks.server.fileOperation).toHaveBeenCalledWith("shows", expect.objectContaining({
      operation: "copy",
      sources: ["alpha.txt"],
  ...
  it("edits native notes and uses Trash only when the filesystem advertises it", async () => {
    mocks.server.fileEntries.mockImplementation(async (root: string, path: string) => ({
      root_id: root,
      path,
      entries: [{ ...entries[2], note_supported: true, trash_supported: true }],
    }));
    mocks.server.readFileNote.mockResolvedValue({ root_id: "shows", path: "alpha.txt", supported: true, note: "Old note" });
    ...
    await waitFor(() => expect(mocks.server.fileOperation).toHaveBeenCalledWith("shows", { operation: "trash", sources: ["alpha.txt"] }));
  });
  ...
  it("reports a failed Trash operation and never retries it as permanent deletion", async () => {
    ...
    expect(mocks.server.fileOperation).toHaveBeenCalledWith("shows", { operation: "trash", sources: ["alpha.txt"] });
  });
  ...
  it("falls back visibly when a removable root disconnects", async () => {
    ...
    mocks.server.fileRoots
      ...
      .mockResolvedValue([{ id: "shows", label: "Shows", icon: "shows", removable: false, writable: true }]);
    render(<FileManager instanceId="disconnect" picker={{ initialRootId: "usb", onSelect: vi.fn(), onCancel: vi.fn() }} />);
    await act(async () => Promise.resolve());
    expect(screen.getByRole("navigation", { name: "Breadcrumb" })).toHaveTextContent("Tour USB");

    await act(async () => vi.advanceTimersByTimeAsync(5_100));
    expect(screen.getByText(/location “usb” was disconnected/i)).toBeVisible();
    expect(screen.getByRole("navigation", { name: "Breadcrumb" })).toHaveTextContent("Shows");
  });
  ...
  it("releases a claimed operation with a visible reason when the connection is lost", async () => {
    ...
    expect(screen.getByText(/cancelled because the desk connection was lost/i)).toBeVisible();
    expect(screen.queryByRole("button", { name: "Copy Here" })).not.toBeInTheDocument();
    expect(mocks.server.releaseFileInput).toHaveBeenCalledWith("connection-loss");
  });
  ...
  it("claims an unowned desk action only after a pointer interaction inside this instance", async () => {
    ...
    fireEvent.pointerDown(within(manager).getByRole("heading", { name: "Locations" }));
    await waitFor(() => expect(mocks.server.claimFileInput).toHaveBeenCalledWith("claim", "copy", "pending"));
    await waitFor(() => expect(mocks.server.resetCommandLine).toHaveBeenCalledOnce());
  ...
  it("reflects a clean save from a dedicated Text Editor", async () => {
    ...
    act(() => {
      window.dispatchEvent(new CustomEvent("light:text-file-saved", { detail: {
        document: { root_id: "shows", path: "alpha.txt", text: "Updated elsewhere", revision: "2", read_only: false },
        sourcePaneId: "text-editor",
      } }));
    });

    expect(editor).toHaveValue("Updated elsewhere");
    expect(screen.getByText(/File Manager editor has been updated/)).toBeVisible();
  });
  ...
  it("preserves a dirty File Manager draft when another editor saves", async () => {
    ...
    expect(screen.getByText("Conflict", { selector: "span[role=status]" })).toBeVisible();
  ...
  it("follows a moved file in the embedded editor and keeps its dirty text", async () => {
    ...
    act(() => {
      window.dispatchEvent(new CustomEvent("light:file-operation", { detail: {
        operation: "move",
        items: [{
    ...
    expect(screen.getByText("notes/renamed.txt", { selector: ".file-editor b" })).toBeVisible();
    expect(screen.getByText("Unsaved", { selector: ".file-editor span[role=status]" })).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: "Save" }));
    await waitFor(() => expect(mocks.server.saveTextFile).toHaveBeenCalledWith("usb", "notes/renamed.txt", "Draft after move", "1"));
  });
  ...
  it("retains and can recreate an embedded editor file after deletion", async () => {
    ...
    expect(screen.getByText("Missing", { selector: ".file-editor span[role=status]" })).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: "Recreate File" }));
    await waitFor(() => expect(mocks.server.saveTextFile).toHaveBeenCalledWith("shows", "alpha.txt", "Alpha", null));
    expect(screen.getByText("Saved", { selector: ".file-editor span[role=status]" })).toBeVisible();
  });
  ...
  it("protects a dirty embedded editor when its File Manager pane is removed", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: "alpha.txt, file" }));
    fireEvent.click(screen.getByRole("button", { name: "Edit Text" }));
    fireEvent.change(await screen.findByLabelText("File text"), { target: { value: "Unsaved embedded draft" } });
