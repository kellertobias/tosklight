...
function ScreenSurface({ id }: { id: string }) {
  ...
  useEffect(() => { const currentScreen = screenRef.current; if (!currentScreen || !hydrated.current || closing.current) return; const timer = window.setTimeout(() => { const latest = screenRef.current; if (latest && !closing.current) void server.saveScreen({ ...latest, layout: { desks: state.desks, activeDeskId: state.activeDeskId } }); }, 600); return () => window.clearTimeout(timer); }, [state.desks, state.activeDeskId]);
  useEffect(()=>{if(!screen||!("__TAURI_INTERNALS__" in window))return;let cleanups:Array<()=>void>=[];let timer:number|undefined;let shuttingDown=false;void Promise.all([import("@tauri-apps/api/window"),import("@tauri-apps/api/event")]).then(async([{getCurrentWindow,currentMonitor},{listen}])=>{const current=getCurrentWindow();cleanups.push(await listen("app-shutting-down",()=>{shuttingDown=true;}));const persist=()=>{window.clearTimeout(timer);if(closing.current)return;timer=window.setTimeout(async()=>{if(closing.current)return;const position=await current.outerPosition();const size=await current.outerSize();const scale=await current.scaleFactor();const fullscreen=await current.isFullscreen();const monitor=await currentMonitor();const latest=screenRef.current;if(!latest||closing.current)return;const display_id=monitor?`${monitor.name??"Display"}|${monitor.position.x},${monitor.position.y}|${monitor.size.width}x${monitor.size.height}`:latest.display_id;void server.saveScreen({...latest,display_id,bounds:{x:position.x/scale,y:position.y/scale,width:size.width/scale,height:size.height/scale},fullscreen});},300);};cleanups.push(await current.onMoved(persist),await current.onResized(persist),await current.onCloseRequested(async(event)=>{event.preventDefault();closing.current=true;window.clearTimeout(timer);const latest=screenRef.current;if(!shuttingDown&&latest)await server.saveScreen({...latest,desired_open:false});await current.destroy();}));persist();});return()=>{window.clearTimeout(timer);cleanups.forEach((cleanup)=>cleanup());};},[screen?.id]);
