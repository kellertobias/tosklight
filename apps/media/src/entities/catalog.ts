...
export function resolveAddress(
	...
	const found = catalog?.folders.find(
		(candidate) => candidate.folder === folder,
	);
	return {
		folder: found,
		item: found?.items.find((candidate) => candidate.file === file),
	};
}

export function folderLabel(folder: CatalogFolderView): string {
	return folder.name
		? `${pad(folder.folder)} · ${folder.name}`
		: pad(folder.folder);
}
...
export function itemDetail(item: CatalogItemView): string {
	...
	const frames =
		item.frames === null ? "unknown length" : `${item.frames} frames`;
