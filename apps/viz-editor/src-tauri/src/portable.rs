...
/// The folder the editor writes its shows, library and handles into: `data/` beside a portable
/// installation, the per-user application data directory otherwise.
pub(crate) fn app_data_dir<R: tauri::Runtime, M: Manager<R>>(app: &M) -> tauri::Result<PathBuf> {
    match data_dir() {
