...
async function placeDefinitions(
	...
	const fixtures = placements.map((placement) => {
		...
		return { ...candidate.input, location: placement.position, rotation: placement.rotation };
	});
 ...
 * which is what a press of an add button does.
 */
export async function placeProfile(
	...
	try {
		return { ok: true, fixtureIds: await placeDefinitions(definition, placements) };
	} catch (reason) {
