...
pub struct Preferences {
    ...
    pub blender: String,
    /// The network interface each protocol is received on. Machine-local: which network carries
    /// DMX is a property of this computer, never of the show.
...
impl Preferences {
    ...
    pub fn to_file(&self) -> String {
        ...
        text.push_str(&format!(
            "art_net_interface {}\n",
            viz_scene::interface_line(&self.listen_interfaces.art_net)
        ));
        text.push_str(&format!(
            "sacn_interface {}\n",
            viz_scene::interface_line(&self.listen_interfaces.sacn)
        ));
        text
    }
    ...
    pub fn renderer_settings(&self) -> viz_scene::RendererSettings {
        viz_scene::RendererSettings {
            ...
            input_overrides: self
                ...
                .collect(),
            art_net_interface: self.listen_interfaces.art_net.clone(),
            sacn_interface: self.listen_interfaces.sacn.clone(),
        }
    }
    ...
    pub fn adopt_file(&mut self, text: &str, options: &Options) {
        for line in text.lines() {
            ...
            match key {
                ...
                "art_net_interface" => {
                    self.listen_interfaces.art_net = viz_scene::parse_interface(value)
                }
                "sacn_interface" => self.listen_interfaces.sacn = viz_scene::parse_interface(value),
                _ => {}
            }
        }
    }
