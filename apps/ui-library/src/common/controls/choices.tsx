...
export function MultiValueToggle<T extends string>({
	...
	return (
		<div
			className={`ui-multi-value-toggle ${className}`.trim()}
			role="radiogroup"
...
export function MultiValueToggleField<T extends string>({
	...
	const ariaLabel =
		props.ariaLabel ?? (typeof label === "string" ? label : undefined);
	return (
		<FormField
			label={label}
...
function popupPosition(
	...
	const maxHeight = Math.max(
		160,
		Math.min(440, below > 220 ? below - 8 : box.top - 8),
	);
	return {
...
function handleSelectKey<T extends string>(
	...
	if (event.key === "Escape") controller.close();
		...
		controller.setActive((index) =>
			Math.min(controller.options.length - 1, index + 1),
		);
		...
		>
			<Button
				...
				aria-activedescendant={
					open ? `${listboxId}-option-${active}` : undefined
				}
				onClick={() => {
					if (!open) {
						place();
	...
	return (
		<>
			<select
				...
				onChange={onChange}
				disabled={disabled}
				className="ui-visually-hidden-select"
				aria-hidden="true"
				tabIndex={-1}
			>
				{children}
