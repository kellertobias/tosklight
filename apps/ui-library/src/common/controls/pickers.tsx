...
export function IconPickerField({
	...
	return (
		...
		>
			...
			>
				<span className="ui-picker-preview">
					...
					<div className="ui-icon-grid" data-icon-group={group?.id}>
						{group?.icons.map((icon) => (
							<Button
								...
								aria-label={
									icon.source === "built-in" ? `Use ${icon.value}` : icon.label
								}
								title={icon.label}
								onClick={() => choose(icon.value)}
							>
								{icon.source === "catalog" ? (
...
function colorPopupPosition(
	...
	const maxHeight = Math.max(
		180,
		Math.min(470, below > 260 ? below - 8 : top - 8),
	);
	return {
...
function ColorPopup({
	...
	return createPortal(
		<div
			className="ui-color-dropdown-backdrop"
			onPointerDown={(event) => event.target === event.currentTarget && close()}
		>
			<section
				className="ui-color-dropdown-panel touch-scrollbars"
				style={position}
				aria-label={ariaLabel}
			>
				<div
					className="ui-color-dropdown-grid"
					role="listbox"
				...
				<div className="ui-color-dropdown-custom">
					<FormField
						label="Custom hex"
						className="ui-color-dropdown-custom-field"
					>
						<div className="ui-color-dropdown-custom-controls">
							...
							<Button
								variant="primary"
								disabled={!validHex(custom)}
								onClick={() => choose(custom)}
							>
								Use custom color
							</Button>
							<span
								className="ui-custom-color-preview"
								aria-label="Color preview"
								style={{
									background: validHex(custom) ? custom : "transparent",
								}}
							/>
						</div>
					</FormField>
...
export function ColorPickerField({
	...
	return (
		...
		>
			<Button
				ref={button}
				className="ui-color-input-trigger"
				disabled={disabled}
			...
			>
				<span
					className="ui-color-trigger-swatch"
					style={{ color: colorInputText(normalized) }}
				>
