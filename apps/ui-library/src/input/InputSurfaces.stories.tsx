...
import { InputModal, ModalNumberInput, ModalTextKeyboard } from "../input";
...
function NumberPadExample(args: Required<InputStoryArgs>) {
  ...
  return (
    <section style={{ width: 620 }}>
      <output aria-label="Current number" style={{ display: "block", marginBottom: 8, fontSize: 24, color: "var(--cyan)" }}>{value}</output>
      <small>{status}</small>
      <ModalNumberInput
        value={value}
        onChange={setValue}
...
function KeyboardExample({ multiline = false }: { multiline?: boolean }) {
  const [value, setValue] = useState(multiline ? "Fixture notes\nSecond line" : "Fixture");
  const [status, setStatus] = useState("Editing");
  return (
    <section>
      <output aria-label="Current text" style={{ display: "block", marginBottom: 8, fontSize: 20, color: "var(--cyan)" }}>{value}</output>
      <small>{status}</small>
      <ModalTextKeyboard
        value={value}
        onChange={setValue}
