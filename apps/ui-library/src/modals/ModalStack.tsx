...
import {
  ModalTitleBar,
  type ModalTitleTab,
} from "../common/ModalTitleBar";
import type { SearchFeatureProps } from "../common/SearchBar";
...
export function useModalStack(): ModalStackApi {
  ...
  if (!context) throw new Error("useModalStack must be used inside ModalProvider");
  return context;
}
...
function useRegisteredModal(
  ...
  if (!stack) throw new Error("Modal surfaces must be used inside ModalProvider");
  ...
  }, [id, resolvedPolicy.escape, resolvedPolicy.backdrop, resolvedPolicy.explicit]);
...
export function ModalRegistration({
  ...
  return createPortal(cloneElement(children, {
...
export function ModalFrame({
  ...
  const { onSearch, search, ...layerProps } = props;
  const resolvedPolicy = requiredPolicy(policy ?? {});
  const titleSearch = onSearch && search ? { onSearch, search } : {};
  return (
