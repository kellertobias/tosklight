...
const meta = {
	title: "ToskLight/Window System/Modal Layer",
	...
	argTypes: {
		role: {
			...
			options: ["modal", "dialog"] satisfies ModalRole[],
		},
	...
	return (
		<ModalProvider>
			<div style={{ height: 500 }}>
				...
				<output aria-label="Policy modal state">
					{open ? "Open" : "Closed"}
				</output>
			</div>
			{open && (
				...
				>
					<p>
						Escape, backdrop, and title-close behavior follow the configured
						policy.
					</p>
				</ModalFrame>
			)}
...
export const ClosePolicies: Story = {
	...
	render: (args) => (
		<PolicyExample role={args.role ?? "modal"} policy={args.policy ?? {}} />
	),
};
...
function TitleBarExample({
	...
	return (
		<ModalProvider>
			...
			>
				<p>
					Tabs, search, grouped actions, details, and explicit close share one
					title bar.
				</p>
...
export const ThirdChildAboveSearchSettings: Story = {
	render: () => (
		<TitleBarExample settingsInitiallyOpen nestedInputInitiallyOpen />
	),
};
	...
	return (
		<div style={{ height: 240 }}>
			<WindowHeader
				...
				search={{ value: search, onSearch: setSearch, placeholder: "Search fixtures" }}
				groups={[
					{
						id: "add",
						actions: [
							{ id: "add", label: "Add fixture", onPress: () => undefined },
						],
					},
				]}
			/>
...
function ProgrammaticCloser({
	children = "Close target by ID",
}: {
	children?: string;
}) {
	const stack = useModalStack();
	return (
		<Button onClick={() => stack.close("programmatic-target")}>
			{children}
		</Button>
	);
}
	...
	return (
		<ModalProvider>
			<div style={{ height: 500 }}>
				...
				<output aria-label="Programmatic modal state">
					{open ? "Open" : "Closed"}
				</output>
			</div>
			{open && (
				...
				>
					<p>
						Application adapters can close this layer through its stable
						identifier.
					</p>
					<ProgrammaticCloser />
				</ModalFrame>
...
function ApplicationRegistrationExample() {
	...
	return (
		<ModalProvider>
			{!open && (
				<Button onClick={() => setOpen(true)}>Reopen application dialog</Button>
			)}
			{open && (
				<ModalRegistration
				...
				>
					<div className="stacked-modal-layer">
						...
						>
							...
							<p>
								The existing workflow body participates in the authoritative
								modal stack.
							</p>
