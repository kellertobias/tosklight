...
describe("Storybook actual source", () => {
  it("maps every included story module to actual source", () => {
    expect(actualSourceCoverage).toHaveLength(40);
    expect(
      actualSourceCoverage.filter(({ implementationPaths }) => implementationPaths.length === 0),
    ).toEqual([]);
    expect(
      actualSourceCoverage.flatMap(({ missingImplementationPaths }) => missingImplementationPaths),
    ).toEqual([]);
  });
