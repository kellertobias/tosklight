...
import { browserStorage, defaultServerUrl, saveServerUrl } from "./serverLocation";
...
describe("desktop and browser server locations", () => {
	...
	it.each(["http://tauri.localhost", "https://tauri.localhost", "tauri://localhost"])(
		"packaged origin %s connects to the bundled backend",
		(origin) => {
	...
	it("Windows respects the saved server and the temporary test override in precedence order", () => {
		...
		expect(sessionStorage.getItem("light.server-url")).toBe("http://edited-desk:5000");
		expect(localStorage.getItem("light.server-url")).toBe("http://saved-desk:5000/");
	});
