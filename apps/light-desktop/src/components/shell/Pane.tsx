...
import { PaneView, type GridRect } from "@tosklight/ui/desktop";
...
export function Pane({
  ...
  return (
    <PaneView
      pane={{ ...pane, title: pane.kind === "file_manager" ? "File Manager" : pane.title, type: pane.kind }}
      active={active}
      maximized={maximized}
      editing={editing}
      dimensions={gridDimensions}
      info={pane.kind === "file_manager" ? { primary: "Browse and manage files", secondary: <span className="pane-chrome-info-target" ref={setChromeInfo} /> } : pane.kind === "text_editor" ? { primary: <span className="pane-chrome-info-target" ref={setChromeInfo} /> } : pane.kind === "stage" ? { primary: `${selection?.selected.length ?? 0} selected`, secondary: "Tap to select · Shift for range" } : pane.kind === "fixtures" ? { primary: `${selection?.selected.length ?? 0} selected`, secondary: <SourceLegend /> } : undefined}
      toolbar={pane.kind === "file_manager" || pane.kind === "text_editor" ? <span className="pane-chrome-toolbar-target" ref={setChromeToolbar} /> : undefined}
      actions={stageActions}
      settings
      onSettings={() => dispatch({ type: "SET_PANE_SETTINGS", id: pane.id })}
      onTitleClick={deleteArmed ? removeFromDelete : undefined}
      titleActionLabel={deleteArmed ? `Remove ${pane.kind === "file_manager" ? "File Manager" : pane.title} pane` : undefined}
      onRectChange={updateRect}
    >
        <PaneChromeProvider value={{ info: chromeInfo, toolbar: chromeToolbar }}>
	          <Window active={active} compact paneId={pane.id} showGroupShortcuts={Boolean(pane.showGroupShortcuts)} showCueSidebar={pane.showCueSidebar ?? true} cueListSource={pane.cueListSource ?? "fixed"} fixedCueListNumber={pane.fixedCueListNumber} stageView={pane.stageView ?? state.stageView} followPreload={Boolean(pane.followPreload)} showBeamGuides={pane.showBeamGuides ?? true} presetFamily={pane.presetFamily ?? state.presetFamily} presetPoolColors={pane.presetPoolColors ?? true} />
        </PaneChromeProvider>
    </PaneView>
  );
}
