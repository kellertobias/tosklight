...
describe("fixture profile model primitives", () => {
	...
	it("starts with one Default mode, one editable main head, and revision zero", () => {
		...
		expect(profile).toMatchObject({
			schema_version: 2,
			revision: 0,
			modes: [{ name: "Default", heads: [{ name: "Main" }] }],
		});
		expect(profile.modes[0].heads[0].id).toBeTruthy();
	});
});

describe("fixture profile model Highlight defaults", () => {
	it("derives full-and-white Highlight raw values for authored and imported channel semantics", () => {
		...
		expect(
			semanticHighlightRaw("color.wheel.1", "u8", 7, false, [
				{ name: "Red", dmx_from: 200, dmx_to: 255 },
			]),
		).toBe(7);
...
describe("fixture profile model calibrated Highlight defaults", () => {
	it("tracks new wheel and calibrated-color defaults only while Highlight raw is untouched", () => {
		...
		expect(
			reconcileColorSystemHighlightDefaults(mode, wheelSystem).channels[0]
				.highlight_raw,
		).toBe(15);
		expect(
			reconcileColorSystemHighlightDefaults(
				{ ...mode, channels: [{ ...wheel, highlight_raw: 73 }] },
				wheelSystem,
			).channels[0].highlight_raw,
		).toBe(73);
		...
		const expected = semanticHighlightDefaultsForMode({
			...additiveMode,
			channels: additiveMode.channels.map((channel, index) =>
				index === 1 ? { ...channel, highlight_raw: 255 } : channel,
			),
			color_systems: additiveSystem,
		});
		const reconciled = reconcileColorSystemHighlightDefaults(
			additiveMode,
			additiveSystem,
		);
		expect(reconciled.channels[0].highlight_raw).toBe(
			expected.get(reconciled.channels[0].id),
		);
		expect(reconciled.channels[0].highlight_raw).toBeLessThan(255);
		expect(reconciled.channels[1].highlight_raw).toBe(33);
		expect(reconciled.channels[2].highlight_raw).toBe(
...
describe("fixture profile model addressing and geometry", () => {
	it("derives primary slots around exact reserved component slots per split", () => {
		...
		const mode = profile.modes[0];
		mode.splits = [
			{ number: 1, footprint: 6 },
			{ number: 2, footprint: 3 },
		];
		const splitTwoHead = {
			...mode.heads[0],
			id: crypto.randomUUID(),
		...
		const first = {
			...blankChannel(mode, 1),
			resolution: "u16" as const,
			secondary_slots: [2],
		};
		const second = {
			...blankChannel(mode, 1),
			resolution: "u24" as const,
			secondary_slots: [5, 6],
		};
		...
		expect([
			result.slots.get(first.id),
			result.slots.get(second.id),
			result.slots.get(third.id),
			result.slots.get(remote.id),
		]).toEqual([1, 3, 4, 1]);
	});
	...
	it("keeps 32-bit raw integers exact and reports duplicate or invalid slots", () => {
		...
		const first: FixtureChannel = {
			...blankChannel(mode),
			resolution: "u32",
			secondary_slots: [2, 3, 4],
			default_raw: maxRaw("u32"),
			highlight_raw: 0xffffffff,
		};
		mode.channels = [first];
	...
	it("creates all five geometry templates with their documented hierarchy and source layouts", () => {
		...
		for (const entry of cases) {
			...
			expect(
				geometry.emitters.map((emitter) => emitter.head_id),
				entry.template,
			).toEqual(heads);
			expect(
				geometry.emitters.map((emitter) => emitter.layout.type),
				entry.template,
			).toEqual([entry.layout, entry.layout]);
			const pan = geometry.nodes.find(
				(node) => node.motion?.attribute === "pan",
			);
			const tilts = geometry.nodes.filter(
				(node) => node.motion?.attribute === "tilt",
			);
			if (
				entry.template === "moving_head" ||
				entry.template === "shared_pan_multi_head"
			) {
				expect(pan, entry.template).toBeTruthy();
				expect(tilts, entry.template).toHaveLength(2);
				expect(
					tilts.every((node) => node.parent_id === pan?.id),
					entry.template,
				).toBe(true);
			} else {
...
describe("fixture profile model migration and catalogs", () => {
	it("keeps every profile mode distinct while suppressing retained migrated legacy rows", () => {
		...
		const merged = mergeFixtureDefinitions(
			[profile],
			[migratedLegacy, unrelated],
		);
		expect(merged).toHaveLength(3);
		expect(merged.map((definition) => definition.mode)).toEqual([
			"Standard",
			"Extended",
			"Standard",
		]);
		expect(
			new Set(
				merged
					.slice(0, 2)
					.map(
						(definition) => `${definition.profile_id}:${definition.mode_id}`,
					),
			).size,
		).toBe(2);
	...
	it("shares one immutable library snapshot across large mode catalogs and clones editor drafts", () => {
		...
		expect(
			definitions.every(
				(definition) => definition.profile_snapshot === profile,
			),
		).toBe(true);
	...
	it("combines an ordered legacy/GDTF mode import into one unsaved atomic profile", () => {
		const standard = legacyDefinition({
			manufacturer: "Acme",
			name: "Orbit",
			model: "Orbit 500",
			mode: "Standard",
		...
		const extended = legacyDefinition({
			manufacturer: "Acme",
			name: "Orbit",
			model: "Orbit 500",
			mode: "Extended",
		...
		expect(profile).toMatchObject({
			...
			manufacturer: "Acme",
			name: "Orbit",
			short_name: "Orbit 500",
		});
		expect(
			profile.modes.map((mode) => [mode.name, mode.splits[0].footprint]),
		).toEqual([
			["Standard", 8],
			["Extended", 16],
		]);
	...
	it("keeps an additive head's color correction matrix in its portable fixture definition", () => {
		...
		expect(
			fixtureDefinitionFromProfileMode(profile, mode).color_calibration
				?.correction_matrix,
		).toEqual(correctionMatrix);
	});
});
