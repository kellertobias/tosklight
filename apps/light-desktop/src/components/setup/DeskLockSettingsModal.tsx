...
export function DeskLockSettingsModal({ onClose }: { onClose: () => void }) {
	...
	const [wallpaper, setWallpaper] = useState<string | null>(
		deskLock?.wallpaper ?? null,
	);
	const [unlockMode, setUnlockMode] = useState<"button" | "pin">(
		deskLock?.unlock_mode ?? "button",
	);
	...
	return (
		<ModalRegistration onClose={onClose}>
			...
			>
				<section
					className="nested-modal desk-lock-settings-modal"
					role="dialog"
					aria-modal="true"
					aria-label="Desk Lock"
				>
					<ModalTitleBar
						title="Desk Lock"
						...
						accept={{
							id: "save",
							label: "Save Lock Configuration",
							variant: "primary",
							onPress: () => void save(),
						}}
					/>
					<p>
						Locking this desk blocks every connected screen and its assigned
						hardware without changing playback, programmer, or output state.
					</p>
