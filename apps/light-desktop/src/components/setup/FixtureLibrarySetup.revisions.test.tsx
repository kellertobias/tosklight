...
describe("fixture profile revision editing", () => {
	it("filters the fixture library immediately while typing and restores it with Clear search", async () => {
		...
		expect(
			screen.queryByRole("button", { name: "Generic" }),
		).not.toBeInTheDocument();
	...
	it("uses a retained revision as the draft but the latest revision as the optimistic save check", async () => {
		...
		server.saveFixtureProfile.mockImplementation(
		...
		await waitFor(() =>
			expect(server.fixtureProfileRevisions).toHaveBeenCalledWith(latest.id),
		);
		const retainedArticle = screen.getByText("Revision 2").closest("article")!;
		fireEvent.click(
			within(retainedArticle).getByRole("button", {
				name: "Edit as new revision",
			}),
		);
		...
		await waitFor(() =>
			expect(server.saveFixtureProfile).toHaveBeenCalledOnce(),
		);
