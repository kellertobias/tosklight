...
/** The Show Patch columns whose header orders the table. */
...
/**
 * The fixtures in the order `sort` asks for. A fixture with nothing in the column — unpatched or
 * without a fixture ID — sorts after every fixture that has a value, in either direction, and ties
 * keep fixture ID order so the table never shuffles between renders.
 *
 * `layerOrder` is the stored layer order by layer ID; the implicit default layer comes first.
 */
export function sortPatchFixtures(
	fixtures: readonly PatchedFixture[],
	sort: PatchSort,
	layerOrder: ReadonlyMap<string, number> = new Map(),
): PatchedFixture[] {
	...
	return fixtures
		.map((fixture) => ({
			fixture,
			key: sortKey(fixture, sort.column, layerOrder),
		}))
		.sort(
			(a, b) =>
...
function sortKey(
	...
	switch (column) {
		...
		case "Fixture / mode":
			return `${definition.name || definition.model} · ${definition.mode}`;
		case "Patch":
			return patchKey(fixture);
		case "Layer":
			return layerOrder.get(fixture.layer_id || "default") ?? -1;
		case "Location X":
		case "Location Y":
		case "Location Z":
