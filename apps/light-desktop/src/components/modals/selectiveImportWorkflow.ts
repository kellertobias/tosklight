...
export function useSelectiveImportWorkflow(
	...
	const previewCurrent =
		state.preview !== null && state.previewKey === selectionKey;
	...
	async function apply() {
		...
		try {
			const outcome = await options.applyImport(
				options.activeShow.id,
				state.sourceId,
				{
	...
	return {
		...
		previewCurrent,
		selection,
		setMode: (mode: SelectiveImportLoadMode) =>
			patch({
				mode,
				objectChoices: new Map(),
				profileChoices: new Map(),
		...
		setObjectChoice: (
			...
			patch((current) => ({
				objectChoices: updatedMap(
					current.objectChoices,
					objectKeyId(key),
					value,
				),
				previewKey: "",
		...
		setProfileChoice: (
			...
			patch((current) => ({
				profileChoices: updatedMap(
					current.profileChoices,
					profileKeyId(key),
					value,
				),
				previewKey: "",
...
export type SelectiveImportWorkflow = ReturnType<
	typeof useSelectiveImportWorkflow
>;
