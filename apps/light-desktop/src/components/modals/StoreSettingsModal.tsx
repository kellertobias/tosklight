...
export function StoreSettingsModal() {
  ...
  return <ModalPortal onClose={close}><div className="modal-backdrop" onPointerDown={(event) => event.target === event.currentTarget && close()}><section className="modal-card store-settings-modal workflow-theme record-workflow" role="dialog" aria-modal="true" aria-label="Record Settings"><ModalTitleBar title={<><span className="workflow-badge">RECORD</span> Record Settings</>} closeLabel="Close Record Settings" onClose={close}/><p>Defaults for the armed Record workflow.</p><RecordDefaultsFields settings={settings} onChange={change}/><div className="modal-actions"><Button className="workflow-primary" onClick={close}>Done</Button></div></section></div></ModalPortal>;
}
