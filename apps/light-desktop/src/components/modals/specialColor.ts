...
export function selectedFixtureIdsSupportingAttribute(
	...
	const headSupports = (fixture: PatchedFixture, headIndex: number) =>
		fixture.definition.heads[headIndex]?.parameters.some((parameter) =>
			attributes.includes(parameter.attribute),
		) ?? false;
	for (const fixture of fixtures) {
		if (
			selected.has(fixture.fixture_id) &&
			fixture.definition.heads.some((_, index) => headSupports(fixture, index))
		) {
			result.add(fixture.fixture_id);
		}
		for (const head of fixture.logical_heads) {
			if (
				selected.has(head.fixture_id) &&
				headSupports(fixture, head.head_index)
			) {
				result.add(head.fixture_id);
			}
		}
	}
	return [...result];
