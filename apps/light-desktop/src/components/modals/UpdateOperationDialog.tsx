			...
			>
				...
				>
					<ModalTitleBar
						title={
							<>
								<span>UPDATE</span> {preview.target.name}
							</>
						}
						details={updateTargetContext(preview.target)}
						groups={[
							{
								id: "update-operation-actions",
								actions: [
									{
										id: "cancel",
										label: "Cancel",
						...
						accept={{
							...
							label: busy
								? "Updating…"
								: `Update ${targetFamilyLabel(preview.target)}`,
							variant: "primary",
							disabled: busy || stats.changed === 0,
							onPress: onApply,
						}}
						closeLabel="Close Update"
						closeDisabled={busy}
						onClose={onCancel}
					/>
					<p>
						Choose how the current programmer values change this target. The
						preview lists what each mode would change; nothing is stored until
						you press Update.
					</p>
