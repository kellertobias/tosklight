...
import type { OutputRuntimeStore } from "./store";
import type {
...
export interface OutputRuntimeSessionOptions {
...
/** Reference-counted owner of the exact Show's global output projection. */
export class OutputRuntimeSession {
	private references = 0;
	private lifecycle = 0;
	private hydrated = false;
	private stopped = false;
	private stream: OutputRuntimeEventStream | null = null;
	...
	constructor(private readonly options: OutputRuntimeSessionOptions) {}
	...
	async repairAuthority(error: Error) {
		if (this.stopped || this.storeScope === null || this.references === 0)
			throw new Error("The Output runtime view is unavailable");
		await this.repair(this.lifecycle, error);
		const state = this.options.store.getSnapshot();
		if (state.repairRequired)
			throw state.error ?? new Error("Output runtime repair failed");
	}
	...
	private openStream(generation: number, cursor: number | null) {
		if (!this.isCurrent(generation)) return;
		let stream: OutputRuntimeEventStream;
		try {
			stream = this.options.transport.subscribe(this.options.scope, cursor, {
				...
				error: (error) => {
					if (!this.isCurrent(generation)) return;
					if (error instanceof OutputRuntimeProtocolError)
	...
	private handleMessage(
		message: OutputRuntimeEventMessage,
		generation: number,
	) {
		...
		if (message.type === "gap" || message.type === "error") {
			void this.repair(
				...
				new OutputRuntimeProtocolError(
					message.type === "gap"
						? "Output runtime event history has a gap"
						: message.error,
	...
	private loadSnapshot(): Promise<OutputRuntimeSnapshot> {
	...
	private ensureStoreScope() {
		...
		if (
			!sameId(scoped.showId, this.options.scope.showId) ||
	...
	private scopeError(subject: string) {
		return new OutputRuntimeProtocolError(
			`Output runtime ${subject} does not match the active Show and desk authority`,
		);
	}
