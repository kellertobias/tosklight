...
import type { ProgrammerCaptureModeStore } from "./store";
import {
	type ProgrammerCaptureModeEventStream,
	type ProgrammerCaptureModeEventTransport,
	ProgrammerCaptureModeProtocolError,
} from "./transport";
...
export interface ProgrammerCaptureModeSessionOptions {
	...
	loadSnapshot(): Promise<ProgrammerCaptureModeSnapshot>;
...
export class ProgrammerCaptureModeSession {
	...
	private readonly loadSnapshot: ProgrammerCaptureModeSessionOptions["loadSnapshot"];
	private readonly onError?: (error: Error | null) => void;
	private references = 0;
	private lifecycle = 0;
	private hydrated = false;
	private stopped = false;
	private stream: ProgrammerCaptureModeEventStream | null = null;
	...
	async repairAuthority(error: Error) {
		if (this.stopped || this.storeScope === null)
			throw new Error("The Programmer capture mode session is unavailable");
		...
		if (state.repairRequired)
			throw state.error ?? new Error("Programmer capture mode repair failed");
	}
	...
	private openStream(generation: number, cursor: number | null) {
		...
		let stream: ProgrammerCaptureModeEventStream;
		try {
			stream = this.transport.subscribe(this.eventScope, cursor, {
				...
				error: (error) => {
					if (!this.isCurrent(generation)) return;
					if (error instanceof ProgrammerCaptureModeProtocolError)
	...
	private handleMessage(
		message: ProgrammerCaptureModeEventMessage,
		generation: number,
	) {
		...
		if (message.type === "gap") {
			void this.repair(
				...
				new ProgrammerCaptureModeProtocolError(
					"Programmer capture mode event history has a gap",
		...
		if (message.type === "error") {
			void this.repair(
				generation,
				new ProgrammerCaptureModeProtocolError(message.error),
			);
			return;
		}
	...
	private scopeError(subject: string) {
		return new ProgrammerCaptureModeProtocolError(
			`Programmer capture mode ${subject} does not match the active user view`,
		);
	}
