...
import { toWireFixture } from "../../api/PatchTransport";
...
describe("a Venue object's model scale through the desk patch", () => {
	it("is sent with every edit and read back unchanged", () => {
		const { input, wire, read } = roundTrip(hall(2.5));
		expect(input.modelScale).toBe(2.5);
		expect(wire.model_scale).toBe(2.5);
		expect(read.model_scale).toBe(2.5);
	});

	it("reads an object placed before the scale existed at its built size", () => {
		const { wire, read } = roundTrip(hall());
		expect(wire.model_scale ?? null).toBeNull();
