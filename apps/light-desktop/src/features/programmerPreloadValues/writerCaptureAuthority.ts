...
import type { ProgrammerPreloadValuesScope } from "./contracts";
...
/** Captures the exact-user active Preload mode generation used by a writer. */
export class ProgrammerPreloadCaptureAuthority {
	...
	preconditionError(expectedRevision: number) {
		...
		if (!capturesProgrammerWrites(projection))
			return new Error(
				"Preload Programmer values require active Preload capture",
			);
		return null;
	}
...
export async function awaitPreloadAuthorityRepairs(
