...
const app = path.join(artifactPaths.cargo, "debug/bundle/macos/ToskLight.app/Contents/MacOS/light-desktop");
const server = path.join(artifactPaths.cargo, "debug/light-headless");
...
async function lightServerChildren(parentPid) {
  ...
  return stdout.split("\n").flatMap((line) => {
    ...
    return match && Number(match[2]) === parentPid && /(?:^|\/)light-headless(?:\s|$)/.test(match[3]) ? [Number(match[1])] : [];
  });
}
