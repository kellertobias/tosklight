...
import { noPatchDiagnostics, type PatchDiagnostics } from "../diagnostics";
...
interface PatchViewProviderProps {
	...
	/** Optional host instrumentation for patch mutations. */
...
export function PatchViewProvider({
	...
	const session = useMemo(() => {
		...
		return new PatchSession({
			...
			diagnostics: {
				beginPatchMutation: (requestId, fixtureCount) =>
					(diagnosticsRef.current ?? noPatchDiagnostics).beginPatchMutation(
						requestId,
						fixtureCount,
					),
			},
