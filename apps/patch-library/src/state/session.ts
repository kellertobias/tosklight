...
export interface PatchSessionOptions {
	...
	/** Optional host instrumentation. Defaults to measuring nothing. */
...
export class PatchSession {
	...
	private readonly diagnostics: PatchDiagnostics;
	...
	constructor(options: PatchSessionOptions) {
		...
		this.diagnostics = options.diagnostics ?? noPatchDiagnostics;
	...
	updatePolicy(
		...
		const performanceSample =
			this.diagnostics.beginPatchMutation(requestId, 1);
	...
	private queuePatch(
		...
		const requestId = crypto.randomUUID();
		const performanceSample =
			this.diagnostics.beginPatchMutation(requestId, initial.length);
	...
	private async runPatch(
		...
		placements: readonly PatchPlacement[],
		lifecycle: number,
		performanceSample: PatchMutationSample,
	): Promise<PatchMutationOutcome> {
		try {
