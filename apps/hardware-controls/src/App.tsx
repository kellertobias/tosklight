...
function Playback({ slot, levels, lamps, send, buttons = 3 }: PlaybackProps) {
  const base = `paged-playback/${slot}`;
...
function EncoderEmulation({ number, nav = false, send }: { number: number; nav?: boolean; send: (path: string, args: unknown[]) => void }) {
  ...
  const path = nav ? "nav" : `encode/${number}`;
...
export function App() {
  ...
  useEffect(() => { const off = listen<Feedback>("osc-feedback", ({ payload }) => {
    ...
    const offset = parts.indexOf("paged-playback"); if (offset < 0) return; const slot = Number(parts[offset + 1]);
  ...
  const action = (name: string, down: boolean) => send(`programmer/${name}`, [down]);
  ...
  const speedGroups = <section className="speed-groups"><h2>Speed groups</h2>{[1, 2, 3, 4, 5].map((number) => <div className="encoder" key={number}><ControlButton label={`SPEED ${number}`} lamp={lamps[`speed/${number}`]} onDown={() => send(`speed-group/${number}/button`, [true])} onUp={() => send(`speed-group/${number}/button`, [false])}/><input type="range" min="1" max="999" defaultValue={120} onChange={(event) => send(`speed-group/${number}/encoder`, [Number(event.target.value)])}/></div>)}</section>;
  ...
  return <main><header><h1>ToskLight <span>Hardware Controls</span></h1><i className={connected ? "connected" : ""}>{connected ? `● Connected · page ${page}` : "○ Connecting…"}</i></header>
    ...
    {tab === "console" ? <section className="console-layout">
      <aside className="left-rail">{key("ESCAPE")}{key("MENU")}{key("PROG-PLAYBACK")}{key("PRELOAD")}<span className="button-spacer"/><button onClick={() => send("page", [Math.max(1, page - 1)])}>PAGE UP</button><strong>{page}</strong><button onClick={() => send("page", [page + 1])}>PAGE DOWN</button></aside>
      <section className={`playback-surface ${top ? "with-top-row" : "without-top-row"}`}><div className="encoder-row">{Array.from({ length: 6 }, (_, index) => <EncoderEmulation key={index} number={index + 1} send={send}/>)}<EncoderEmulation number={7} nav send={send}/></div><div className="top-row">{Array.from({ length: 20 }, (_, index) => index + 21).map((slot) => <ControlButton key={slot} label={String(slot)} lamp={lamps[`${slot}/1`]} onDown={() => send(`paged-playback/${slot}/button/1`, [true])} onUp={() => send(`paged-playback/${slot}/button/1`, [false])}/>)}</div>
      ...
      <aside className="programmer-panel"><div className="record-clear">{key("RECORD")}{key("CLEAR")}{key("UNDO")}</div><div className="edit-actions">{key("DEL")}{key("CPY")}{key("MOV")}</div><div className="keypad">
        {key("GRP")}{key("SET")}{key("DIV")}{key("ADD")}{key("7")}{key("8")}{key("9")}{key("THRU")}{key("4")}{key("5")}{key("6")}{key("AT")}{key("1")}{key("2")}{key("3")}{key("ENTER", "enter-double")}{key(".")}{key("0")}{key("⌫", "", "backspace")}
