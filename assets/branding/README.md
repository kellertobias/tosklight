...
Media uses the regular ToskLight application icon with a glowing LED-wall badge in its bottom-right
corner. `tosklight-media-badge.svg` is the editable overlay; `tosklight-media-icon.svg` and
`tosklight-media-icon.png` are generated by:

```sh
python3 tools/generate_media_icon.py
```
