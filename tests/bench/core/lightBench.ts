...
export class LightBench {
  ...
  async stopServerGracefully(token: string): Promise<number> {
    ...
    if (!process?.pid || process.exitCode !== null) throw new Error("light-headless is not running");
  ...
  async startServer(): Promise<number> {
    if (this.process && this.process.exitCode === null) throw new Error("light-headless is already running");
    await this.spawnServer();
    if (!this.process?.pid) throw new Error("light-headless started without a PID");
  ...
  async failureArtifacts(token: string): Promise<Record<string, string>> {
    ...
    return {
      "light-headless.log": this.recentLog(),
  ...
  private async waitForServerExit(process: ChildProcess, timeout: number): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        process.off("exit", onExit);
        reject(new Error(`Timed out waiting for light-headless PID ${process.pid} to exit`));
      }, timeout);
      timer.unref();
  ...
  private async waitUntilReady(): Promise<void> {
    ...
    while (Date.now() < deadline) {
      if (this.process?.exitCode !== null) throw new Error(`light-headless exited during startup:\n${this.recentLog()}`);
      try {
    ...
    throw new Error(`Timed out starting light-headless:\n${this.recentLog()}`);
  }
}
