...
describe("Plan 76 preset library", () => {
  it("defines the exact 13 Color, 7 Position, and 10 Beam presets", async () => {
    ...
    const result = await installPlannedDemoPresets({
      seedShowObject: async (_showId: string, _kind: string, id: string, body: any) => {
        writes.push({ id, body });
      },
    } as any, "show", fixtures);
    expect(result).toEqual({ colors: 13, positions: 7, beam: 10 });
    expect(writes).toHaveLength(30);
    expect(writes.filter((write) => write.body.family === "Color").map((write) => write.body.name))
      .toEqual([
        "Red", "Orange", "Yellow", "Lime", "Green", "Teal", "Cyan",
        "Light Blue", "Dark Blue", "Purple", "Magenta", "White", "Tungsten White",
      ]);
    expect(writes.find((write) => write.body.name === "Blind")?.body.values["fixture-101"])
      .toHaveProperty("pan");
    expect(writes.find((write) => write.body.name === "Prism Rotation")?.body.values["fixture-101"])
