...
describe("Plan 76 demo manifest", () => {
	...
	it("reconciles every moving-family location group", () => {
		const expected = {
			profile: { stage: 28, audience: 6, aux: 4, all: 38 },
			wash: { stage: 26, audience: 8, aux: 4, all: 38 },
			led: { stage: 16, audience: 100, aux: 10, all: 126 },
		} as const;
		for (const [family, counts] of Object.entries(expected)) {
			expect(
				plannedDemoFamilyNumbers(family as "profile" | "wash" | "led", "stage"),
			).toHaveLength(counts.stage);
			expect(
				plannedDemoFamilyNumbers(
					family as "profile" | "wash" | "led",
					"audience",
				),
			).toHaveLength(counts.audience);
			expect(
				plannedDemoFamilyNumbers(family as "profile" | "wash" | "led", "aux"),
			).toHaveLength(counts.aux);
			expect(
				plannedDemoFamilyNumbers(family as "profile" | "wash" | "led"),
			).toHaveLength(counts.all);
		}
	...
	it("defines four eight-lamp ACL controls and the approved Front Split composition", () => {
		...
		const acls = PLANNED_DEMO_FIXTURES.filter((fixture) =>
			fixture.roles.includes("All ACLs"),
		);
