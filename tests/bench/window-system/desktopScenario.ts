...
import type { PaneType, PaneConfiguration, OperatorPaneType } from "./paneTypes";
import { paneLabels, PresetFamily, StageView } from "./paneTypes";
...
export class DesktopConfiguration {
	...
	setConfiguration<T extends OperatorPaneType>(
		...
		if (!definition) throw new Error(`Pane "${handle.slug}" does not belong to this Desktop configuration`);
		definition.configuration = configuration as PaneConfiguration<OperatorPaneType>;
	}
...
export class PaneHandle<T extends PaneType> {
	...
	move = (position: Pick<PaneGeometry, "column" | "row">) => this.desktops.setGeometry(this, position);
	resize = (size: Pick<PaneGeometry, "width" | "height">) => this.desktops.setGeometry(this, size);

	readonly expect = {
		geometry: (geometry: PaneGeometry) => this.desktops.expectGeometry(this, geometry),
		visible: () => expect(this.desktops.locatorFor(this)).toBeVisible(),
		maximized: (value = true) => expect(this.desktops.locatorFor(this)).toHaveAttribute("aria-expanded", String(value)),
	};
}

export class BrowserDesktops {
	private readonly bindings = new Map<string, PaneBinding>();
	private readonly layouts = new Map<string, (configuration: DesktopConfiguration) => void>();
	...
	define(name: string, recipe: (configuration: DesktopConfiguration) => void): void {
		if (this.layouts.has(name)) throw new Error(`Desktop "${name}" is already registered`);
	...
	async create(name: string): Promise<void> {
		...
		if (await this.desktopButton(name).count()) throw new Error(`Desktop "${name}" already exists`);
		await this.page.getByRole("button", { name: "New desktop", exact: true }).click();
	...
	async open(name: string): Promise<void> {
		...
		if (!(await button.count())) throw new Error(`Desktop "${name}" does not exist`);
		await button.click();
		await expect(this.desktopRoot(name)).toBeVisible();
	}
	...
	async closeSettings(): Promise<void> {
		const dialog = this.page.getByRole("dialog", { name: "Desktop settings" });
		if (await dialog.count()) await dialog.getByRole("button", { name: "×" }).click();
	}

	async clone(name: string, cloneName: string): Promise<void> {
		await this.open(name);
		await this.openSettings(this.desktopButton(name));
		await this.page.getByRole("dialog", { name: "Desktop settings" }).getByRole("button", { name: "Clone current desktop" }).click();
		const active = this.page.locator("[data-desktop-id][aria-current=page]");
		await this.openSettings(active);
		const dialog = this.page.getByRole("dialog", { name: "Desktop settings" });
	...
	getPane<T extends PaneType>(type: T, slug: string): PaneHandle<T> {
		...
		if (!binding) throw new Error(`No pane is bound to slug "${slug}" on the current Desktop`);
		if (binding.type !== type) throw new Error(`Pane "${slug}" is ${binding.type}, not requested type ${type}`);
		return new PaneHandle(this, type, slug);
	}

	createHandle<T extends PaneType>(type: T, slug: string, draft?: DesktopConfiguration): PaneHandle<T> {
	...
	async configurePane<T extends PaneType>(handle: PaneHandle<T>, configuration: PaneConfiguration<T>): Promise<void> {
		const pane = this.locatorFor(handle);
		if (!(await pane.count())) throw new Error(`Pane "${handle.slug}" is no longer present`);
	...
	async setGeometry<T extends PaneType>(handle: PaneHandle<T>, geometry: Partial<PaneGeometry>): Promise<void> {
		...
		for (const [label, value] of [["Grid column", geometry.column], ["Grid row", geometry.row], ["Grid width", geometry.width], ["Grid height", geometry.height]] as const) {
			if (value !== undefined) await chooseOption(this.page, dialog, label, String(value));
		}
		await this.closePaneSettings();
	}
	...
	async maximize<T extends PaneType>(handle: PaneHandle<T>, maximized: boolean): Promise<void> {
		const pane = this.locatorFor(handle);
		if ((await pane.getAttribute("aria-expanded")) === String(maximized)) return;
		await this.openPaneSettings(pane);
		await this.page.getByRole("dialog", { name: "Pane Settings" }).getByRole("button", { name: maximized ? "Maximize pane" : "Restore pane" }).click();
		await this.closePaneSettings();
	...
	async remove<T extends PaneType>(handle: PaneHandle<T>): Promise<void> {
		const pane = this.locatorFor(handle);
		await this.openPaneSettings(pane);
		await this.page.getByRole("dialog", { name: "Pane Settings" }).getByRole("button", { name: "Remove pane" }).click();
		await expect(pane).toHaveCount(0);
		this.bindings.delete(handle.slug);
	}

	async screenshot<T extends PaneType>(handle: PaneHandle<T>, name: string): Promise<void> {
		await this.attach(name, await this.locatorFor(handle).screenshot());
	}

	async expectGeometry<T extends PaneType>(handle: PaneHandle<T>, geometry: PaneGeometry): Promise<void> {
		const pane = this.locatorFor(handle);
		for (const [attribute, value] of [["data-grid-column", geometry.column], ["data-grid-row", geometry.row], ["data-grid-width", geometry.width], ["data-grid-height", geometry.height]] as const) {
	...
	locatorFor<T extends PaneType>(handle: PaneHandle<T>): Locator {
		...
		if (!binding) throw new Error(`Pane handle "${handle.slug}" is unbound; apply its Desktop configuration first`);
		if (binding.type !== handle.type) throw new Error(`Pane "${handle.slug}" is ${binding.type}, not requested type ${handle.type}`);
		return this.page.locator(`[data-pane-id="${binding.runtimeId}"]`);
	}

	private async addPane(definition: PaneDefinition): Promise<void> {
		const before = await this.page.locator("[data-pane-id]").evaluateAll((elements) => elements.map((element) => element.getAttribute("data-pane-id")));
		const grid = this.page.locator(".desk-grid");
		const box = await grid.boundingBox();
		if (!box) throw new Error("The active Desktop grid is not visible");
		const x = box.x + (definition.placement.column - 0.5) * box.width / GRID_COLUMNS;
		const y = box.y + (definition.placement.row - 0.5) * box.height / GRID_ROWS;
		await grid.click({ position: { x: x - box.x, y: y - box.y } });
		await this.page.getByRole("dialog", { name: "Open Window" }).getByRole("button", { name: paneLabels[definition.type], exact: true }).click();
		await expect(this.page.locator("[data-pane-id]")).toHaveCount(before.length + 1);
		const runtimeId = await this.page.locator("[data-pane-id]").evaluateAll(
			(elements, previous) => elements.map((element) => element.getAttribute("data-pane-id")).find((id) => id && !previous.includes(id)) ?? null,
			before,
		);
		const pane = this.page.locator(`[data-pane-id="${runtimeId}"]`);
		await expect(pane).toBeVisible();
		if (!runtimeId) throw new Error(`Created ${definition.type} pane has no runtime identity`);
		this.bindings.set(definition.placement.slug, { type: definition.type, runtimeId });
		await this.setGeometry(definition.handle, definition.placement);
		if (definition.configuration) await this.configurePane(definition.handle, definition.configuration);
	}

	private async readGeometry(pane: Locator): Promise<PaneGeometry> {
		const values = await Promise.all(["data-grid-column", "data-grid-row", "data-grid-width", "data-grid-height"].map((name) => pane.getAttribute(name)));
		return { column: Number(values[0]), row: Number(values[1]), width: Number(values[2]), height: Number(values[3]) };
	}

	private async openPaneSettings(pane: Locator): Promise<void> {
		await pane.getByRole("button", { name: "Settings", exact: true }).click();
		await expect(this.page.getByRole("dialog", { name: "Pane Settings" })).toBeVisible();
	}

	private async closePaneSettings(): Promise<void> {
		const dialog = this.page.getByRole("dialog", { name: "Pane Settings" });
		if (await dialog.count()) await dialog.getByRole("button", { name: "Close settings" }).click();
	}

	private async showDesktops(): Promise<void> {
		await this.page.getByRole("button", { name: "DESKTOPS", exact: true }).click();
	}

	private desktopButton(name: string): Locator {
		return this.page.locator("[aria-label=Desktops]").getByRole("button", { name, exact: true });
	}

	private desktopRoot(name: string): Locator {
		return this.page.locator(`[data-light-surface="desktop"][aria-label="Desktop ${name}"]`);
	}
	...
	private async openSettings(button: Locator): Promise<void> {
		...
		await expect(this.page.getByRole("dialog", { name: "Desktop settings" })).toBeVisible();
	}
}

function validatePlacement(candidate: PanePlacement, existing: readonly PanePlacement[]): void {
	if (!SLUG.test(candidate.slug)) throw new Error(`Pane slug "${candidate.slug}" must be unique kebab-case`);
	if (existing.some((pane) => pane.slug === candidate.slug)) throw new Error(`Duplicate pane slug "${candidate.slug}"`);
	if ([candidate.column, candidate.row, candidate.width, candidate.height].some((value) => !Number.isInteger(value) || value < 1)
		|| candidate.column + candidate.width - 1 > GRID_COLUMNS
		|| candidate.row + candidate.height - 1 > GRID_ROWS) {
		throw new Error(`Pane "${candidate.slug}" must fit inside the 24 × 18 Desktop grid`);
	}
	if (existing.some((pane) => candidate.column < pane.column + pane.width && candidate.column + candidate.width > pane.column && candidate.row < pane.row + pane.height && candidate.row + candidate.height > pane.row)) {
		throw new Error(`Pane "${candidate.slug}" collides with another configured pane`);
	}
}

async function applyPaneConfiguration<T extends PaneType>(page: Page, type: T, configuration: PaneConfiguration<T>): Promise<void> {
	...
	if (type === "stage") {
		...
		if (options.view) await dialog.getByRole("radio", { name: options.view === StageView.ThreeDimensional ? "3D" : "2D" }).click();
		await setSwitch(dialog, "Follow Preload", options.followPreload);
...
async function setSwitch(dialog: Locator, name: string, value: unknown): Promise<void> {
