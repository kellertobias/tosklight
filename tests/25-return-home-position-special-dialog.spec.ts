			...
			return fixture.definition.heads.flatMap((head: any) => {
				...
				const home = (["pan", "tilt"] as const).flatMap((attribute) => {
					const parameter = head.parameters.find(
						(candidate: any) => candidate.attribute === attribute,
					);
					if (!parameter) return [];
					return [
						{
		...
		const nonPosition = patch.fixtures.find((fixture: any) =>
			fixture.definition.heads.every((head: any) =>
				head.parameters.every(
					(parameter: any) => !["pan", "tilt"].includes(parameter.attribute),
				),
			),
		);
		const selected = [
		...
		.poll(async () => {
			...
			return expected.every((assignment) => {
				const actual = values.find(
					(value) =>
						value.fixture_id === assignment.fixture_id &&
