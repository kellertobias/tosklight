...
test("FIXTURE-SHEET-002-010 @ui › scenery stays out of the programmable sheet and compact choices are literal", async ({
	api,
	desk,
	page,
	show,
	...
	const hidden = [
		...
		{
			id: crypto.randomUUID(),
			name: "Visual-only reserved-ID scenery",
			fixtureNumber: null,
			address: null,
	...
	const selected = await api.liveAction<ProgrammingSelectionActionOutcome>(
		{
			...
			request: {
				...
				action: "gesture",
				source: { type: "fixture", fixture_id: hidden[1].id },
				remove: false,
			},
		},
		selectionRequest,
	);
	expect(selected.payload?.selection.selected).toEqual([hidden[1].id]);
	await expect(fixturePane).toContainText("1 selected");
	await expect(fixtureSheet.locator(".ui-data-table-row.selected")).toHaveCount(
		0,
	);
...
test("FIXTURE-SHEET-002-020 @ui › a Dynamic changes DMX while the Fixture Sheet base stays stable", async ({
	api,
	bench,
	desk,
	page,
	...
	await page.getByRole("button", { name: "DMX output", exact: true }).click();
	...
	expect(new Set(outputValues).size).toBeGreaterThanOrEqual(2);

	await page.getByRole("button", { name: "Programming", exact: true }).click();
	await expect(page.locator(".fixture-window")).toBeVisible();
	await expect(fixtureRow(page, show.fixtureIds[0])).toContainText("50%");
	await expect(
		fixtureRow(page, show.fixtureIds[0]).locator(".fixture-dynamic-stack"),
	).toContainText("77");
