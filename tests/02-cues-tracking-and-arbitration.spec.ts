...
test.describe("docs/testing/02-cues-tracking-and-arbitration.md", () => {
  pairedScenario<{ completed: boolean }>({
    ...
    api: async ({ api, bench }, state) => {
      for (const transfer of cueTransfers) {
        ...
        await api.command("programmer.execute", {
          value: `${transfer.operation} ${transfer.mode.toUpperCase()} SET 1 CUE 2 AT SET 2 CUE 2`,
        });
  ...
  pairedScenario<{ starting: number[]; completed: boolean }>({
    ...
    api: async ({ api, bench }, state) => {
      ...
      await api.command("programmer.execute", { value: "SPD GRP 1 AT + 5" });
      await api.command("programmer.execute", { value: "SPD GRP 1 AT - 5" });
      expect((await speedConfiguration(api))[0]).toBe(120);
      await api.command("programmer.execute", { value: "SPD GRP 3 AT 90" });
      await api.command("programmer.execute", { value: "SPD GRP 1 AT SPD GRP 3" });
