...
test.describe("docs/plans/Done/22-client-history-and-removal.DONE.md", () => {
	...
	test("CLIENT-001 @ui @restart › client presence, history, removal, and clean re-registration remain desk-local", async ({
		api,
		bench,
		desk,
		page,
		...
		await expect(removeClient(api, historical.desk.id)).rejects.toThrow(
			/409.*actively connected/,
		);
		await expect(removeClient(api, observer.desk.id)).rejects.toThrow(
			/409.*current client/,
		);
		...
		expect(historical).toMatchObject({
			connected: false,
			last_connected_at: disconnectedAt,
		...
		await expect(
			chooser.getByRole("heading", {
				name: "Disconnected clients",
				exact: true,
			}),
		).toBeVisible();
		const historicalRow = chooser
			.getByRole("article")
		...
		const confirmation = page.getByRole("alertdialog", {
			name: new RegExp(`Remove client ${escapeRegex(historical.name)}`),
		});
		...
		const reconnected = await createSession(
			bench.baseUrl,
			clientB,
			historicalDesk.id,
		);
		...
		clients = await clientSummaries(api);
		expect(
			clients.filter((client) => client.client_id === clientB),
		).toHaveLength(1);
		expect(
		...
		await reconnectedApi.request(
			"DELETE",
			`/api/v2/sessions/${reconnected.session_id}`,
		);
		...
		await expect(
			removeClient(api, historicalDesk.id, removalRequestId),
		...
		expect(
			clients.some((client) => client.client_id === api.session!.client_id),
		).toBe(true);
		expect(
			(await api.request<any>("GET", "/api/v2/bootstrap", undefined, false))
				.users,
		...
		expect(
			afterReRegistration.filter((client) => client.client_id === clientB),
		).toHaveLength(1);
		expect(
			afterReRegistration.find((client) => client.client_id === clientB)?.desk
				.id,
		).toBe(fresh.desk.id);
	});
});
