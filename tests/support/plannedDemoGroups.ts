...
export function plannedDemoGroupSpecs(): PlannedDemoGroupSpec[] {
  ...
  for (const [family, label] of [
    ["profile", "Profile"], ["wash", "Wash"], ["led", "LED"],
  ] as const) {
    specs.push(
      group(number++, `${label} All`, plannedDemoFamilyNumbers(family)),
      group(number++, `${label} Stage`, plannedDemoFamilyNumbers(family, "stage")),
      group(number++, `${label} Audience`, plannedDemoFamilyNumbers(family, "audience")),
  ...
  for (const [family, label] of showFamilies) {
    specs.push(
      ...
      group(number++, `Aux Show ${label}`, plannedDemoFamilyNumbers(family, "aux")),
    );
  }
  const show = showFamilies.flatMap(([family]) => showNumbers(family));
  const auxShow = showFamilies.flatMap(([family]) => plannedDemoFamilyNumbers(family, "aux"));
  specs.push(group(number++, "Show", show), group(number++, "Aux Show", auxShow));
  for (const [family, label] of showFamilies) {
    const members = showNumbers(family);
    specs.push(
      group(number++, `Show ${label} Odd`, members.filter((_, index) => index % 2 === 0)),
      group(number++, `Show ${label} Even`, members.filter((_, index) => index % 2 === 1)),
    );
  }
  specs.push(
...
export async function installPlannedDemoGroups(
  ...
  const targetsByNumber = new Map(fixtures.flatMap((fixture) =>
...
function group(id: number, name: string, fixtureNumbers: readonly number[]): PlannedDemoGroupSpec {
