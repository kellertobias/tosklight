...
import { ensurePlannedDemoFixtureLibrary } from "./plannedDemoState";
...
export async function installPlannedDemoPatch(
  ...
  await api.request(
    ...
    "/api/v2/patch/fixtures",
    {
      ...
      fixtures,
      remove_fixture_ids: before.fixtures.map((fixture: any) => fixture.fixture_id),
      placements: [],
    },
    true,
    before.revision,
  ...
  const physicalInstances = after.fixtures.reduce(
    (count: number, fixture: any) => count + 1 + (fixture.multipatch?.length ?? 0),
    0,
  );
  if (after.fixtures.length !== PLANNED_DEMO_CONTROL_FIXTURES)
    throw new Error(`Plan 76 patch has ${after.fixtures.length} controls; expected 262`);
  if (physicalInstances !== PLANNED_DEMO_PHYSICAL_INSTANCES)
    throw new Error(`Plan 76 patch has ${physicalInstances} physical instances; expected 301`);
  return { ...built, fixtures: after.fixtures, fixtureRecords: after.fixtures.length, physicalInstances };
}
...
export function createPlannedDemoPatchInputs(
  ...
  const fixtures = PLANNED_DEMO_FIXTURES.map((entry) => {
    ...
    const mode = profile.modes.find((candidate) => candidate.name === entry.profile.mode);
    if (!mode) throw new Error(`Missing demo mode ${entry.profile.name} / ${entry.profile.mode}`);
    ...
    const multipatch = Array.from({ length: entry.multipatches }, (_, index) => {
      const instance = placement.multipatches[index];
      if (!instance) throw new Error(`${entry.name} is missing physical placement ${index + 2}`);
...
function resolveProfile(profiles: FixtureProfile[], entry: DemoFixtureManifestEntry) {
  const found = profiles.find((candidate) =>
    candidate.manufacturer === entry.profile.manufacturer &&
    candidate.name === entry.profile.name);
  if (!found) throw new Error(`Missing demo profile ${entry.profile.manufacturer} / ${entry.profile.name}`);
  return found;
}
...
function allocateSplits(
  ...
  return splits.map((split) => {
    if (split.footprint === 0) return { split: split.number, universe: null, address: null };
    if (cursor.address + split.footprint - 1 > 512) {
      cursor.universe++;
      cursor.address = 1;
    }
    const patch = { split: split.number, universe: cursor.universe, address: cursor.address };
...
function layerFor(entry: DemoFixtureManifestEntry, layers: Readonly<Record<string, string>>) {
  const name = entry.name;
  const preferred =
    name.startsWith("Back ") || (entry.location === "stage" && stageIndex(entry) < stageBackCount(entry))
      ? "Back Truss"
      : name.startsWith("Mid ") || entry.location === "stage"
        ? "Mid Truss"
        : name.startsWith("Front ") || name.startsWith("Fresnel") || name.startsWith("Static Profile")
          ? "Front Truss"
          : entry.location === "audience"
            ...
            : entry.location === "aux"
              ...
              : name.startsWith("House")
                ? "House Lights"
                : "Floor";
  return layers[preferred] ?? layers.Stage ?? Object.values(layers)[0] ?? "default";
}
...
function ordinaryLocation(entry: DemoFixtureManifestEntry): Point {
  ...
  if (entry.family === "profile" && entry.location === "stage")
    return trussLine(index < 16 ? index : index - 16, index < 16 ? 16 : 12, index < 16 ? 4 : 0);
  if (entry.family === "wash" && entry.location === "stage")
    return trussLine(index < 15 ? index : index - 15, index < 15 ? 15 : 11, index < 15 ? 4 : 0);
  ...
  if (entry.roles.includes("Sunstrips")) return { x: -1.5 + Math.floor(index / 2), y: 4.15, z: 1.7 + (index % 2) * 1.15 };
  if (entry.roles.includes("Blinders")) return { x: index ? 2 : -2, y: -3, z: 4 };
  if (entry.roles.includes("Hazers")) return { x: index ? 3.5 : -3.5, y: 3.5, z: 0.2 };
...
function ordinaryRotation(entry: DemoFixtureManifestEntry, origin: Point): Point {
  if (entry.roles.includes("Sunstrips")) return { x: 0, y: 90, z: 0 };
  if (entry.location === "audience") return aimAt(origin, { x: origin.x, y: origin.y, z: 0 });
...
function aclPlacement(entry: DemoFixtureManifestEntry) {
  ...
  const targets = number === 1 || number === 3
  ...
  const instances = positions.map((x, index) => {
    ...
    return { location, rotation: aimAt(location, { x: targets[index], y: 1, z: 0 }) };
  });
...
function housePlacement() {
  const instances = Array.from({ length: 12 }, (_, index) => {
    const location = { x: (index % 3 - 1) * 2.2, y: -5 - Math.floor(index / 3) * 1.2, z: 5 };
    return { location, rotation: aimAt(location, { x: location.x, y: location.y, z: 0 }) };
  });
  return { primary: instances[0], multipatches: instances.slice(1) };
}

function familyIndex(entry: DemoFixtureManifestEntry) {
  return PLANNED_DEMO_FIXTURES.filter((candidate) =>
    candidate.family === entry.family && candidate.location === entry.location).indexOf(entry);
}
...
function grid(index: number, columns: number, spacing: number, y: number, xOffset: number, z: number): Point {
  return { x: xOffset + (index % columns) * spacing, y: y - Math.floor(index / columns) * spacing, z };
}
...
function millimetres(point: Point) {
  return { x: Math.round(point.x * 1_000), y: Math.round(point.y * 1_000), z: Math.round(point.z * 1_000) };
}
