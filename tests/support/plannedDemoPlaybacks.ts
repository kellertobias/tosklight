...
const ACL_CUE_LIST_IDS = [
  CUE_LIST_IDS.acl1, CUE_LIST_IDS.acl2, CUE_LIST_IDS.acl3, CUE_LIST_IDS.acl4,
] as const;
...
export async function installPlannedDemoPlaybacks(
  ...
  const targetMap = new Map(fixtures.flatMap((fixture) =>
    fixture.fixture_number == null ? [] : [[fixture.fixture_number, targetIds(fixture)] as const]));
  const groups = new Map(plannedDemoGroupSpecs().map((group) => [
    group.name,
    group.fixtureNumbers.flatMap((number) => targetMap.get(number) ?? []),
  ]));
  const cuelists = [
    cueList(CUE_LIST_IDS.start, "Start", [stateCue(1, "Start", [
    ...
    {
      ...cueList(CUE_LIST_IDS.chase, "ACL Chase", [1, 2, 3, 4].map((active) =>
        stateCue(active, `ACL ${active}`, [1, 2, 3, 4].flatMap((number) =>
  ...
  const playbacks = [
    ...
    playback(11, "Start", { type: "cue_list", cue_list_id: CUE_LIST_IDS.start }),
    ...[1, 2, 3, 4].map((number) =>
      playback(11 + number, `ACL ${number}`, {
        type: "cue_list",
        cue_list_id: ACL_CUE_LIST_IDS[number - 1],
      })),
    playback(16, "Hazer", { type: "cue_list", cue_list_id: CUE_LIST_IDS.hazer }),
...
function playback(number: number, name: string, target: Record<string, unknown>) {
  ...
  return {
    ...
    name,
    target,
    buttons: group ? ["select", "flash", "select_dereferenced"] : ["go", "go_minus", "flash"],
    button_count: 3,
    fader: "master",
    has_fader: true,
    go_activates: true,
