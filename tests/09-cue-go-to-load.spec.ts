...
import { fixtureIdsByNumber, loadCanonicalCopy, pressCommand, putObject } from "./support/catalog";

test.describe("planned feature 09 — Cue Go To and Load", () => {
  test("SELECT is desk-local and the visible keypad drives Go To and Load", async ({ api, bench, desk, page }) => {
    ...
    await expect.poll(async () => runtime(api, 2)).toMatchObject({ current_cue_number: 3, normal_next_cue_number: null, effective_next_cue_number: 2, effective_next_is_loaded: true });
    await page.locator(".mode-toggle").click();
    ...
    await second.getByRole("button", { name: "GO", exact: true }).click();
    await expect.poll(async () => runtime(api, 2)).toMatchObject({ current_cue_number: 2, normal_next_cue_number: 3, effective_next_cue_number: 3, effective_next_is_loaded: false });
    expect((await bench.tick(0)).universes.find((universe: any) => universe.universe === 1)?.slots[0]).toBe(204);
  ...
  test("explicit addresses and errors are atomic while global output controls remain authoritative", async ({ api, bench }) => {
    ...
    await api.command("programmer.execute", { value: "CUE SET 1 CUE 3" });
    await api.command("programmer.execute", { value: "CUE CUE SET 1 . 2 CUE 2" });
    ...
    await expect(api.command("programmer.execute", { value: "CUE 99" })).rejects.toThrow(/cue does not exist/i);
    await expect(api.command("programmer.execute", { value: "CUE SET 99 CUE 1" })).rejects.toThrow(/playback 99 does not exist/i);
    ...
    await expect(otherDesk.command("programmer.execute", { value: "CUE 2" })).rejects.toThrow(/no playback is selected/i);
    ...
    await api.command("programmer.execute", { value: "CUE CUE SET 1 CUE 2" });
  ...
  test("OSC selection, Cue keys, and loaded-next feedback use the same desk runtime", async ({ api, bench }) => {
...
async function installTwinPlaybacks(api: ApiDriver): Promise<string> {
  ...
  await putObject(api, "cue_list", cueListId, {
    id: cueListId, name: "Twin Cuelist", priority: 0, mode: "sequence", looped: false,
    wrap_mode: "off", restart_mode: "first_cue", cues,
  });
  for (const [number, name] of [[1, "Twin A"], [2, "Twin B"]] as const) {
    await putObject(api, "playback", String(number), {
      number, name, target: { type: "cue_list", cue_list_id: cueListId },
      buttons: ["go_minus", "go", "flash"], fader: "master", go_activates: true,
      auto_off: false, xfade_millis: 0, color: "#20c997",
      flash_release: "release_all", protect_from_swap: false,
    });
  }
...
function cue(number: number, id: string, fixture: string, level: number) {
  return {
    id, number, name: `Cue ${number}`, fade_millis: 0, delay_millis: 0,
    trigger: { type: "manual" }, phasers: [], group_changes: [],
