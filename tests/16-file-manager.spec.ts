...
test.describe("docs/testing/09-file-manager-and-text-editor.md", () => {
  test("FILE-016 @api › confined file services authenticate, stream ranges, expose native capabilities, and resolve conflicts", async ({ api, bench, request }) => {
    const unauthenticated = await request.get(`${bench.baseUrl}/api/v2/files/roots`);
    expect(unauthenticated.status()).toBe(401);
    const authorization = { authorization: `Bearer ${api.session!.token}` };
    const rootsResponse = await request.get(`${bench.baseUrl}/api/v2/files/roots`, { headers: authorization });
    ...
    try {
      await api.request("POST", "/api/v2/files/shows/operations", { operation: "create_folder", sources: [], destination: "", name: workspace });
      await api.request("POST", "/api/v2/files/shows/operations", { operation: "create_folder", sources: [], destination: workspace, name: "Destination" });
      await api.request("POST", "/api/v2/files/shows/operations", { operation: "create_file", sources: [], destination: workspace, name: "range.txt" });
      await api.request("POST", "/api/v2/files/shows/operations", { operation: "create_file", sources: [], destination: workspace, name: ".hidden" });
      const document = await api.request<any>("GET", `/api/v2/files/shows/text?path=${encodeURIComponent(`${workspace}/range.txt`)}`);
      await api.request("PUT", "/api/v2/files/shows/text", { path: `${workspace}/range.txt`, text: "0123456789", revision: document.revision });

      const ordinary = await api.request<any>("GET", `/api/v2/files/shows/entries?path=${encodeURIComponent(workspace)}`);
      expect(ordinary.entries.map((entry: any) => entry.name)).not.toContain(".hidden");
      const withHidden = await api.request<any>("GET", `/api/v2/files/shows/entries?path=${encodeURIComponent(workspace)}&hidden=true`);
      expect(withHidden.entries.map((entry: any) => entry.name)).toContain(".hidden");

      const traversal = await request.get(`${bench.baseUrl}/api/v2/files/shows/entries?path=${encodeURIComponent("../")}`, { headers: authorization });
      expect(traversal.status()).toBe(400);
      const range = await request.get(`${bench.baseUrl}/api/v2/files/shows/content?path=${encodeURIComponent(`${workspace}/range.txt`)}`, { headers: { ...authorization, range: "bytes=2-5" } });
      ...
      const suffix = await request.get(`${bench.baseUrl}/api/v2/files/shows/content?path=${encodeURIComponent(`${workspace}/range.txt`)}`, { headers: { ...authorization, range: "bytes=-3" } });
      expect(await suffix.text()).toBe("789");

      const metadata = await api.request<any>("GET", `/api/v2/files/shows/metadata?path=${encodeURIComponent(`${workspace}/range.txt`)}`);
      ...
      if (metadata.note_supported) {
        await api.request("PUT", "/api/v2/files/shows/notes", { path: `${workspace}/range.txt`, note: "Operator metadata" });
        const note = await api.request<any>("GET", `/api/v2/files/shows/notes?path=${encodeURIComponent(`${workspace}/range.txt`)}`);
        expect(note).toEqual(expect.objectContaining({ supported: true, note: "Operator metadata" }));
        const names = (await api.request<any>("GET", `/api/v2/files/shows/entries?path=${encodeURIComponent(workspace)}&hidden=true`)).entries.map((entry: any) => entry.name as string);
        expect(names.some((name: string) => /tosklight.*note|\.note/i.test(name))).toBe(false);
      }

      await api.request("POST", "/api/v2/files/shows/operations", { operation: "copy", sources: [`${workspace}/range.txt`], destination: `${workspace}/Destination` });
      const conflict = await request.post(`${bench.baseUrl}/api/v2/files/shows/operations`, { headers: { ...authorization, "content-type": "application/json" }, data: { request_id: crypto.randomUUID(), operation: "copy", sources: [`${workspace}/range.txt`], destination: `${workspace}/Destination` } });
      expect(conflict.status()).toBe(409);
      const keepBoth = await api.request<any>("POST", "/api/v2/files/shows/operations", { operation: "copy", sources: [`${workspace}/range.txt`], destination: `${workspace}/Destination`, conflict: "keep_both", apply_to_all: true });
      ...
      const claimed = await api.request<any>("POST", "/api/v2/files/input-context", { instance_id: "acceptance-file-manager", action: "copy", origin: "pending" });
      ...
      const competingClaim = await request.post(`${bench.baseUrl}/api/v2/files/input-context/claim`, { headers: { ...authorization, "content-type": "application/json" }, data: { request_id: crypto.randomUUID(), instance_id: "another-pane", action: "copy", origin: "toolbar" } });
      ...
      try {
        ...
        await expect.poll(() => api.request<any>("GET", "/api/v2/files/input-context"))
        ...
        await expect.poll(() => api.request("GET", "/api/v2/files/input-context")).toBeNull();
      } finally {
        await hardware.close();
      }
    } finally {
      await api.request("POST", "/api/v2/files/shows/operations", { operation: "delete", sources: [workspace] }).catch(() => undefined);
    }
  });

  test("FILE-016 @ui › three-column browsing and file operations share one visible state machine", async ({ api, bench, desk, page }) => {
    const showsRoot = (await api.request<any[]>("GET", "/api/v2/files/roots")).find((root) => root.id === "shows");
    const workspace = `file-manager-${crypto.randomUUID()}`;
    const destination = `${workspace}/Destination`;
    await api.request("POST", "/api/v2/files/shows/operations", { operation: "create_folder", sources: [], destination: "", name: workspace });
    await api.request("POST", "/api/v2/files/shows/operations", { operation: "create_folder", sources: [], destination: workspace, name: "Destination" });
    await api.request("POST", "/api/v2/files/shows/operations", { operation: "create_file", sources: [], destination: workspace, name: "alpha.txt" });
    await api.request("POST", "/api/v2/files/shows/operations", { operation: "create_file", sources: [], destination: workspace, name: ".operator-note" });
    ...
    await expect.poll(async () => (await api.request<any>("GET", `/api/v2/files/shows/entries?path=${encodeURIComponent(destination)}`)).entries.map((entry: any) => entry.name)).toContain("alpha.txt");
    ...
    await api.request("POST", "/api/v2/files/shows/operations", { operation: "delete", sources: [workspace] });
  });
