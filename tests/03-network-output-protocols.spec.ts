...
test.describe("docs/testing/03-network-output-protocols.md", () => {
	pairedScenario<{
		...
		arrange: async ({ api, bench }, surface) => {
			const show = await loadCanonicalCopy(api, bench, `dmx-001-${surface}`);
			return {
				showId: show.id,
		...
		api: async ({ api, bench }, state) => {
			...
			for (const percent of state.values) {
				await setNormalizedFixtureValue(
					api,
					state.showId,
					fixture,
					"intensity",
				...
				state.observations.push(
					await captureConversion(api, bench, fixture, percent),
				);
			}
		},
		assert: async (_context, state) => {
			...
			expect(
				state.observations.map(({ percent, logical, artnet, sacn }) => ({
					percent,
					logical,
					artnet,
					sacn,
			...
			for (const [index, observation] of state.observations.entries()) {
				expect(observation.normalized).toBeCloseTo(
	...
	pairedScenario<{}>({
		...
		assert: async ({ bench }) => {
			...
			expect(packets.map((packet: any) => packet.protocol)).toEqual([
				"artnet",
				"artnet",
				"artnet",
			]);
			...
			expect(
				packets.every(
					(packet: any) =>
						packet.slots.length === 512 && packet.slots[0] === 64,
				),
			).toBe(true);
			expect(
				packets.every(
					(packet: any) =>
						packet.artnet?.id === "Art-Net\0" &&
						packet.artnet.opcode === 0x5000 &&
						packet.artnet.protocolVersion === 14 &&
						packet.artnet.physical === 0 &&
						packet.artnet.payloadLength === 512,
				),
			).toBe(true);
		},
	});
	...
	pairedScenario<{}>({
		...
		assert: async ({ api, bench }) => {
			...
			expect(packets.map((packet: any) => packet.universe)).toEqual([
				101, 101, 101,
			]);
			expect(packets.map((packet: any) => packet.priority)).toEqual([
				100, 100, 100,
			]);
			expect(packets.map((packet: any) => packet.sequence)).toEqual([1, 2, 3]);
			expect(
				packets.every(
					(packet: any) =>
						packet.slots.length === 512 && packet.slots[0] === 128,
				),
			).toBe(true);
			expect(new Set(packets.map((packet: any) => packet.sacn?.cid)).size).toBe(
				1,
			);
			expect(
				new Set(packets.map((packet: any) => packet.sacn?.sourceName)).size,
			).toBe(1);
			expect(packets[0].sacn?.sourceName).toBeTruthy();
			expect(
				packets.every(
					(packet: any) =>
						packet.sacn?.preambleSize === 0x10 &&
						...
						packet.sacn.startCode === 0,
				),
			).toBe(true);
			const route = (await objects(api, "route")).find(
				(entry) => entry.body.protocol === "sacn",
			)!;
				...
				.poll(
					() =>
						bench.sacn
							...
							.filter(
								(packet) =>
									packet.protocol === "sacn" && packet.universe === 101,
							).length,
				)
				.toBe(3);
			const terminated = bench.sacn
				.packetsAfter(mark)
				.filter(
	...
	pairedScenario<{
		extraArt: DmxReceiver;
		disabledSacn: DmxReceiver;
		showId: string;
	}>({
		id: "DMX-004",
		title:
		...
		surfaces: ["api"],
		arrange: async ({ api, bench }, surface) => {
			const show = await loadCanonicalCopy(api, bench, `dmx-004-${surface}`);
			...
			await putObject(
				...
				"route",
				"artnet-11",
				route("art_net", 1, 11, extraArt.port, true),
			);
			await putObject(
				api,
				"route",
				"sacn-102",
				route("sacn", 1, 102, disabledSacn.port, false),
			);
			return { extraArt, disabledSacn, showId: show.id };
		},
		api: async ({ api }, state) => {
			...
			await batchProgrammerValues(api, {
				...
				mutations: (
					...
					normalizedFixtureMutation(fixtures[number], "intensity", value),
				),
			});
		},
	...
	pairedScenario<PatchConflictState>({
		id: "DMX-005",
		title:
			"a conflicting patch edit preserves both previous addresses atomically",
		surfaces: ["api"],
		arrange: async ({ api, bench }, surface) => {
		...
		assert: async ({ api }, state) => {
			...
			const first = snapshot.fixtures.find(
				(fixture) => fixture.fixture_id === state.firstId,
			);
			const second = snapshot.fixtures.find(
				(fixture) => fixture.fixture_id === state.secondId,
			);
	...
	pairedScenario<SixteenBitState>({
		id: "DMX-006",
		title:
			"16-bit metadata and virtual heads encode deterministic component bytes",
		surfaces: ["api"],
		arrange: async ({ api, bench }, surface) => {
			...
			return {
				...(await installSixteenBitMatrix(api, bench)),
				showId: show.id,
			};
		},
		api: async ({ api }, state) => {
		...
		assert: async ({ api, bench }, state) => {
			await expectFixtureBytes(
				...
				state.fixtures.msb.address,
				[128, 0],
				3_000,
			);

			const defaultEncoded = encodeSixteenBit(
				state.fixtures.defaulted.defaultValue,
				false,
			);
			await expectFixtureBytes(
				bench,
			...
			await releaseProgrammerFixtureValue(api, {
				surface: "api",
				showId: state.showId,
				fixtureId: state.fixtures.defaulted.id,
				attribute: "intensity",
			});
			await expectFixtureBytes(
				bench,
			...
			await expectSunstripVirtualDimmers(
				api,
				bench,
				state.showId,
				state.sunstripDestinationUniverse,
			);
		},
	});
	...
	pairedScenario<{
		...
		surfaces: ["api"],
		arrange: async ({ api, bench }, surface) => {
			const show = await loadCanonicalCopy(api, bench, `dmx-007-${surface}`);
		...
		api: async ({ api }, state) => {
			const fixture = (await fixtureIdsByNumber(api))[1];
			const outcome = await setNormalizedFixtureValue(
				api,
				state.showId,
				fixture,
				"intensity",
		...
		assert: async ({ api, bench }, state, surface) => {
			...
			if (surface === "api") expect(state.valuesEventSequence).not.toBeNull();
				...
				await expect
					.poll(async () =>
						(await api.request<any[]>("GET", "/api/v2/audit?after=0")).some(
							(event) => event.kind === "command_applied",
						),
					)
					.toBe(true);
			const auditBefore = await api.request<any[]>(
				"GET",
				"/api/v2/audit?after=0",
			);
			const auditRevision = Math.max(
				0,
				...auditBefore.map((event) => event.revision),
			);
			...
			await bench.tick(3_000);
			expect(
				(await bench.artnet.nextAfter(healthyMark, "artnet", 1)).slots[0],
			).toBe(64);
			await new Promise((resolve) => setTimeout(resolve, 75));
			...
			expect(routeSendErrors(after, state.destination)).toBe(
				failingErrorsBefore + 1,
			);
			expect(routeSendErrors(after, healthyDestination)).toBe(
				healthyErrorsBefore,
			);
			expect(
				await api.request<any[]>("GET", `/api/v2/audit?after=${auditRevision}`),
			).toEqual([]);

			await api.request(
				"POST",
				"/api/v2/test/output/failure",
				{ destination: state.destination, enabled: false },
				false,
			);
			const recoveryMark = state.failing.mark();
			await bench.tick(0);
			const recovered = await state.failing.nextAfter(
				recoveryMark,
				"artnet",
				11,
			);
			...
			const recoveredDiagnostics = await api.request<any>(
				"GET",
				"/api/v2/diagnostics",
			);
			expect(routeSendErrors(recoveredDiagnostics, state.destination)).toBe(
				failingErrorsBefore + 1,
			);
	...
	pairedScenario<MinimumRouteState>({
		id: "DMX-008",
		title:
			"minimum universe size sends idle zeros, includes patched defaults, and disables without deletion",
		surfaces: ["api"],
		...
		api: async ({ api, bench }, state) => {
			await putObject(
				...
				"route",
				"dmx-008-artnet",
				route("art_net", 32, 32, state.receiver.port, true, 128),
			);
			await putObject(
				api,
				"route",
				"dmx-008-sacn",
				route("sacn", 32, 132, state.receiver.port, true, 128),
			);
		...
		assert: async ({ api }, state) => {
			...
			expect(
				routes.find(
					(entry) =>
						entry.body.logical_universe === 32 &&
						entry.body.protocol === "art_net",
				)?.body,
	...
	pairedScenario<DeliveryRouteState>({
		id: "DMX-009",
		title:
			"protocol-correct delivery modes persist and resolve to the actual socket destinations",
		surfaces: ["api"],
		arrange: async ({ api, bench }, surface) => {
			const show = await loadCanonicalCopy(api, bench, `dmx-009-${surface}`);
			return { receiver: await DmxReceiver.bind(), showId: show.id };
		},
		api: async ({ api }, state) => {
			await putObject(
				api,
				"route",
				"dmx-009-art-broadcast",
				deliveryRoute("art_net", 201, "broadcast"),
			);
			await putObject(
				api,
				"route",
				"dmx-009-art-unicast",
				deliveryRoute("art_net", 202, "unicast", state.receiver.port),
			);
			await putObject(
				api,
				"route",
				"dmx-009-sacn-multicast",
				deliveryRoute("sacn", 301, "multicast"),
			);
			await putObject(
				api,
				"route",
				"dmx-009-sacn-unicast",
				deliveryRoute("sacn", 302, "unicast", state.receiver.port),
			);
			const fixture = (await fixtureIdsByNumber(api))[1];
			await setNormalizedFixtureValue(
				api,
		...
		assert: async ({ api, bench }, state) => {
			const stored = (await objects<any>(api, "route"))
				.filter((entry) =>
					[201, 202, 301, 302].includes(entry.body.destination_universe),
				)
				.sort(
					(left, right) =>
						left.body.destination_universe - right.body.destination_universe,
				);
			expect(
				stored.map((entry) => [
					entry.body.protocol,
					entry.body.delivery_mode,
			...
			expect(diagnostics.output_routes).toEqual(
				expect.arrayContaining([
					...
					expect.objectContaining({
						...
						universe: 202,
						delivery_mode: "unicast",
						destination: `127.0.0.1:${state.receiver.port}`,
						enabled: true,
					}),
					expect.objectContaining({
						protocol: "sacn",
...
function routeSendErrors(diagnostics: any, destination: string): number {
	return (
		diagnostics.route_send_errors?.find(
			(entry: any) => entry.destination === destination,
		)?.errors ?? 0
	);
}
...
async function captureConversion(
	...
	const fixture = (await objects<any>(api, "patched_fixture")).find(
		(entry) => entry.body.fixture_id === fixtureId,
	)!;
	...
	return {
		...
		logical: tick.universes.find(
			(entry: any) => entry.universe === fixture.body.universe,
		)!.slots[offset],
...
function deliveryRoute(
	protocol: "art_net" | "sacn",
	destinationUniverse: number,
	deliveryMode: "broadcast" | "multicast" | "unicast",
	port?: number,
) {
	return {
		protocol,
...
async function exerciseMinimumRoute(
	api: ApiDriver,
	bench: any,
	state: MinimumRouteState,
	disableArtNet: () => Promise<void>,
) {
	let mark = state.receiver.mark();
	await bench.tick(0);
	state.emptyArtSlots = Array.from(
		(await state.receiver.nextAfter(mark, "artnet", 32)).slots,
	);
	state.emptySacnSlots = Array.from(
		(await state.receiver.nextAfter(mark, "sacn", 132)).slots,
	);

	const source = (await objects<any>(api, "patched_fixture")).find(
		(entry) => entry.body.fixture_number === 1,
	)!;
					...
					parameters: [
						{
							...firstParameter,
							default: 0.4,
							components: [{ ...firstParameter.components[0], offset: 0 }],
						},
						{
							...firstParameter,
							attribute: "minimum_size_zero",
							components: [{ ...firstParameter.components[0], offset: 1 }],
							default: undefined,
						},
					],
				},
	...
	await bench.tick(0);
	state.patchedArtSlots = Array.from(
		(await state.receiver.nextAfter(mark, "artnet", 32)).slots,
	);
	state.patchedSacnSlots = Array.from(
		(await state.receiver.nextAfter(mark, "sacn", 132)).slots,
	);
	...
	await new Promise((resolve) => setTimeout(resolve, 75));
	state.artDisabled = !state.receiver
		.packetsAfter(mark)
		.some((packet) => packet.protocol === "artnet" && packet.universe === 32);
}

async function installPatchConflict(
	api: ApiDriver,
): Promise<PatchConflictState> {
	...
	const first = outcome.fixtures.find(
		(fixture) => fixture.fixture_id === firstId,
	);
	const second = outcome.fixtures.find(
		(fixture) => fixture.fixture_id === secondId,
	);
	if (!first || !second)
		throw new Error("Patch did not return both conflict fixtures");
	return {
		firstId,
		secondId,
...
async function installSixteenBitMatrix(
	api: any,
	bench: any,
): Promise<SixteenBitState> {
	...
	await putObject(
		api,
		"route",
		"dmx-006-sunstrip-route",
		route("art_net", 3, sunstripDestinationUniverse, bench.artnet.port, true),
	);
...
function sixteenBitPatchedFixture(options: {
	...
	return {
		...
		split_patches: [
			{ split: 1, universe: options.universe, address: options.address },
		],
		logical_heads: [],
		multipatch: [],
		definition: {
...
async function expectFixtureBytes(
	...
	const logical = tick.universes
		.find((entry: any) => entry.universe === logicalUniverse)!
		.slots.slice(address - 1, address - 1 + expected.length);
	const wire = Array.from(
		(
			await bench.artnet.nextAfter(mark, "artnet", destinationUniverse)
		).slots.slice(address - 1, address - 1 + expected.length),
	);
...
async function setNormalizedFixtureValue(
	api: ApiDriver,
	showId: string,
	fixtureId: string,
	attribute: string,
...
function normalizedFixtureMutation(
	fixtureId: string,
	attribute: string,
	value: number,
...
async function expectSunstripVirtualDimmers(
	api: ApiDriver,
	bench: any,
	showId: string,
	destinationUniverse: number,
): Promise<void> {
	const sunstrip = (await objects<any>(api, "patched_fixture")).find(
		(entry) => entry.body.fixture_number === 501,
	)!;
	const heads = [...sunstrip.body.logical_heads].sort(
		(left: any, right: any) => left.head_index - right.head_index,
	);
	expect(heads).toHaveLength(10);
	await batchProgrammerValues(api, {
		surface: "api",
		showId,
		mutations: heads.flatMap((head: any, index: number) => [
			normalizedFixtureMutation(head.fixture_id, "color.red", 1),
			normalizedFixtureMutation(head.fixture_id, "intensity", (index + 1) / 10),
		]),
	});
	const mark = bench.artnet.mark();
	const tick = await bench.tick(3_000);
	const logical = tick.universes.find(
		(entry: any) => entry.universe === 3,
	)!.slots;
	const wire = (
		await bench.artnet.nextAfter(mark, "artnet", destinationUniverse)
	).slots;
	...
	for (let index = 0; index < 10; index += 1) {
		...
		expect(
			Array.from(wire.slice(start + index * 3, start + index * 3 + 3)),
