...
test.describe("docs/testing/00-generate-show-files.md", () => {
  pairedScenario<Show000State>({
    ...
    ui: async ({ api, desk, page }, state) => {
      ...
      const groupName = properties.getByLabel("Group name");
      await groupName.fill("Copy Center Spot");
      await groupName.blur();
      await expect.poll(async () => (await groupBody(api, state.compactCopy!.id, "4")).name).toBe("Copy Center Spot");
      await properties.getByRole("button", { name: /#718596/ }).click();
      await page.getByRole("option", { name: "Use color #1bd6ec" }).click();
      await expect.poll(async () => (await groupBody(api, state.compactCopy!.id, "4")).color).toBe("#1bd6ec");
      await properties.getByRole("button", { name: /Choose icon/ }).click();
      await page.getByRole("button", { name: "Use ★" }).click();
      await expect.poll(async () => (await groupBody(api, state.compactCopy!.id, "4")).icon).toBe("★");
      await properties.getByRole("button", { name: "Close settings" }).click();
    ...
    assert: async ({ api }, state) => {
      ...
      await openShow(api, state.compactRevisionCopy!.id);
  ...
  test("SHOW-000 @supplemental › extended visible Group-properties and alternate-gesture workflow", async ({ api, desk, page }) => {
    ...
    await groupName.fill("Copy Center Spot");
    await groupName.blur();
    await expect.poll(async () => (await groupBody(api, compactCopy.id, "4")).name).toBe("Copy Center Spot");
    await properties.getByRole("button", { name: /#718596/ }).click();
    await page.getByRole("option", { name: "Use color #1bd6ec" }).click();
    await expect.poll(async () => (await groupBody(api, compactCopy.id, "4")).color).toBe("#1bd6ec");
    await properties.getByRole("button", { name: /Choose icon/ }).click();
    await page.getByRole("button", { name: "Use ★" }).click();
    await expect.poll(async () => (await groupBody(api, compactCopy.id, "4")).icon).toBe("★");
    await properties.getByRole("button", { name: "Close settings" }).click();
    ...
    const compactRevisionCopy = await activeShowEntry(api);
    await openGroups(page);
    await expect(groupTile(page, "Copy Center Spot")).toBeVisible();
    await expect.poll(async () => groupBody(api, compactRevisionCopy.id, "4")).toMatchObject({
      name: "Copy Center Spot",
      color: "#1bd6ec",
      icon: "★",
...
async function objects(api: ApiDriver, showId: string, kind: string): Promise<VersionedObject[]> {
  const result = await api.showObjects(showId, kind);
