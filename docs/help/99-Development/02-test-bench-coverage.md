...
- **Actions:** Open Show Patch on the empty show; visibly add all three four-segment trusses through the Touch UI, keeping the first interaction at normal recording pace before compacting the repeated action gaps; fast-forward the stage deck, curtains, back and side railings, and vertical pipes through the API; visibly patch and place eight front Fresnels through the UI; visibly add one eight-lamp ACL control with seven multi-patches and one moving Profile; then fast-forward the remaining canonical lighting patch. Configure one output route visibly and the remaining routes behind a labelled fast-forward. Build the first Groups through the pool and hardware keypad, assign Group Masters by touch and command line, record the first Color, Position, and Beam presets by hand, create the Profile Circle and PWM Dynamics and assign them to a new Virtual Playbacks desktop, then record the first ACL Cuelist through the keypad before completing the remaining canonical objects. Continue through Fixture Sheet, control actions, busking, Preload, and the ACL chaser on production operator surfaces.
- **Oracle:** The maintained video starts only after the empty show is active; the complete show contains eight layers, 262 controllable lighting fixtures, 33 visual-only Venue records, 295 total patch records, 343 physical Stage instances, 3,783 occupied DMX slots, three enabled output routes, 38 Groups, 30 presets, 30 Dynamics, seven Cuelists, 13 Playbacks, and non-zero authoritative DMX output.
