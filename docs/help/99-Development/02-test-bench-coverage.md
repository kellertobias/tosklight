...
- **Surface:** Lightning Desk keypad/command line and the hardware-connected encoder value modal, plus engine interpolation and real output.
- **Actions:** Through visible desk controls, enter `FIXTURE 1 THRU 5 AT 20 THRU 50`, complete a fixture selection and then enter `AT 0 THRU 50`, and enter `0 THRU 50` in the Dimmer encoder value modal. Also cover uniform `0`, descending `100 THRU 0`, and multi-point `100 THRU 0 THRU 100` intensity commands, live `GROUP` references, and dereferenced `DEGRP`/`GROUP GROUP` fixture captures; reserve empty cases for color and position spreads.
...
- **Actions:** Add five patch layers; import the exact shipped fixture packages; visibly patch and place the first fixture in every layer; visibly add, address, and place the first lamp multi-patch; then use one brief labelled fast-forward card in the bottom narrative area, immediately above the chapter progress, while show generation completes the repeated venue, truss, lamp, multi-patch, and utility rig and prepares the named pool/playback targets. After that boundary, add all three output routes through their editor and use operator UI, keypad, or command-line actions only: enable Fixture Sheet group shortcuts; touch-select the front fixtures and record Group 9 into its pool tile; record the Wash and odd/even reference groups through mixed command-line and pool-tile paths; apply empty-selection **Lamps On**; record one Color preset through the UI and another through the command line; busk the prepared looks; then prepare playback and position changes in Preload with a four-second programmer fade and commit them before learning the chaser speed.
- **Oracle:** Exact active show name; five named layers; 65 persisted logical fixtures and 113 physical instances after expanding multi-patches; one logical Stage element with 15 physical multi-patches; three enabled output routes; named and ordered Group objects; empty-selection programmer lamp/intensity values; preset, Cuelist, playback, page, wrap, chaser, and Speed Group data; enabled playback runtime; pending then committed Preload state; and non-zero authoritative DMX output.
...
| CUELIST-LAYOUT-001 | Cuelist and Cue Settings layout | full-inner-width Cuelist Settings sidebar replacement, inline selected-Cue editor, always-visible selection-only Cue table, object-level field ownership, dirty Save/Discard/Stay decision, and narrow software/hardware-connected reachability | production element bounds and clipping, visible table/selection state, component save/discard mutation counts, and persisted object revisions |
