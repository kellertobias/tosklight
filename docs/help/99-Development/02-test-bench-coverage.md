...
| --- | --- | --- | --- |
| SHOW-000–005 | Show files and recovery | Save As copy isolation, save/restart/reopen, atomic replacement recovery, invalid-active-show recovery, stable legacy migration, and named-revision copy/overwrite isolation | independent files, provenance, hashes/revisions, restarted server, preserved backups and corrupt files |
...
| UPDATE-001 | Update | four Cue modes and authoritative tracked sources, exact eligibility, Preset/Group existing-versus-new semantics, touch/default/menu flows, current/explicit page context, mutually exclusive gestures, and atomic revisions | paired stored-object result, previews, revision history, programmer retention, same-desk feedback |
| HIGHLIGHT-001 | Highlight and Step Through | ordered capture, multi-selection/step/no-wrap, programmer isolation, fixture look overrides, output reveal, OSC dedupe, ownership, Blind/Preview, and master/blackout/safe-value behavior | paired Highlight/programmer state, resolved raw output, OSC feedback, visible controls |
| HIGHLIGHT-002 | Highlight surface and lifecycle isolation | Fixtures/Stage/Group capture paths, Preset exclusion, same-user reconnect retention, and show-load clearing | paired authenticated API and production Fixtures, Stage, Groups, Presets, reconnect, and reload |
| FIXTURE-001 | Fixture profiles | desk-wide atomic revisions, editor/lookup/reorder, exact channels/functions/actions/color/geometry, independent split patching, portable snapshots, GDTF retention, and v1 migration/recovery | paired profile/history, raw DMX, patch ranges, Stage geometry, startup artifacts and warnings |
| MATTER-001 | Matter playback bridge | desk-persistent enablement, global page/slot endpoint stability, omissions, OnOff/Level writes, bidirectional tracking, pairing/identity, transport lifecycle, and truthful failures | paired configuration/status, adapter values, production socket smoke and subscription state |
