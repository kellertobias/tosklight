...
| Programmer and playback | `/programmer/set`, `/programmers`, `/playbacks`, `/playback-pool/{number}/{action}` |
...
Connect to `/api/v1/events` with the authenticated token subprotocol. WebSocket events carry live changes that would be inefficient or ambiguous to poll, including show revisions, authoritative desk/programmer state, and typed control feedback. REST establishes or changes state; WebSocket confirms and follows the resulting live state.
