...
NEXT from the last valid item wraps to the first; PREV from the first wraps to the last. After ALL, NEXT starts again at the first item and PREV starts at the last. If there is no remembered source, ALL leaves an already-complete selection unchanged. PREV and NEXT remain available whenever the remembered source resolves to at least one valid item. OSC feedback reports complete-versus-step mode, the step index and total, and the active fixture or head identity without requiring a separate on-desk status panel.
