# The Programmer

The programmer is the operator's temporary working area. These topics cover selection, values, presets, groups, recording and editing cues, the command line, and how programmer output combines with playbacks.

![Fixture Sheet, programmer source cells, and attribute controls](../assets/screenshots/fixture-sheet-programmer.png)

Start with [Selecting and Setting Values](02-selecting-and-setting-values.md), including HIGH, Low Light, and PREV/NEXT/ALL focus through a frozen original set, then [Programming Cues](03-programming-cues.md). The [Command Line Reference](01-command-line.md) is the precise keypad, geometry, and keyboard-shortcut contract.
