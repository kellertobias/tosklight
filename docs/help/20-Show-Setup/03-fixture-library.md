...
- **Generic rare-capability references** — **Endless Pan Tilt** retains endless 16-bit axis representation on the canonical Pan and Tilt controls; **Beam Size and Edge** keeps Zoom independent from Softness; and **Media Positioning** provides independent media-layer X and Y axes. These are explicit transferable reference personalities: match their documented channel order to the device rather than treating them as manufacturer profiles.
...
- **ToskLight** product and Visualizer profiles — **Media Server Layer** carries the canonical transferable 34-slot layer personality and **Media Server Master** its 7-slot output personality. **Visualizer Camera** keeps the stable 17-slot X/Y/Z, Yaw/Pitch/Roll, and Zoom wire contract, while **Visualizer Laser** provides the packaged demo laser and its scan program. This manufacturer is reserved for implemented ToskLight-owned product fixtures; planned music-playback or further Visualizer fixtures do not appear until their capabilities exist.
