...
Location and rotation editors follow the same title-bar pattern: **Set** is directly beside Close, with no duplicate footer actions. Close acts as Cancel. If an axis changed, Close asks whether to discard the edits or keep editing; closing an unchanged editor returns immediately. Editors that provide a complete on-screen keyboard continue to use its Enter key and therefore do not add a redundant Set action.
