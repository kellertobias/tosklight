...
For a temporary change, hold `[REC]` to open **Record Settings** and enable **Cue only** before recording. The following Cue automatically restores each Cue-only address to its previous tracked value, or releases an address that had no earlier value. Turn **Cue only** off again for ordinary tracking records. The setting and generated restoration data survive a show refresh or reopen. Record Settings saves each change immediately; press title-bar **Done** to close it. Each option explains its effect below its label: **Record mode** chooses whether a record keeps the target's stored values (Merge) or replaces them (Overwrite), and **Merge into active Cue** describes adding the programmer values to the Cue a playback is on while keeping that Cue's other stored values.
