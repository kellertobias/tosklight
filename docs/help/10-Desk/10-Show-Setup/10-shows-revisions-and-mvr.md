...
The PreViz Rig Editor plans a rig against the same patch sheet the desk uses, and a rig planned there is a show the desk can run. When a PreViz Rig Editor with a document open is on the same network, **Load Show** offers **Load from Visualizer** naming that editor and the document it is holding. Pressing it copies the document across, imports it into the show library like any other portable show, and opens it.
...
Export preview reports fixture counts, embedded profiles, missing profiles, omissions, and warnings. The export includes fixture UUIDs, patch, transforms, and one GDTF file for every fixture type in the patch, so another application — including the GDTF Share MVR viewer — can open the archive. A profile imported from GDTF carries its retained original GDTF unchanged. Every other profile, such as a shipped library fixture or one authored on the desk, gets a GDTF that ToskLight generates from the profile: all of its modes with their exact channels, fine bytes, splits, defaults, Highlight values and named ranges, and one beam per logical head. A generated GDTF has no wheels, emitters or 3D model, and the export preview warns when the archive contains one. Fixtures are grouped into MVR layers named after the patch layers, in the order the patch lists them; a layer with no fixtures is exported too. Each fixture's position and rotation follow the MVR transform convention, so another application draws it facing the way the Stage does; the bracket angle is folded into the exported transform, because MVR has no separate bracket. A lamp that hangs in a frame turns its body about the frame's bracket bolts, as the CAD and the Visualizer show it, so the exported transform is the turned body's: its lens sits where it does in ToskLight and its beam points the same way. Another application therefore turns the hanging frame and clamp with the body, since MVR cannot keep them level. A fixture drawn from its own 3D model turns as a whole about its origin, and a lamp at bracket 0 exports exactly as before. An imported MVR fixture takes its whole orientation as its rotation, with its bracket level. An MVR this desk or the PreViz Rig Editor exported brings its fixtures back with their own location, rotation and bracket angle, so a lamp shines from the same place in the same direction after the round trip. If another application saves the archive without ToskLight's fixture information, its fixtures are read like any other MVR: the bracket stays in the rotation and the location is the turned body's. Visual-only Venue fixtures are exported as fixtures; there is no separate Stage scene-asset collection. Resolve warnings before relying on the archive as an interchange master.
