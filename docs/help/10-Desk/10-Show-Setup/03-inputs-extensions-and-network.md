...
On **Screens & playback**, the default-screen card can enable or disable all software keyboard shortcuts. Software shortcuts are also disabled automatically while hardware controls are connected so one physical action is not processed twice. The complete key map is in [Command Line Reference](../20-Programmer-and-Cues/01-command-line.md).
...
Matter advertises assigned **Cuelist**, **Dynamic**, and **Group Master** playbacks. Cuelists and Dynamics appear as dimmable lights; Group Masters appear as color lights. One-button and faderless assignments are included. Speed Groups, Programmer Fade, Cue Fade, Grand Master, Macro, and every other playback target are deliberately omitted, as are empty page/playback slots and pool entries that are not assigned to a page. **Desk Setup** lists every advertised playback with its Dimmable or Color kind so you can diagnose an object that is missing from the controller. A Color entry also says **Group color active** while that runtime contribution is coloring its Group.

Each endpoint is derived from the explicit global page and playback address, so changing the page visible on an operator screen never retargets a Matter light. Adding, removing, renaming, or changing the eligible kind of an assigned playback briefly restarts only Matter networking so controllers can rediscover the changed endpoint list; commissioned fabric data is retained.

Matter On/Off and Level Control writes use the same authoritative playback dispatcher as desk controls. A faderless assignment gains a Matter-only virtual master without adding a fader to its desk layout: a non-zero Cuelist or Dynamic Level always activates it, while Off or Level 0 always stops it, independently of the playback's authored activation settings. A later On restores the current non-zero level or starts at full. Group Level controls the normal Group Master.

A Group color write creates a runtime Group color contribution for every fixture in that Group. It is below live Programmer color and above Cue, playback, and Dynamic color; fixtures use their profile's normal RGB, CMY, hue/saturation, or color-wheel projection, and fixtures without color capability ignore it. Returning the Group Master to 0% does not release the color. It is released explicitly, removed if the Group is deleted, and cleared when a different show is installed; it is not saved into the show or desk file. Hue/saturation and color-temperature changes are synchronized both ways, and the controller retains its last-used color mode when a desk-side Group color changes.

Desk-side button actions, fader movement, tracking, automatic release, Group color changes, and other playback changes are read from the same authoritative runtime and mirrored back to subscribed Matter controllers. The standard Matter UDP port is exclusive; only one Matter service can own it on a host.
...
For address structure, authentication, subscriptions, and current-page versus explicit-page playback addressing, continue to [OSC Protocol](../../90-Protocols/01-osc.md).
