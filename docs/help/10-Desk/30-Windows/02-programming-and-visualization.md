...
Every patched **ToskLight Audio Player** appears in **Server** alongside the Media Servers; several patched players remain several entries. The Audio Player is an Internal fixture with no DMX address and no CITP endpoint, so it is listed as **Internal** instead of Online or Offline, and the CITP note is not shown for it. Because audio has no rendered frame, its master preview shows a music note, and its layer image shows a music note with the live volume percentage and the addressed folder/file number above it. Browsing a folder and file programs its Media Folder and Media File exactly as for a Media Server; a player patched before this change keeps its stored Audio Folder and Audio File attributes instead.
