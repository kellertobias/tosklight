...
To see a slot on the real output instead, turn on **Enable preview** below **Take over playback** in the dock while the Library is open. Pixel takes over playback of the output chosen in the dock, puts every layer out and brings the master full up. Each plain click on a media item then plays it on **Layer 1** at full level, looping, with every other layer out, so the output shows that one slot alone through its current display, pixel-map and effect settings; clicking another item switches at once. Shift- and Command/Ctrl-clicks only change the selection. **Enable preview** is also in the dock on the **Visualizers**, **Text**, **Effects** and **Models** pages, and stays on while you move between them and the Library. There it shows what the editor has selected, again whenever the selection changes: the selected visualizer or text slot on Layer 1; the selected Effects library slot on Layer 1's first effect bank at full strength, over the slot last previewed; or the last previewed slot mapped onto the selected 3D model. The effect or model is taken off again when the next page previews something else. Edits a page saves as you make them, such as a visualizer's settings or a text's words, show on the output straight away, so scale and look can be judged without leaving the editor. Turn **Enable preview** off, or open any other page, and Pixel puts every layer's slot, level, play mode, effect banks and 3D model and the master level back as they were when preview started, and hands playback back to the desk unless it was already taken over. Preview is never saved: it changes nothing in the library and is forgotten on reload.
