...
- the Fixture Sheet covers subdued remembered-base rows, a prominent current step, multi-head rows, all three **Included heads** modes, bare fixture IDs on master-only rows, unindented subhead-only rows, aligned name/type text, the profile-icon column between ID and Name, the separately selectable Patch address column, master-only contained state when subheads are hidden, HIGH-independent presentation, ALL restoration, and external-selection reset;
