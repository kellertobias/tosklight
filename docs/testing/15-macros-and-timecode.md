...
Open **Timecode Settings** and confirm it owns name, duration and frames, transport offset, auto-start, audio file, and marker CSV. Each change saves immediately with revision-safe ordering and there is no Save button. Confirm **Play**, **Pause**, **Rewind to start**, and **Stop** are grouped in the title. The title **Add** menu contains **Add Marker**, **Add Speed Lane**, and **Add Cuelist Lane**, and one **Add <Audio Player> Lane** for each patched Internal Audio Player, disabled once that player already owns a lane; it does not contain Add Playhead.

At **1×**, confirm the complete timeline fits without horizontal overflow. At maximum zoom, adjacent frames render approximately 15–20 CSS pixels apart. Scroll the production editor, scrub without changing live output, then explicitly seek the runtime to the editor playhead. From a new empty Timecode add a marker and audio-volume, Speed Group, and Cuelist lanes; add keyframes and a Cuelist clip, then edit BPM/phase, volume/fade/curve, Cue range, and clip start/end behavior. Copy and delete each supported timeline item and use editor Undo/Redo. In Settings, import marker CSV once with **Append** and once with **Replace**, using both frame-number and `HH:MM:SS:FF` positions; malformed or out-of-range rows must reject the complete import.

Inside the Cuelist clip, confirm every Cue in the selected range is visible as an individual Cue rather than the clip appearing as one opaque block. Each Cue shows its Cue/start position, its **In fade** range along the top of the lane, and its **Out fade** range along the bottom. Drag the start and end handles of both ranges. Every handle snaps to a real Timecode frame, impossible crossings and Cue-order conflicts are constrained or rejected with a concise cause, and the authoritative Cuelist—not a private Timecode copy—stores the resulting Cue delay and fade timing. Close and reopen both the Timecode and Cuelist views and confirm that each shows the same saved values.

## TIMECODE-002 — Cuelist clips and safe Preload

Create State Start and Cue Start clips for one Cuelist, including Hold and Release endings and one missing Cue reference. Give the Cues visibly different fixture values and overlapping In and Out timing. Start Timecode and confirm the Cuelist's authoritative current Cue, timing progress, and resolved output advance with the individual Cues shown in the clip; a transport that says Playing without changing its target fails this scenario. State Start reconstructs the intended tracked state, Cue Start executes the chosen Cue, Hold retains the final Cue until another authoritative action supersedes it, and Release removes the clip's Cuelist ownership at the exact end frame.

Pause during a Cue transition and confirm its current Cue, output, and both timing ranges freeze. Seek directly into the same transition and compare it with uninterrupted playback at that frame; the reconstructed current Cue, tracked state, In/Out progress, and output are identical. Resume and confirm the transition continues without restarting or duplicating a Cue trigger. Repeat with a backward seek and a loop across the clip boundary. A missing Cuelist, missing Cue, invalid range, unsupported Cue kind, or rejected timing remains visible on the lane with a concrete recovery cause; the clip is never shown as executing when it cannot drive its target. Scrubbing and editing in Preload do not reach live output; Preload Play begins only on Preload GO.
...
Import PCM WAV and MP3; confirm MP3 is normalized into the managed portable format and the original path is not required after Save As/export. Select one exact external source, an audio output, and a per-output latency trim. Verify audible Play, Pause/Resume, seek, Stop, Rewind to start, and loop on the real device. Exercise each configured source-loss policy and immediate re-lock. Restart the server: internal Timecodes remain stopped, while an armed externally synchronized Timecode reconstructs from the current external position. Unplugging or changing the selected audio device reports an actionable error without stopping the lighting output engine.
