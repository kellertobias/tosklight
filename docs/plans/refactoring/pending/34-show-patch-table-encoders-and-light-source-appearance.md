...
Implement this after Plan 33 unless the queue is explicitly reordered. Move this file to
`../doing/` before changing implementation code and complete the normal `## Result`, verification,
finished move, and semantic commit workflow.
...
snapshot contains stable identity, display name, optional manufacturer/code, and an sRGB preview
color so a show remains portable if another desk lacks that catalog revision. The first version
does not require spectral curves or measured transmission.
...
- built-in gel search/list with name plus manufacturer/code where available; and
...
4. multiply by the source/CCT tint and gel preview color in linear space;
...
- Gel colors use a validated canonical sRGB hex value; unknown built-ins retain fallback data.
...
- built-in/custom/open-white gel round-trip and unknown catalog fallback; and
