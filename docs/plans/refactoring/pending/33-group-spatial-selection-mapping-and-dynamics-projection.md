...
This plan is implemented after plans 31 and 32 unless the queue is explicitly reordered. Before
implementation, move this file to `../doing/` under the refactoring workflow.
...
- the Playback assignment owns master level, fader/button layout, Flash state, name/color
  presentation, revision, and runtime contribution;
...
- choosing another Playback function or **None**, or deleting that assignment, removes the Group
  Master without changing the Group;
- deleting a referenced Group follows the explicit reference-integrity rule and cannot leave a
  hidden master; and
- overlapping Group Master Playbacks retain independent levels and resolve eligible intensity by
  HTP. They do not overwrite, merge, or suppress one another.
...
- Remove `GroupDefinition.master` and legacy `playback_fader`/assigned-Playback pointers from live
  Group authority. Existing Playback Group targets remain authoritative and retain their level,
  layout, behavior, and HTP output.
- Before removing legacy fields, prove from representative shows whether an assigned Group Master
  level currently resides on the Playback, Group, or both. If a Playback depends on a Group value,
  migrate it deterministically into that Playback. With several Playbacks referencing one Group,
  preserve explicit per-Playback levels and do not copy one ambiguous Group value to all of them.
...
- SET + Playback remains settings-only regardless of an existing Group, fixture, empty, or mixed
  selection.
- Clearing/reassigning a Group Master Playback removes only that Playback contribution; overlapping
  masters continue HTP independently.
...
- independent overlapping Group Master HTP, clearing, reassignment, and explicit page addressing.
...
- [ ] Group Masters exist only through Playback assignments; Group settings/data own no master, and
      overlapping Playback masters remain independent HTP contributors.
...
- Inferring Playback assignment source from selection, the last Group, Highlight, or current
  Playback contents.
- Redesigning non-Group Playback functions or changing HTP arbitration beyond retaining independent
  overlapping Group Masters.
