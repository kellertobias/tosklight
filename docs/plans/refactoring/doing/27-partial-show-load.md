...
- **Show > Load > Partial Show Load** with another-show selection and a real catalog;
- an individually selectable portable-object list;
- cancellable catalog and preview requests, followed by a non-cancellable atomic apply phase;
- typed conflict choices for keeping, replacing, or duplicating individual objects and fixture
  profiles;
- dependency, blocker, fixture-profile, and managed-asset preview sections;
- authoritative source and target revision checks;
- typed reference rewriting for fixtures, Groups, Presets, Dynamics, Cuelists, Playbacks, Stage
  layouts, fixture profiles, and capability-owned object descriptors;
- candidate compilation, persistence/runtime failure rollback, and event reconciliation; and
- proof that a changed import commits one show revision while an identical import is a no-op.

The implementation is not yet complete against this plan because:

- the UI is a flat object checklist rather than selectable feature sections such as Preset
  families, Dynamics, Groups, Cuelists, Playbacks, complete Fixture Patch, and patch layers;
- the operator does not choose the documented top-level **Replace by position** or **Add to end**
  mode; current keep/replace/duplicate choices appear only after individual conflicts;
- complete-patch and one-patch-layer import are not represented as explicit catalog choices;
- patch-layer dependency preview and preservation of fixtures outside the selected layer lack
  retained acceptance coverage;
- File Manager entry-point parity and the complete operator vocabulary remain unproven; and
- the final acceptance pass must prove that the complete confirmed operation is exposed as one
  operator Undo step, not only one committed show revision.
