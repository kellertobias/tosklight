...
Removal is intentionally scoped to the client's home control desk. It deletes the control-desk record, its cascading per-show page and selected-playback rows, desk lock, and per-desk Update defaults. Optional Tauri screens are installation-wide rather than client-owned and are retained, as are portable shows, users, fixture data, persisted programmer data, other clients, global configuration, Virtual Playback assignments, and show-owned exclusion zones. A removed identity reconnecting with its stale desk ID receives a new control desk with the normal 8×1, three-button defaults and no prior playback layout.
