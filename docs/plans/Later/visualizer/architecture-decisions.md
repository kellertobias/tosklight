...
- The Tauri/React planner belongs under `apps/rig-planner`.
- The Godot renderer project belongs under `apps/visualizer-godot`.
- Reusable Rust behavior belongs in existing or new workspace crates under `crates/`.
...
Common controls come from `packages/ui`. Shared components remain presentational and accept typed values, view models and callbacks. Planner state, Tauri calls, document services and renderer coordination remain in app-owned adapters under `apps/rig-planner`.
...
## ADR-005 — Use Godot for the high-quality visualizer

Status: Provisional

Godot 4 Forward+ is the selected baseline for native high-quality rendering. Phase 0 must prove fixture counts, moving shadowed lights, fog, volumetric beams and cross-platform packaging before the choice becomes irreversible.

Reasoning: Godot supplies a cross-platform native renderer, scene graph, materials, shadows, volumetrics, particles and post-processing without building a complete engine from raw graphics APIs. It also keeps renderer implementation open source and approachable.
...
## ADR-006 — Run Godot as a supervised separate process

Status: Accepted

The Godot visualizer runs as a separate process with its own native window. The Tauri/Rust application owns the document, persistence, commands and undo; Godot owns only its mirrored render scene and GPU resources.

Reasoning:

- A renderer crash must not lose document work.
- Heavy GPU work and render-loop stalls must not block the editor.
- Godot and Tauri have independent window and event loops.
...
The canonical scene and domain rules live in Rust. SQLite is the canonical persisted show document. TypeScript and Godot consume generated types, view models or protocol projections rather than defining competing scenes.
...
The editor sends a versioned full snapshot when the renderer starts or resynchronizes, followed by batched incremental changes and high-rate live values. The protocol describes semantic renderer inputs without exposing Godot object paths as the public contract.

Reasoning: this keeps process recovery deterministic, prevents a chatty per-property IPC design and leaves room to replace or supplement Godot later.
...
Reasoning: this application spans Rust, TypeScript, SQL, WebGL, IPC and Godot. Deep abstraction layers would make cross-boundary behavior difficult to trace. A small public API and consistent domain vocabulary make calculations and renderer synchronization easier to test.
...
- pinned Godot minor version after the spike;
