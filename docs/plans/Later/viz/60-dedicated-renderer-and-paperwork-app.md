...
The preferred first technology to prove is a Rust `wgpu` render core. Keep the
core independent of its presentation target: the dedicated application owns a
standalone native window, while a future built-in Stage may instantiate the
same core in process and place a native GPU child surface over the rectangular
viewport reserved by its existing React pane. That embedded option keeps the
React components and window-management behavior, uses no frameset or nested
WebView, and must not make the desk depend on the dedicated application.
