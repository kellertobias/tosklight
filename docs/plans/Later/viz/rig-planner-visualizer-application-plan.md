...
The planning model is authoritative. Neither the WebGL viewport nor the Rust renderer owns show data; both consume projections of the canonical model.
...
6. **The renderer is replaceable.** The renderer is implemented in Rust, but communication and graphics-library adapters remain engine-neutral.
...
- Separate synchronized Rust 3D renderer.
- Wireframe, hidden-line and flat-shaded WebGL editor rendering.
- Rust visualizer with PBR materials, fixture movement, dimmer, color, zoom, focus, gobos, shadows, haze, bloom and basic temporal effects.
...
- If the renderer crashes, the document remains open and editable.
...
- Imported assets that fail in the renderer are replaced with bounds or placeholders and reported in an asset-errors panel.
...
├── apps/
│   ├── viz-editor/               Tauri + React editor application
│   ├── viz-renderer/             Rust renderer entry point and packaging
│   └── ...                       existing ToskLight applications
├── crates/
│   ├── viz/                      Viz domain, use cases, protocols, and adapters
│   └── shared/                   suite-wide core, fixture, show, and MVR crates
├── packages/
...
- Use the same domain terms in Rust, TypeScript, SQLite, and the renderer protocol. Do not translate one concept into several competing names.
...
Domain crates must not depend on React, Tauri, graphics libraries, or platform UI details. The React application and Rust renderer may consume shared schemas and generated types, but neither may redefine the canonical scene.
...
- Renderer adapter: one centrally tested coordinate-system conversion.
...
The filter assignment affects the WebGL representation, renderer beam/color rendering, fixture schedule and gel paperwork. User-defined filters may later add manufacturer/code, spectral data and transmission; the prototype only requires a named color and optional note.
...
A separate renderer view provides high-quality 3D visualization. All five views share:
...
An object can be selected and moved from any planning pane. Axis constraints derive from the active view. The renderer view participates in selection and picking, but document mutations still pass through the Rust command core.
...
The renderer interpolates pan, tilt and media timing locally. React is never in the live renderer frame loop.
...
- Renderer-ready artifact.
- Thumbnail and preview.
- License and source metadata.

The document references immutable library and asset revisions. Replacing a shared ToskLight library definition creates an explicit upgrade operation rather than silently changing old projects. The editor may cache derived render assets but must not fork the semantic fixture definition.
...
- Never let imported assets execute scripts or load executable plugins.
...
- Unit conversions.
- Transform hierarchy and renderer coordinate conversion.
...
- Rust renderer Direct3D 12, Metal, and Vulkan capability testing.
- Signed installer.
...
The Rust renderer can use a genuinely open-source stack, but code, fixture data, manufacturer models and imported user assets have separate licensing concerns.
...
- App-specific adapters remain in `apps/viz-editor`, with the workspace dependency rules passing in CI.
...
### Phase 3 — Rust visualizer prototype and v1
...
- Moving or patching a fixture updates the renderer without a full-scene reload.
...
11. Launch the Rust renderer as a supervised helper and send the scene snapshot.
12. Load the exact shared fixture/scenery model assets.
13. Move a fixture and prove all planning panes and the renderer update without scene reload.
