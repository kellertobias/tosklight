...
**Next priority 12 — specified but not yet authorized for implementation.** Its supported-scale,
output-isolation, and warm-UI
prerequisites were completed in the [major refactoring execution](../Done/major-refactoring-execution.DONE.md),
and the language, package model, execution
...
Macros are the twelfth item in the current [Next plan order](README.md), directly after
Timecode. Once all preceding queue items are complete and the operator authorizes implementation,
Macro work may begin without a separate hold release. Any native-extension telemetry or MQTT seam
consumed here must be reconciled with the capability boundary in the native-extension plan when
Macro work begins.
...
Wall-clock trigger rules remain owned by the completed Schedules implementation. Selective import
workflow remains owned by the completed Partial Show Load implementation. Their refactoring records
are consolidated in the [major refactoring execution](../Done/major-refactoring-execution.DONE.md);
those features consume the Macro contracts defined here.
...
[Native Hardware Extensions, USB DMX, and Telemetry](06-native-hardware-extensions-usb-dmx-and-telemetry.md).
...
Automated MQTT coverage may start an ephemeral local broker and create its runtime configuration
below the canonical `.artifacts/` paths. Broker configuration, credentials, certificates, retained
test data, and runtime state are test artifacts and must not be committed. A production or external
broker is not required for repository-owned acceptance.
...
[Schedules](../Done/major-refactoring-execution.DONE.md) remains solely responsible for wall-clock triggers, the authoritative
