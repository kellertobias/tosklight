...
- Fresnels at stage left and stage right, cross-lighting the stage from both sides.

The side Fresnel quantity was not fixed in the brief. Decide it before implementation and retain a
balanced left/right count. These static Profile and Fresnel fixtures are front-lighting tools; do
not silently include them in the moving-Profile family Groups defined below.
...
2. Establish the benchmark-only name and preserve the existing sustained workload and performance
   acceptance behavior.
3. Resolve the side Fresnel count and fourth ACL placement.
...
10. Create and verify the Busking, Programming, and Theater desktops.
11. After the royalty-free script is supplied, add it to the Theater desktop and program the
    script-specific Theater Cues.
12. Connect the canonical generated demo to the agreed built-in/default and product-demo consumers
    without duplicating the inventory.
13. Update focused documentation and executable coverage for the renamed benchmark interface and
...
1. How many Fresnels are used for the balanced stage-left/stage-right side-light pairs?
2. Where is the fourth ACL set mounted and which fan direction/composition does it use?
3. Which royalty-free Theater script should be stored, and which scenes or beats should become the
   first programmed Theater Cues?

Record the first two answers before patch implementation. The patch, Groups, presets, Playbacks,
and desktops may be completed before the script arrives, but Theater Cue programming remains
incomplete until the third input is provided.
