...
**Remediation pass (2026-07-22), HEAD now `41c11b2`: 179 passed, 83 failed** (262-test executed
population unchanged). +10/−10 vs pre-work (169/93), no regressions. Landed four commits:
`6206b1e` (re-hydrate `patched_fixture.definition` on the object read — clears clusters A+C and
FIXTURE-001; the A/C error class is fully eliminated), `b361a73` (default-family preset storage
key — clears HIGHLIGHT-001), and **`41c11b2` the A-fix Part 2** (v2 patch `profile_revisions` now
carry a server-resolved parameterized `profile_snapshot`; the scoped Patch store builds full
definitions from it — clears all four introduced Patch regressions: POSITION-HOME-001,
HIGHLIGHT-003, ENCODER-DISPLAY-001, PROG-002). The A-fix (both parts) and the introduced
regressions are done. Remaining clusters and the next slice are in
[`e2e-failure-diagnosis.md`](e2e-failure-diagnosis.md) → "Post-fix status"; the largest remaining
lever is cluster B's GO 404 (`Snapshot::read → unknown_programmer`, not `clear_command_line`),
which contaminates several otherwise-fixed A/C specs, then clusters D/E/F.
