...
| Slice 2 — renderer and one output | `Renderer.*`, `ofApp.*` render lifecycle | `media_domain::{geometry, clock}`, `media_render::{gpu, compositor, offscreen, texture, OutputRenderer}` | 14 deterministic off-screen reference renders over exact pixels | [Separate render adapter](media-renderer-reuse-decision.md), not `viz-render`; per-output render clock replacing the fixed 60 fps request; presentation mode chosen from the surface's real capabilities; the target monitor is actually applied; a failed source draws transparent instead of contributing black | see branch history | pending review |
