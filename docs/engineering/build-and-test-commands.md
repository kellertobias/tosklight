...
| `npm run install-all` | Release-builds ToskLight Control, ToskLight Architect and ToskLight Pixel for this machine's OS and installs them where applications live: `~/Applications/<Product>.app` on macOS (sealed), `%LOCALAPPDATA%\\Programs\\<Product>` with a Start menu shortcut on Windows, and `~/.local/opt/<product>` with a launcher in `~/.local/share/applications` on Linux. `-- --self-contained` places portable copies on the Desktop instead: the `.app` bundles on macOS, folders with `portable.txt` (data kept beside the executable, static C runtime) on Windows, and the desk as one AppImage plus portable Architect and Pixel folders on Linux. Every install moves the version already there into `.old/` beside it, replacing the one kept before, so each product keeps exactly one previous version; `-- --revert` (with `--self-contained` for the Desktop copies) deletes the current version and restores it. A running macOS copy is quit first. `-- --only control,architect,pixel` narrows it; `-- --dry-run` prints where each product would go. |
