...
| `npm run install-all` | Release-builds ToskLight Control, ToskLight Architect and ToskLight Pixel for this machine's OS and installs them where applications live: `~/Applications/<Product>.app` on macOS (sealed, replacing and first quitting a running copy), `%LOCALAPPDATA%\Programs\<Product>` with a Start menu shortcut on Windows, and `~/.local/opt/<product>` with a launcher in `~/.local/share/applications` on Linux. `-- --only control,architect,pixel` narrows it; `-- --dry-run` prints where each product would go. |
