...
const inputs = [
	"package-lock.json",
	"apps/light-desktop/index.html",
	"apps/light-desktop/package.json",
	"apps/light-desktop/tsconfig.json",
	"apps/light-desktop/vite.config.ts",
...
const signaturePath = path.join(
	...
	"cache",
	"frontend",
	"light-desktop-signature",
);
const outputEntry = path.join(artifactPaths.controlFrontend, "index.html");
...
if (
	...
	console.log(
		"Control UI inputs are unchanged; reusing the existing frontend bundle.",
	);
	process.exit(0);
}

console.log("Building control UI assets for the Light headless bundle...");
const build = spawnSync("npm", ["run", "build"], {
	cwd: path.join(repositoryRoot, "apps", "light-desktop"),
	env: process.env,
	stdio: "inherit",
});
if (build.error) {
	console.error(
		`error: failed to start the control UI build: ${build.error.message}`,
	);
	process.exit(1);
}
