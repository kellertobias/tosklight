...
export function renderPerformancePage(performance) {
	...
	return (
		`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta name="theme-color" content="#07090d">` +
		`<title>ToskLight Development</title><link rel="icon" href="../icon.png" type="image/png"><link rel="stylesheet" href="../site.css"><style>:root{color-scheme:dark;--bg:#07090d;--surface:#121823;--surface-soft:#171e2a;--line:#2a3443;--text:#f5f7fb;--muted:#a0aaba;--quiet:#727d8e;--amber:#5fd8dc;--blue:#72c7ff}body{font:16px/1.6 Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;background:radial-gradient(circle at 75% 0,rgba(79,117,173,.14),transparent 32rem),var(--bg);color:var(--text)}.development-main{width:min(100% - 3rem,76rem);margin:0 auto;padding-bottom:5rem}.performance-hero{padding:5rem 0 3.5rem}.performance-kicker{margin:0 0 .8rem;color:var(--amber);font-size:.76rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.performance-hero h1{max-width:850px;margin:0;font-size:clamp(3rem,7vw,5.6rem);line-height:.95;letter-spacing:-.055em}.performance-intro{max-width:720px;color:var(--muted);font-size:1.08rem}.status-line{margin-top:1.5rem;padding:1rem 1.2rem;border:1px solid var(--line);border-radius:.75rem;background:var(--surface)}.development-main h2{margin-top:4rem;font-size:1.75rem;letter-spacing:-.025em}.development-main a{color:var(--blue)}.table-scroll,.performance-table-scroll{overflow-x:auto}table{border-collapse:collapse;width:100%;margin:1rem 0 2rem;background:rgba(18,24,35,.65)}th,td{border:1px solid var(--line);padding:.7rem;text-align:left;vertical-align:top}.performance-compact td strong,.performance-compact th strong{display:block}.performance-compact td small,.performance-compact th small{display:block;margin-top:.25rem;color:#aab4bf;font-size:.72rem;line-height:1.3}.performance-row-healthy{background:#123326}.performance-row-warning{background:#3a3014}.performance-row-caution{background:#3a2414}.performance-row-degraded{background:#3b1d20}.performance-row-unknown{background:#252b32}.performance-legend{color:#c5ced8}` +
		...
		`code{background:#20262d;padding:.15rem .35rem}.healthy{color:#39d98a}.warning{color:#ffd166}.caution{color:#ff9f45}.degraded{color:#ff6b6b}.unknown{color:#9aa5b8}.developer-resources{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.8rem;margin-top:1.5rem}.resource-card{display:block;padding:1.15rem;border:1px solid var(--line);border-radius:.75rem;background:var(--surface);color:inherit;text-decoration:none}.resource-card:hover{border-color:var(--blue)}.resource-card strong{display:block;color:var(--text)}.resource-card span{display:block;margin-top:.3rem;color:var(--muted);font-size:.84rem}.performance-footer{margin-top:4rem;padding-top:2rem;border-top:1px solid var(--line);color:var(--quiet);font-size:.82rem}@media(max-width:700px){.development-main{width:min(100% - 2rem,76rem)}.performance-hero{padding-top:3.5rem}.developer-resources{grid-template-columns:1fr}.performance-hero h1{font-size:3.2rem}}</style><nav class="topbar shell" aria-label="Primary navigation"><a class="wordmark" href="../"><img src="../icon.png" alt=""><span>ToskLight</span></a><div class="nav-links"><a href="../#applications">Applications</a><a class="is-active" href="./">Development</a><a href="../manual/">Manual</a><a class="nav-cta" href="../downloads/">Downloads</a></div></nav><main class="development-main">` +
		`<header class="performance-hero"><p class="performance-kicker">Measured in public</p><h1>Development</h1><p class="performance-intro">Release evidence, test methodology, source, component stories, architecture tours, and operator-facing acceptance contracts in one place.</p><p class="status-line"><strong class="${escapePerformanceText(performance.status)}">${escapePerformanceText(performance.status.toUpperCase())}</strong>: ${escapePerformanceText(performance.summary)}</p></header><div id="evidence"></div>` +
		...
		reportLink +
		`<section id="developer-resources"><h2>Developer resources</h2><p>Start with the surface or contract closest to the change you want to make.</p><div class="developer-resources">` +
		`<a class="resource-card" href="../storybook/"><strong>Storybook →</strong><span>Reviewed operator surfaces, states, and deterministic product stories.</span></a>` +
		`<a class="resource-card" href="../safari/"><strong>Code Safari →</strong><span>Guided routes through architecture, persistence, tests, and one action end to end.</span></a>` +
