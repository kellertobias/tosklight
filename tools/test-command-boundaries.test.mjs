...
test("Playwright exposes only UI and API-exception suites", () => {
	...
	const pairedScenario = fs.readFileSync(
	...
	const scan = scanTestCommandBoundaries([
		{
			...
			source: 'await api.executeLegacyCommandLine("CUE 2");',
		},
	]);
...
test("a new direct literal WebSocket action is rejected", () => {
	...
	assert.match(
		failures[0],
		/new direct WebSocket action call site: tests\/new\.spec\.ts/u,
	);
	assert.match(
		failures[1],
		/new direct WebSocket action family use: programmer\.command_line/u,
	);
});
...
test("a grown compatibility call site is rejected while the baseline count passes", () => {
	...
	assert.match(
		failures[0],
		/compatibility command call site grew: tests\/cues\.spec\.ts has 2/u,
	);
	assert.match(
		failures[1],
		/compatibility command family use grew: cue_delete has 2/u,
	);
});
...
test("a removed compatibility call site reports a stale baseline entry so the ratchet tightens", () => {
	const failures = evaluateTestCommandBoundaries(
		scanTestCommandBoundaries([
			{
				path: "tests/cues.spec.ts",
				source: 'await api.executeCommandLine("GROUP 1 AT 50");',
			},
		]),
