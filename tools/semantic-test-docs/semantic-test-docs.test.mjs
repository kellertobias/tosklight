...
test("AST compiler preserves dynamic expressions and unknown helpers as diagnostics", async () => {
	const fixture = fs.mkdtempSync(path.join(os.tmpdir(), "light-semantic-docs-"));
...
`,
	...
	assert.equal(catalog.scenarios[0].migration.status, "migrated-semantic-world");
	assert.match(catalog.scenarios[0].steps[1].description, /<unresolved: `Dynamic/u);
	assert.deepEqual(
...
test("ambiguous or missing migration rows stay unresolved and branches stay visible", async () => {
	const fixture = fs.mkdtempSync(path.join(os.tmpdir(), "light-semantic-status-"));
	const tests = path.join(fixture, "tests");
	const docs = path.join(fixture, "docs/engineering");
	fs.mkdirSync(tests, { recursive: true });
	fs.mkdirSync(docs, { recursive: true });
...
test("Playwright JSON merges only into separate last-run metadata", () => {
	...
	assert.deepEqual(
		...
		{ status: "passed", durationMs: 12, attempts: 1 },
	);
	assert.equal(
		resultFor(results, "tests/example.spec.ts", "EXAMPLE-001", "expected outcome"),
		null,
	);
});
...
test("CLI check rejects stale alternate outputs and accepts a subsequent write", () => {
	const outputDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "light-doc-check-"));
