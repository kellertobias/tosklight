...
pub(super) fn migrate_desk(conn: &mut Connection) -> Result<(), StoreError> {
    ...
    add_column_if_missing(
        ...
        "hardware_led_brightness INTEGER NOT NULL DEFAULT 100 CHECK (hardware_led_brightness BETWEEN 0 AND 100)",
    )?;
    add_column_if_missing(
        &illumination_tx,
        "control_desks",
        "hardware_gooseneck_brightness",
        "hardware_gooseneck_brightness INTEGER NOT NULL DEFAULT 100 CHECK (hardware_gooseneck_brightness BETWEEN 0 AND 100)",
    )?;
    add_column_if_missing(
        &illumination_tx,
        "control_desks",
        "hardware_gooseneck_color",
        "hardware_gooseneck_color INTEGER NOT NULL DEFAULT 100 CHECK (hardware_gooseneck_color BETWEEN 0 AND 100)",
    )?;
