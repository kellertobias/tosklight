...
/// The frontend's wire types are not written by hand, and this is what keeps that true.
#[test]
fn checked_in_contract_artifacts_match_the_rust_dtos() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../..");
    for artifact in media_http::generated_artifacts() {
        let path = workspace_root.join(&artifact.path);
        let actual = fs::read_to_string(&path).unwrap_or_else(|error| {
            panic!(
                "generated wire artifact {} is missing: {error}; run cargo run -p media-http --example generate-contracts",
                artifact.path
            )
        });
        assert_eq!(
