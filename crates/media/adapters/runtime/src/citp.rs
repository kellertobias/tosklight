...
mod tests {
    ...
    fn available_port() -> u16 {
        ...
        let udp = std::net::UdpSocket::bind((std::net::Ipv4Addr::LOCALHOST, port)).unwrap();
        drop(udp);
        drop(listener);
        port
    }
    ...
    async fn light_desk_client_drives_and_reads_the_real_media_server() {
        let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../../../tests/fixtures/media-server-citp/library");
        let art_net_port = available_port();
        let citp_port = available_port();
        let mut configuration = MediaConfiguration::default();
        configuration.instance_id = media_application::configuration::InstanceId::new("citp-e2e");
        configuration.library.root = root;
        configuration.network.same_computer_preset = true;
        configuration.network.art_net_listen =
            SocketAddr::from((Ipv4Addr::LOCALHOST, art_net_port));
        configuration.network.citp_listen = SocketAddr::from((Ipv4Addr::LOCALHOST, citp_port));
        configuration.outputs[0].personality = media_domain::LayerPersonality::TwoLayers;
        configuration.outputs[0].universe = 1;
        configuration.outputs[0].start_address = 1;
        configuration.outputs[0].resolution = media_application::configuration::Resolution {
            width: 128,
            height: 72,
        };
        configuration.outputs[0].citp.source_name = Some("CITP Test Program".into());

        let state: SharedState = Arc::new(arc_swap::ArcSwap::from_pointee(crate::initial_state(
            &configuration,
        )));
        ...
        let rendering = {
            ...
            std::thread::spawn(move || crate::off_screen::run(&configuration, shared, shutdown))
        };

        let address = SocketAddr::from((Ipv4Addr::LOCALHOST, citp_port));
        let mut client = loop {
            match light_media::CitpClient::connect(address, Duration::from_secs(2)).await {
                Ok(client) => break client,
                Err(_) => tokio::time::sleep(Duration::from_millis(20)).await,
            }
        };
        let inspection = client.inspect().await.unwrap();
        assert_eq!(inspection.folders[0].id, 1);
        assert_eq!(inspection.files[0].folder_id, 1);
        assert_eq!(inspection.files[0].id, 1);
        assert_eq!(inspection.files[0].name, "CITP Test Pattern");

        let thumbnail = client
            .request_thumbnail(
                1,
                light_media::LibraryId {
                    level: 1,
                    ids: [1, 0, 0],
                },
                &[1],
        ...
        for layer in 0..2 {
            for channel in media_domain::personality::LAYER_CHANNELS {
                slots[layer * 35 + usize::from(channel.offset)] = match channel.resolution {
                    ...
                    _ => channel.default_value as u8,
                };
            }
        }
        for channel in media_domain::personality::MASTER_CHANNELS {
            slots[68 + usize::from(channel.offset)] = match channel.resolution {
                media_domain::personality::Resolution::Coarse => (channel.default_value >> 8) as u8,
                _ => channel.default_value as u8,
            };
        }
        slots[0] = 1;
        ...
        sender
            .send_to(
                &media_net::artnet::encode(1, 1, &slots),
                (Ipv4Addr::LOCALHOST, art_net_port),
            )
            .await
            .unwrap();
        tokio::time::timeout(Duration::from_secs(5), async {
            loop {
                let output = state.load();
                let layer = &output.outputs[0].layers[0];
                if layer.address.folder == 1
                    && layer.address.file == 1
        ...
        .await
        .expect("Art-Net must select 001/001 and the real output must decode it");

        let source = inspection.preview_sources[0].id;
        let preview = client.request_preview(source, 128, 72).await.unwrap();
        assert_eq!((preview.width, preview.height), (128, 72));
        assert!(preview.bytes.len() > 100, "Program returned an empty JPEG");
        ...
        assert!(
            distinct.len() >= 4,
            "Program must contain the rendered multicolour test pattern"
        );
        assert_ne!(
            preview.bytes, expected_thumbnail,
            "Program must be a compositor capture, not the library thumbnail"
        );
