// One motif, folded into rotating radial segments. Following the audio, the bass turns and lights
// it; on beat, every landed beat turns it on by half a segment and flashes it.
fn shade(p: vec2<f32>, uv: vec2<f32>) -> vec4<f32> {
    let segments = max(floor(count()), 2.0);
    let wedge = TAU / segments;
    var spin = seconds() * speed() * 0.3 + bass() * reactivity();
    var lift = bass() * 0.15;
    if on_beat() {
        spin = seconds() * speed() * 0.1 + beat_steps() * wedge * 0.5;
        lift = beat_pulse() * 0.3;
    }
    ...
    let bright = smoothstep(0.35, 0.85, motif * rings + lift);
