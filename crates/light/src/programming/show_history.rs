...
pub(super) struct ShowHistory {
    entries: HashMap<Uuid, Vec<ShowHistoryEntry>>,
}
...
impl ShowHistory {
    pub(super) fn record(
        ...
        let entries = self.entries.entry(desk_id).or_default();
...
pub(super) fn history_identity(context: &ActionContext) -> Result<SessionId, ActionError> {
...
mod tests {
    ...
    fn recording_history_is_desk_scoped_and_waits_for_newer_programmer_steps() {
        ...
        history.record(desk, 3, target.clone());

        assert!(history.next(other_desk, 3).is_none());
        assert!(history.next(desk, 4).is_none());
        assert_eq!(history.next(desk, 3), Some(target));
    }
    ...
    fn undo_retargets_the_previous_entry_for_the_new_object_revision() {
        ...
        let mut history = ShowHistory::default();
        history.record(desk, 0, target(show_id, 1));
        history.record(desk, 0, target(show_id, 2));

        history.finish(desk, &target(show_id, 2), 3);

        assert_eq!(history.next(desk, 0), Some(target(show_id, 3)));
    }
}
