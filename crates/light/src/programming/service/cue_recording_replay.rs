...
#[derive(Default)]
pub(super) struct CueRecordingReplayCache {
    ...
    pub(super) fn get(
        ...
        if entry.request != *request {
            return Err(ActionError::new(
                ...
                "request_id was already used for a different Cue recording action",
            ));
        }
...
fn retained_entry_bytes(
    ...
    let bodies = std::iter::once(&projections.cue_list)
        .chain(projections.playback.iter())
        .chain(projections.page.iter())
        .map(|projection| json_bytes(&projection.raw_body))
        .sum::<usize>();
    size_of::<ReplayKey>()
        .saturating_add(size_of::<ReplayEntry>())
        .saturating_add(key.request_id.capacity())
        .saturating_add(request.name.as_ref().map_or(0, String::capacity))
