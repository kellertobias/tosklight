...
`light` is a headless-first show-lighting engine and control server. It uses portable SQLite show files, session-isolated programmers, tracked cue lists, calibrated fixture attributes, and Art-Net/sACN network output.
...
./build archive          # separate release server and app ZIPs in artifacts/
./build archive install  # build archives and install/open ~/Applications/Light.app
```
...
- A session is created by selecting a configured username. Selection, command line, programmer values, blind/preview/highlight modes, editing context, and bounded undo/redo history belong to the session, not the user. Disconnected programmers remain present until explicitly cleared and survive server restart.
